...
static int cpkt_sasl_option_native(void *context, const char *plugin,
    const char *option, const char **result, unsigned *length) {
  cpkt_sasl_callback_owner *owner;
  unsigned long public_length;
  owner = (cpkt_sasl_callback_owner *) context;
  public_length = 0;
  if (owner == NULL || owner->callbacks.option == NULL) return SASL_FAIL;
  {
    int status;
    status = owner->callbacks.option(owner->callbacks.context,
        plugin, option, result, &public_length);
    if (length != NULL) *length = (unsigned) public_length;
    return status;
  }
}
...
static int cpkt_sasl_log_native(void *context, int level, const char *message) {
  cpkt_sasl_callback_owner *owner;
  owner = (cpkt_sasl_callback_owner *) context;
  return owner == NULL || owner->callbacks.log == NULL ? SASL_FAIL :
...
static int cpkt_sasl_path_native(void *context, const char **path) {
  cpkt_sasl_callback_owner *owner;
  owner = (cpkt_sasl_callback_owner *) context;
  return owner == NULL || owner->callbacks.plugin_path == NULL ? SASL_FAIL :
...
static int cpkt_sasl_verify_native(void *context, const char *path,
  ...
  cpkt_sasl_callback_owner *owner;
  owner = (cpkt_sasl_callback_owner *) context;
  return owner == NULL || owner->callbacks.verify_file == NULL ? SASL_FAIL :
...
static int cpkt_sasl_configuration_path_native(void *context, char **path) {
  cpkt_sasl_callback_owner *owner;
  owner = (cpkt_sasl_callback_owner *) context;
  return owner == NULL || owner->callbacks.configuration_path == NULL ? SASL_FAIL :
      owner->callbacks.configuration_path(owner->callbacks.context, path);
}
...
static int cpkt_sasl_simple_native(void *context, int id, const char **result,
  ...
  int status;
  owner = (cpkt_sasl_callback_owner *) context;
  if (owner == NULL || owner->callbacks.simple == NULL) return SASL_FAIL;
  public_length = 0;
  status = owner->callbacks.simple(owner->callbacks.context, id,
...
static int cpkt_sasl_secret_native(sasl_conn_t *native, void *context, int id,
  ...
  (void) native;
  owner = (cpkt_sasl_callback_owner *) context;
  return owner == NULL || owner->callbacks.secret == NULL ? SASL_FAIL :
      owner->callbacks.secret(owner->public_receiver,
          owner->callbacks.context, id, (void **) secret);
}
...
static int cpkt_sasl_challenge_native(void *context, int id,
  ...
  int status;
  owner = (cpkt_sasl_callback_owner *) context;
  if (owner == NULL || owner->callbacks.challenge == NULL) return SASL_FAIL;
  public_length = 0;
  status = owner->callbacks.challenge(owner->callbacks.context, id,
      challenge, prompt, default_result, result, &public_length);
  if (length != NULL) *length = (unsigned) public_length;
  return status;
}

static int cpkt_sasl_realm_native(void *context, int id,
    const char **available, const char **result) {
  cpkt_sasl_callback_owner *owner;
  owner = (cpkt_sasl_callback_owner *) context;
  return owner == NULL || owner->callbacks.realm == NULL ? SASL_FAIL :
      owner->callbacks.realm(owner->callbacks.context, id,
          (const char *const *) available, result);
}

static int cpkt_sasl_authorize_native(sasl_conn_t *native, void *context,
    const char *requested, unsigned requested_length, const char *identity,
    unsigned identity_length, const char *realm, unsigned realm_length,
    struct propctx *properties) {
  cpkt_sasl_callback_owner *owner;
  (void) properties;
  (void) native;
  owner = (cpkt_sasl_callback_owner *) context;
  return owner == NULL || owner->callbacks.authorize == NULL ? SASL_FAIL :
      owner->callbacks.authorize(owner->public_receiver,
          owner->callbacks.context, requested, (unsigned long) requested_length,
          identity, (unsigned long) identity_length, realm,
          (unsigned long) realm_length);
}

static int cpkt_sasl_check_password_native(sasl_conn_t *native, void *context,
    const char *user, const char *password, unsigned password_length,
    struct propctx *properties) {
  cpkt_sasl_callback_owner *owner;
  (void) properties;
  (void) native;
  owner = (cpkt_sasl_callback_owner *) context;
  return owner == NULL || owner->callbacks.check_password == NULL ? SASL_FAIL :
      owner->callbacks.check_password(owner->public_receiver,
          owner->callbacks.context, user, password,
          (unsigned long) password_length);
}

static int cpkt_sasl_set_password_native(sasl_conn_t *native, void *context,
    const char *user, const char *password, unsigned password_length,
    struct propctx *properties, unsigned flags) {
  cpkt_sasl_callback_owner *owner;
  (void) properties;
  (void) native;
  owner = (cpkt_sasl_callback_owner *) context;
  return owner == NULL || owner->callbacks.set_password == NULL ? SASL_FAIL :
      owner->callbacks.set_password(owner->public_receiver,
          owner->callbacks.context, user, password,
...
static int cpkt_sasl_canonicalize_native(sasl_conn_t *native, void *context,
    const char *input, unsigned input_length, unsigned flags, const char *realm,
    char *output, unsigned output_capacity, unsigned *output_length) {
  cpkt_sasl_callback_owner *owner;
  unsigned long public_length;
  int status;
  (void) native;
  owner = (cpkt_sasl_callback_owner *) context;
  if (owner == NULL || owner->callbacks.canonicalize == NULL) return SASL_FAIL;
  public_length = 0;
  status = owner->callbacks.canonicalize(owner->public_receiver,
      owner->callbacks.context, input, (unsigned long) input_length,
      (unsigned long) flags, realm, output, (unsigned long) output_capacity,
      &public_length);
  if (output_length != NULL) *output_length = (unsigned) public_length;
  return status;
}

static void cpkt_sasl_callbacks_build(sasl_callback_t *native,
    cpkt_sasl_callbacks *stored, const cpkt_sasl_callbacks *source,
    void *context) {
  int count;
  cpkt_sasl_native_callback callback;
  if (source == NULL) memset(stored, 0, sizeof(*stored));
  else *stored = *source;
...
#define CPKT_SASL_ADD_CALLBACK(identifier, public_field, native_field) \
  ...
  CPKT_SASL_ADD_CALLBACK(SASL_CB_GETCONFPATH, configuration_path, configuration_path)
  ...
  CPKT_SASL_ADD_CALLBACK(SASL_CB_SERVER_USERDB_CHECKPASS, check_password, check_password)
  CPKT_SASL_ADD_CALLBACK(SASL_CB_SERVER_USERDB_SETPASS, set_password, set_password)
...
static int cpkt_sasl_start(cpkt_sasl *self, const char *mechanisms,
    ...
    unsigned long *output_length, const char **mechanism) {
  cpkt_sasl_state *state;
  unsigned native_length;
  state = cpkt_sasl_state_for(self);
  if (interactions != NULL) *interactions = NULL;
  if (output != NULL) *output = NULL;
  if (output_length != NULL) *output_length = 0;
  if (mechanism != NULL) *mechanism = NULL;
  if (state == NULL || state->native == NULL) return SASL_BADPARAM;
  native_length = 0;
  if (state->is_server) {
  ...
  }
  {
    ...
    if (interactions != NULL) *interactions = (cpkt_sasl_interaction *) native_interactions;
    if (output_length != NULL) *output_length = (unsigned long) native_length;
    return status;
  }
}

static int cpkt_sasl_step(cpkt_sasl *self, const char *input,
    unsigned long input_length, cpkt_sasl_interaction **interactions,
  ...
  if (output != NULL) *output = NULL;
  if (output_length != NULL) *output_length = 0;
  if (state == NULL || state->native == NULL || input_length > UINT_MAX) return SASL_BADPARAM;
  native_length = 0;
  if (state->is_server) {
    int status;
...
static int cpkt_sasl_server_start(cpkt_sasl *self, const char *mechanism,
    const char *input, unsigned long input_length, const char **output,
  ...
  if (state == NULL || state->native == NULL || !state->is_server ||
      mechanism == NULL || input_length > UINT_MAX) return SASL_BADPARAM;
  native_length = 0;
...
static int cpkt_sasl_list_mechanisms(cpkt_sasl *self, const char *user,
    ...
    const char **result, unsigned long *length, int *count) {
  cpkt_sasl_state *state;
  unsigned native_length;
  int status;
  state = cpkt_sasl_state_for(self);
  ...
  if (state == NULL || state->native == NULL) return SASL_BADPARAM;
  native_length = 0;
  status = sasl_listmech(state->native, user, prefix, separator, suffix,
      result, &native_length, count);
  if (length != NULL) *length = (unsigned long) native_length;
  return status;
}

static int cpkt_sasl_encode(cpkt_sasl *self, const char *input,
    unsigned long input_length, const char **output, unsigned long *output_length) {
  cpkt_sasl_state *state;
  unsigned native_length;
  int status;
  state = cpkt_sasl_state_for(self);
...
static int cpkt_sasl_set_external_ssf(cpkt_sasl *self, unsigned long value) {
  ...
  state = cpkt_sasl_state_for(self);
  if (state == NULL || state->native == NULL || value > UINT_MAX) return SASL_BADPARAM;
  native_value = (unsigned) value;
  return sasl_setprop(state->native, SASL_SSF_EXTERNAL, &native_value);
}

static int cpkt_sasl_set_security_properties(cpkt_sasl *self,
  ...
  if (state == NULL || state->native == NULL || properties == NULL ||
      properties->minimum_ssf > UINT_MAX || properties->maximum_ssf > UINT_MAX ||
      properties->maximum_buffer_bytes > UINT_MAX ||
      properties->security_flags > UINT_MAX) return SASL_BADPARAM;
...
static const char *cpkt_sasl_error_detail(const cpkt_sasl *self) {
  cpkt_sasl_state *state;
  state = cpkt_sasl_state_for(self);
  return state == NULL || state->native == NULL ? NULL : sasl_errdetail(state->native);
}
...
void cpkt_sasl_close(cpkt_sasl *self) {
  ...
  if (self == NULL) return;
  state = cpkt_sasl_state_for(self);
  if (state != NULL && state->native != NULL) sasl_dispose(&state->native);
  free(state);
  self->internal = NULL;
  free(self);
}

/** Implements the documented public C89 SASL facade operation cpkt_sasl_version. */
...
/** Implements the documented public C89 SASL facade operation cpkt_sasl_error_string. */
const char *cpkt_sasl_error_string(int status, const char *languages,
    const char **language) {
  return sasl_errstring(status, languages, language);
}

/** Implements the documented public C89 SASL facade operation cpkt_sasl_set_path. */
int cpkt_sasl_set_path(int type, const char *path) {
  return sasl_set_path(type, (char *) path);
}

/** Implements the documented public C89 SASL facade operation cpkt_sasl_client_initialize. */
int cpkt_sasl_client_initialize(const cpkt_sasl_callbacks *callbacks) {
  if (callbacks != NULL && (callbacks->secret != NULL ||
      callbacks->authorize != NULL || callbacks->check_password != NULL ||
      callbacks->set_password != NULL || callbacks->canonicalize != NULL)) {
    return SASL_BADPARAM;
  }
  cpkt_sasl_callbacks_build(cpkt_sasl_client_callbacks.native,
      &cpkt_sasl_client_callbacks.callback_owner.callbacks, callbacks,
      &cpkt_sasl_client_callbacks.callback_owner);
  return sasl_client_init(callbacks == NULL ? NULL : cpkt_sasl_client_callbacks.native);
}

/** Implements the documented public C89 SASL facade operation cpkt_sasl_server_initialize. */
...
static cpkt_sasl *cpkt_sasl_new(int is_server, const char *service,
  ...
  self->internal = state;
  if (status_out != NULL) *status_out = SASL_OK;
  return self;
}

/** Implements the documented public C89 SASL facade operation cpkt_sasl_client_new. */
...
/** Implements the documented public C89 SASL facade operation cpkt_sasl_server_new. */
