...
typedef char cpkt_sqlite_i64_is_64_bits[
    (sizeof(sqlite3_int64) * CHAR_BIT == 64) ? 1 : -1];
...
typedef struct cpkt_sqlite_native_virtual_table cpkt_sqlite_native_virtual_table;
typedef struct cpkt_sqlite_native_virtual_cursor cpkt_sqlite_native_virtual_cursor;
typedef struct cpkt_sqlite_virtual_function_binding cpkt_sqlite_virtual_function_binding;
...
static cpkt_sqlite_vfs_file *cpkt_sqlite_vfs_file_from_native(sqlite3_file *file) {
...
static int cpkt_sqlite_vfs_file_close(sqlite3_file *file) {
  ...
  if (native_file == NULL || native_file->public_file.methods == NULL ||
      native_file->public_file.methods->close == NULL) return SQLITE_OK;
...
static int cpkt_sqlite_vfs_file_read(sqlite3_file *file, void *buffer,
  ...
  native_file = cpkt_sqlite_vfs_file_from_native(file);
  if (native_file == NULL || native_file->public_file.methods == NULL ||
      native_file->public_file.methods->read == NULL) return SQLITE_IOERR_READ;
  return native_file->public_file.methods->read(&native_file->public_file, buffer,
      byte_count, cpkt_sqlite_public_i64(offset));
}
...
static int cpkt_sqlite_vfs_file_write(sqlite3_file *file, const void *buffer,
  ...
  native_file = cpkt_sqlite_vfs_file_from_native(file);
  if (native_file == NULL || native_file->public_file.methods == NULL ||
      native_file->public_file.methods->write == NULL) return SQLITE_IOERR_WRITE;
  return native_file->public_file.methods->write(&native_file->public_file, buffer,
      byte_count, cpkt_sqlite_public_i64(offset));
}

static int cpkt_sqlite_vfs_file_truncate(sqlite3_file *file, sqlite3_int64 size) {
  cpkt_sqlite_vfs_file *native_file;
  native_file = cpkt_sqlite_vfs_file_from_native(file);
  if (native_file == NULL || native_file->public_file.methods == NULL ||
      native_file->public_file.methods->truncate == NULL) return SQLITE_IOERR_TRUNCATE;
  return native_file->public_file.methods->truncate(&native_file->public_file,
      cpkt_sqlite_public_i64(size));
}
...
static int cpkt_sqlite_vfs_file_sync(sqlite3_file *file, int flags) {
  ...
  native_file = cpkt_sqlite_vfs_file_from_native(file);
  if (native_file == NULL || native_file->public_file.methods == NULL ||
      native_file->public_file.methods->sync == NULL) return SQLITE_IOERR_FSYNC;
  return native_file->public_file.methods->sync(&native_file->public_file, flags);
}

static int cpkt_sqlite_vfs_file_size(sqlite3_file *file, sqlite3_int64 *size_out) {
  cpkt_sqlite_vfs_file *native_file;
  ...
  if (native_file == NULL || native_file->public_file.methods == NULL ||
      native_file->public_file.methods->size == NULL || size_out == NULL) return SQLITE_IOERR_FSTAT;
  public_size = cpkt_sqlite_i64_make(0, 0);
  status = native_file->public_file.methods->size(&native_file->public_file, &public_size);
  if (status == SQLITE_OK) *size_out = cpkt_sqlite_native_i64(public_size);
  return status;
}
...
static int cpkt_sqlite_vfs_file_lock(sqlite3_file *file, int level) {
  ...
  native_file = cpkt_sqlite_vfs_file_from_native(file);
  if (native_file == NULL || native_file->public_file.methods == NULL ||
      native_file->public_file.methods->lock == NULL) return SQLITE_IOERR_LOCK;
  return native_file->public_file.methods->lock(&native_file->public_file, level);
}
...
static int cpkt_sqlite_vfs_file_unlock(sqlite3_file *file, int level) {
  ...
  native_file = cpkt_sqlite_vfs_file_from_native(file);
  if (native_file == NULL || native_file->public_file.methods == NULL ||
      native_file->public_file.methods->unlock == NULL) return SQLITE_IOERR_UNLOCK;
  return native_file->public_file.methods->unlock(&native_file->public_file, level);
}

static int cpkt_sqlite_vfs_file_reserved_lock(sqlite3_file *file, int *result_out) {
  cpkt_sqlite_vfs_file *native_file;
  native_file = cpkt_sqlite_vfs_file_from_native(file);
  if (result_out != NULL) *result_out = 0;
  if (native_file == NULL || native_file->public_file.methods == NULL ||
      native_file->public_file.methods->check_reserved_lock == NULL) return SQLITE_IOERR_CHECKRESERVEDLOCK;
...
static int cpkt_sqlite_vfs_file_control(sqlite3_file *file, int operation,
  ...
  native_file = cpkt_sqlite_vfs_file_from_native(file);
  if (native_file == NULL || native_file->public_file.methods == NULL ||
      native_file->public_file.methods->control == NULL) return SQLITE_NOTFOUND;
...
static int cpkt_sqlite_vfs_file_sector_size(sqlite3_file *file) {
  ...
  native_file = cpkt_sqlite_vfs_file_from_native(file);
  if (native_file == NULL || native_file->public_file.methods == NULL ||
      native_file->public_file.methods->sector_size == NULL) return 0;
  return native_file->public_file.methods->sector_size(&native_file->public_file);
}
...
static int cpkt_sqlite_vfs_file_characteristics(sqlite3_file *file) {
  ...
  native_file = cpkt_sqlite_vfs_file_from_native(file);
  if (native_file == NULL || native_file->public_file.methods == NULL ||
      native_file->public_file.methods->characteristics == NULL) return 0;
  return native_file->public_file.methods->characteristics(&native_file->public_file);
}

static int cpkt_sqlite_vfs_file_shm_map(sqlite3_file *file, int page,
    int page_byte_count, int extend, void volatile **out) {
  cpkt_sqlite_vfs_file *native_file;
  native_file = cpkt_sqlite_vfs_file_from_native(file);
  if (out != NULL) *out = NULL;
  if (native_file == NULL || native_file->public_file.methods == NULL ||
      native_file->public_file.methods->shm_map == NULL) return SQLITE_IOERR_SHMMAP;
  return native_file->public_file.methods->shm_map(&native_file->public_file,
      page, page_byte_count, extend, out);
}
...
static int cpkt_sqlite_vfs_file_shm_lock(sqlite3_file *file, int offset,
  ...
  native_file = cpkt_sqlite_vfs_file_from_native(file);
  if (native_file == NULL || native_file->public_file.methods == NULL ||
      native_file->public_file.methods->shm_lock == NULL) return SQLITE_IOERR_SHMLOCK;
...
static int cpkt_sqlite_vfs_file_shm_unmap(sqlite3_file *file, int delete_flag) {
  ...
  native_file = cpkt_sqlite_vfs_file_from_native(file);
  if (native_file == NULL || native_file->public_file.methods == NULL ||
      native_file->public_file.methods->shm_unmap == NULL) return SQLITE_IOERR_SHMOPEN;
...
static int cpkt_sqlite_vfs_file_fetch(sqlite3_file *file, sqlite3_int64 offset,
  ...
  if (out != NULL) *out = NULL;
  if (native_file == NULL || native_file->public_file.methods == NULL ||
      native_file->public_file.methods->fetch == NULL) return SQLITE_OK;
  return native_file->public_file.methods->fetch(&native_file->public_file,
      cpkt_sqlite_public_i64(offset), byte_count, out);
}

static int cpkt_sqlite_vfs_file_unfetch(sqlite3_file *file, sqlite3_int64 offset,
    void *memory) {
  cpkt_sqlite_vfs_file *native_file;
  native_file = cpkt_sqlite_vfs_file_from_native(file);
  if (native_file == NULL || native_file->public_file.methods == NULL ||
      native_file->public_file.methods->unfetch == NULL) return SQLITE_OK;
  return native_file->public_file.methods->unfetch(&native_file->public_file,
      cpkt_sqlite_public_i64(offset), memory);
}

static int cpkt_sqlite_vfs_open_native(sqlite3_vfs *vfs,
    sqlite3_filename name, sqlite3_file *file, int flags, int *flags_out) {
  cpkt_sqlite_vfs_binding *binding;
  cpkt_sqlite_vfs_file *native_file;
  cpkt_sqlite_vfs *public_vfs;
  int status;
  ...
  if (native_file == NULL || public_vfs == NULL || public_vfs->methods.open == NULL) {
  ...
  if (native_file->public_file.methods == NULL) return SQLITE_MISUSE;
  native_file->native_methods.iVersion = native_file->public_file.methods->version;
  ...
  native_file->native_methods.xCheckReservedLock = cpkt_sqlite_vfs_file_reserved_lock;
  ...
  native_file->native_methods.xDeviceCharacteristics = cpkt_sqlite_vfs_file_characteristics;
...
static int cpkt_sqlite_vfs_delete_native(sqlite3_vfs *vfs, const char *name,
  ...
  if (public_vfs == NULL || public_vfs->methods.delete_file == NULL) return SQLITE_IOERR_DELETE;
...
static int cpkt_sqlite_vfs_access_native(sqlite3_vfs *vfs, const char *name,
  ...
  public_vfs = cpkt_sqlite_vfs_public(vfs);
  if (result_out != NULL) *result_out = 0;
  if (public_vfs == NULL || public_vfs->methods.access == NULL) return SQLITE_IOERR_ACCESS;
...
static int cpkt_sqlite_vfs_full_path_native(sqlite3_vfs *vfs, const char *name,
  ...
  cpkt_sqlite_vfs *public_vfs;
  public_vfs = cpkt_sqlite_vfs_public(vfs);
  if (public_vfs == NULL || public_vfs->methods.full_path == NULL) return SQLITE_CANTOPEN;
  return public_vfs->methods.full_path(public_vfs, name, output_byte_count, output);
}

static void *cpkt_sqlite_vfs_dl_open_native(sqlite3_vfs *vfs, const char *name) {
  cpkt_sqlite_vfs *public_vfs;
  public_vfs = cpkt_sqlite_vfs_public(vfs);
  return public_vfs == NULL || public_vfs->methods.dl_open == NULL ? NULL :
...
static void (*cpkt_sqlite_vfs_dl_symbol_native(sqlite3_vfs *vfs, void *handle,
  ...
  cpkt_sqlite_vfs *public_vfs;
  public_vfs = cpkt_sqlite_vfs_public(vfs);
  return public_vfs == NULL || public_vfs->methods.dl_symbol == NULL ? NULL :
...
static int cpkt_sqlite_vfs_randomness_native(sqlite3_vfs *vfs, int byte_count,
  ...
  cpkt_sqlite_vfs *public_vfs;
  public_vfs = cpkt_sqlite_vfs_public(vfs);
  return public_vfs == NULL || public_vfs->methods.randomness == NULL ? 0 :
...
static int cpkt_sqlite_vfs_sleep_native(sqlite3_vfs *vfs, int microseconds) {
  cpkt_sqlite_vfs *public_vfs;
  public_vfs = cpkt_sqlite_vfs_public(vfs);
  return public_vfs == NULL || public_vfs->methods.sleep == NULL ? 0 :
...
static int cpkt_sqlite_vfs_current_time_native(sqlite3_vfs *vfs, double *out) {
  cpkt_sqlite_vfs *public_vfs;
  public_vfs = cpkt_sqlite_vfs_public(vfs);
  return public_vfs == NULL || public_vfs->methods.current_time == NULL ? SQLITE_ERROR :
...
static int cpkt_sqlite_vfs_last_error_native(sqlite3_vfs *vfs, int byte_count,
  ...
  cpkt_sqlite_vfs *public_vfs;
  public_vfs = cpkt_sqlite_vfs_public(vfs);
  return public_vfs == NULL || public_vfs->methods.last_error == NULL ? 0 :
...
static int cpkt_sqlite_vfs_current_time_i64_native(sqlite3_vfs *vfs,
  ...
  public_vfs = cpkt_sqlite_vfs_public(vfs);
  if (out != NULL) *out = 0;
  if (public_vfs == NULL || public_vfs->methods.current_time_i64 == NULL || out == NULL) {
    return SQLITE_ERROR;
  }
  public_time = cpkt_sqlite_i64_make(0, 0);
  status = public_vfs->methods.current_time_i64(public_vfs, &public_time);
  if (status == SQLITE_OK) *out = cpkt_sqlite_native_i64(public_time);
  return status;
}

static int cpkt_sqlite_vfs_set_system_call_native(sqlite3_vfs *vfs,
    const char *name, sqlite3_syscall_ptr symbol) {
  cpkt_sqlite_vfs *public_vfs;
  public_vfs = cpkt_sqlite_vfs_public(vfs);
...
static sqlite3_syscall_ptr cpkt_sqlite_vfs_get_system_call_native(sqlite3_vfs *vfs,
    const char *name) {
  cpkt_sqlite_vfs *public_vfs;
  public_vfs = cpkt_sqlite_vfs_public(vfs);
  return public_vfs == NULL || public_vfs->methods.get_system_call == NULL ? NULL :
...
static const char *cpkt_sqlite_vfs_next_system_call_native(sqlite3_vfs *vfs,
  ...
  cpkt_sqlite_vfs *public_vfs;
  public_vfs = cpkt_sqlite_vfs_public(vfs);
  return public_vfs == NULL || public_vfs->methods.next_system_call == NULL ? NULL :
      public_vfs->methods.next_system_call(public_vfs, name);
}

static sqlite3_stmt *cpkt_sqlite_native_statement(const cpkt_sqlite_statement *self) {
...
static sqlite3_backup *cpkt_sqlite_native_backup(const cpkt_sqlite_backup *self) {
...
static sqlite3_context *cpkt_sqlite_native_context(const cpkt_sqlite_context *self) {
  return self == NULL ? NULL : (sqlite3_context *) self->context;
}

static sqlite3_session *cpkt_sqlite_native_session(const cpkt_sqlite_session *self) {
  return self == NULL ? NULL : (sqlite3_session *) self->session;
}

static sqlite3_changeset_iter *cpkt_sqlite_native_iterator(
    const cpkt_sqlite_changeset_iterator *self) {
  return self == NULL ? NULL : (sqlite3_changeset_iter *) self->iterator;
}

static sqlite3_rebaser *cpkt_sqlite_native_rebaser(const cpkt_sqlite_rebaser *self) {
  return self == NULL ? NULL : (sqlite3_rebaser *) self->rebaser;
}

static sqlite3_changegroup *cpkt_sqlite_native_changegroup(
    const cpkt_sqlite_changegroup *self) {
...
static sqlite3_filename cpkt_sqlite_native_filename(const cpkt_sqlite_filename *self) {
...
static const Fts5ExtensionApi *cpkt_sqlite_native_fts5_extension_api(
    const cpkt_sqlite_fts5_context *self) {
  return self == NULL ? NULL : (const Fts5ExtensionApi *) self->api;
}

static Fts5Context *cpkt_sqlite_native_fts5_context(const cpkt_sqlite_fts5_context *self) {
...
static void cpkt_sqlite_global_lock(sqlite3_mutex *mutex) {
  if (mutex != NULL) sqlite3_mutex_enter(mutex);
}

static void cpkt_sqlite_global_unlock(sqlite3_mutex *mutex) {
  if (mutex != NULL) sqlite3_mutex_leave(mutex);
}

static void cpkt_sqlite_memory_alarm_trampoline(void *context,
    sqlite3_int64 requested_bytes, int prior_allocation_failed) {
...
static void cpkt_sqlite_auto_extension_free(
...
static int cpkt_sqlite_auto_extension_trampoline(sqlite3 *native_database,
    char **error_out, const sqlite3_api_routines *api) {
  cpkt_sqlite_auto_extension_binding *binding;
  ...
  if (database == NULL) return SQLITE_NOMEM;
  mutex = cpkt_sqlite_global_mutex();
  cpkt_sqlite_global_lock(mutex);
  count = 0;
  binding = cpkt_sqlite_auto_extension_head;
  while (binding != NULL) {
    if (binding->registered) ++count;
    binding = binding->next;
  }
  snapshot = count == 0 ? NULL : (cpkt_sqlite_auto_extension_binding **) calloc(
  ...
  for (index = 0; index < count; ++index) {
    binding = snapshot[index];
    if (binding->active_callbacks > 0) --binding->active_callbacks;
...
static int cpkt_sqlite_auto_extension_register(
    cpkt_sqlite_auto_extension *self) {
  cpkt_sqlite_auto_extension_binding *binding;
  sqlite3_mutex *mutex;
  int status;
  if (self == NULL || self->internal == NULL) return SQLITE_MISUSE;
  ...
  if (!cpkt_sqlite_auto_extension_trampoline_registered) {
    status = sqlite3_auto_extension((void (*)(void))
        cpkt_sqlite_auto_extension_trampoline);
    if (status != SQLITE_OK) {
      cpkt_sqlite_global_unlock(mutex);
      return status;
    }
...
static int cpkt_sqlite_auto_extension_cancel(
    cpkt_sqlite_auto_extension *self) {
  cpkt_sqlite_auto_extension_binding *binding;
  ...
  while (current != NULL) {
    if (current->registered) any_registered = 1;
    current = current->next;
  }
  status = SQLITE_OK;
  if (!any_registered && cpkt_sqlite_auto_extension_trampoline_registered) {
    status = sqlite3_cancel_auto_extension((void (*)(void))
        cpkt_sqlite_auto_extension_trampoline);
...
static void cpkt_sqlite_auto_extension_close(cpkt_sqlite_auto_extension *self) {
  ...
  cpkt_sqlite_auto_extension_binding *binding;
  sqlite3_mutex *mutex;
  if (self == NULL || self->internal == NULL) return;
  (void) cpkt_sqlite_auto_extension_cancel(self);
  binding = (cpkt_sqlite_auto_extension_binding *) self->internal;
  mutex = cpkt_sqlite_global_mutex();
  cpkt_sqlite_global_lock(mutex);
  link = &cpkt_sqlite_auto_extension_head;
  while (*link != NULL && *link != binding) link = &(*link)->next;
...
static void cpkt_sqlite_retain_child(cpkt_sqlite *database) {
  ...
  if (state != NULL) ++state->child_count;
}
...
static void cpkt_sqlite_release_child(cpkt_sqlite *database) {
  ...
  if (state->child_count > 0) --state->child_count;
  if (state->closing && state->child_count == 0) cpkt_sqlite_finish_close(database);
}
...
static int cpkt_sqlite_busy_trampoline(void *context, int retry_count) {
  ...
  if (state == NULL || state->busy_callback == NULL) return 0;
...
static int cpkt_sqlite_authorizer_trampoline(void *context, int action,
  ...
  cpkt_sqlite_state *state;
  state = (cpkt_sqlite_state *) context;
  if (state == NULL || state->authorizer_callback == NULL) return SQLITE_OK;
  return state->authorizer_callback(state->authorizer_context, action,
      detail1, detail2, database, trigger);
}

static void cpkt_sqlite_collation_needed_trampoline(void *context,
    sqlite3 *native_database, int text_representation, const char *name) {
  cpkt_sqlite_state *state;
  (void) native_database;
  state = (cpkt_sqlite_state *) context;
  if (state != NULL && state->collation_needed_callback != NULL) {
    state->collation_needed_callback(state->collation_needed_context, state->database,
        (unsigned long) text_representation, name);
  }
}

static void cpkt_sqlite_collation_needed16_trampoline(void *context,
    sqlite3 *native_database, int text_representation, const void *name) {
  cpkt_sqlite_state *state;
  (void) native_database;
  state = (cpkt_sqlite_state *) context;
  if (state != NULL && state->collation_needed16_callback != NULL) {
    state->collation_needed16_callback(state->collation_needed_context, state->database,
        (unsigned long) text_representation, name);
  }
}
...
static int cpkt_sqlite_trace_trampoline(unsigned int event, void *context,
  ...
  state = (cpkt_sqlite_state *) context;
  if (state == NULL || state->trace_callback == NULL) return 0;
  sql = NULL;
  elapsed = 0;
  if (event == SQLITE_TRACE_STMT) {
    ...
    if (second != NULL) elapsed = *(sqlite3_uint64 *) second;
  } else if (event == SQLITE_TRACE_ROW) {
    sql = sqlite3_sql((sqlite3_stmt *) first);
  }
  state->trace_callback(state->trace_context, state->database, (unsigned long) event,
      sql, cpkt_sqlite_public_u64(elapsed));
  return 0;
}

static void cpkt_sqlite_legacy_trace_trampoline(void *context, const char *sql) {
  cpkt_sqlite_state *state;
  state = (cpkt_sqlite_state *) context;
...
static void cpkt_sqlite_legacy_profile_trampoline(void *context, const char *sql,
...
static void cpkt_sqlite_unlock_notify_trampoline(void **arguments,
  ...
  int index;
  if (arguments == NULL || argument_count <= 0) return;
  ...
  for (index = 0; index < argument_count; ++index) {
    ...
    if (binding != NULL) {
      if (contexts != NULL) contexts[index] = binding->context;
  ...
  for (index = 0; index < argument_count; ++index) {
    ...
    if (binding != NULL && binding->callback != NULL) {
      binding->callback(binding->context,
          contexts == NULL ? 0 : argument_count, contexts);
    }
  }
  free(contexts);
  for (index = 0; index < argument_count; ++index) free(bindings[index]);
}

static int cpkt_sqlite_rtree_geometry_trampoline(sqlite3_rtree_geometry *native,
    int coordinate_count, sqlite3_rtree_dbl *coordinates, int *within_out) {
  cpkt_sqlite_rtree_geometry_binding *binding;
  cpkt_sqlite_rtree_geometry public_geometry;
  int status;
  if (native == NULL) return SQLITE_MISUSE;
  binding = (cpkt_sqlite_rtree_geometry_binding *) native->pContext;
  if (binding == NULL || binding->callback == NULL) return SQLITE_MISUSE;
  ...
  status = binding->callback(binding->context, &public_geometry, coordinate_count,
      coordinates, within_out);
...
static void cpkt_sqlite_rtree_query_destroy(void *context) {
  ...
  if (binding == NULL) return;
  if (binding->destroy != NULL) binding->destroy(binding->context);
  free(binding);
}

static int cpkt_sqlite_rtree_query_trampoline(sqlite3_rtree_query_info *native) {
  cpkt_sqlite_rtree_query_binding *binding;
  ...
  if (native->nParam > 0 && native->apSqlParam != NULL) {
    values = (cpkt_sqlite_value *) calloc((size_t) native->nParam, sizeof(*values));
...
static int cpkt_sqlite_progress_trampoline(void *context) {
  cpkt_sqlite_state *state;
  state = (cpkt_sqlite_state *) context;
  if (state == NULL || state->progress_callback == NULL) return 0;
...
static int cpkt_sqlite_commit_trampoline(void *context) {
  cpkt_sqlite_state *state;
  state = (cpkt_sqlite_state *) context;
  if (state == NULL || state->commit_callback == NULL) return 0;
...
static void cpkt_sqlite_update_trampoline(void *context, int operation,
    const char *database_name, const char *table_name, sqlite3_int64 row_id) {
  cpkt_sqlite_state *state;
  state = (cpkt_sqlite_state *) context;
...
static int cpkt_sqlite_wal_trampoline(void *context, sqlite3 *native_database,
  ...
  (void) native_database;
  state = (cpkt_sqlite_state *) context;
  if (state == NULL || state->wal_callback == NULL) return SQLITE_OK;
  return state->wal_callback(state->wal_context, state->database, database_name, page_count);
}

static void cpkt_sqlite_preupdate_trampoline(void *context, sqlite3 *native_database,
    int operation, const char *database_name, const char *table_name,
    sqlite3_int64 old_row_id, sqlite3_int64 new_row_id) {
  cpkt_sqlite_state *state;
  (void) native_database;
  state = (cpkt_sqlite_state *) context;
  if (state != NULL && state->preupdate_callback != NULL) {
    state->preupdate_callback(state->preupdate_context, state->database, operation,
        database_name, table_name, cpkt_sqlite_public_i64(old_row_id),
...
static void cpkt_sqlite_function_destroy(void *context) {
  ...
  if (binding == NULL) return;
  if (binding->destroy != NULL) binding->destroy(binding->user_data);
  free(binding);
}
...
static void cpkt_sqlite_auxdata_destroy(void *context) {
  ...
  if (binding == NULL) return;
  if (binding->destroy != NULL) binding->destroy(binding->data);
  free(binding);
}
...
static int cpkt_sqlite_collation_trampoline(void *context, int left_byte_count,
  ...
  cpkt_sqlite_collation_binding *binding;
  binding = (cpkt_sqlite_collation_binding *) context;
  if (binding == NULL || binding->compare == NULL) return 0;
...
static unsigned int cpkt_sqlite_autovacuum_trampoline(void *context,
    const char *schema_name, unsigned int database_page_count,
    unsigned int free_page_count, unsigned int page_byte_count) {
  cpkt_sqlite_autovacuum_binding *binding;
  unsigned long result;
  binding = (cpkt_sqlite_autovacuum_binding *) context;
  if (binding == NULL || binding->callback == NULL) return 0;
  result = binding->callback(binding->context, schema_name,
      (unsigned long) database_page_count, (unsigned long) free_page_count,
...
static void cpkt_sqlite_function_call(sqlite3_context *native_context,
  ...
  if (binding == NULL || callback == NULL) return;
  public_context.context = native_context;
  public_context.database = binding->database;
  values = NULL;
  arguments = NULL;
  if (argument_count > 0) {
    values = (cpkt_sqlite_value *) calloc((size_t) argument_count, sizeof(*values));
    arguments = (cpkt_sqlite_value **) calloc((size_t) argument_count, sizeof(*arguments));
...
static void cpkt_sqlite_function_scalar_trampoline(sqlite3_context *context,
    int argument_count, sqlite3_value **arguments) {
  cpkt_sqlite_function_binding *binding;
...
static void cpkt_sqlite_iterator_init(cpkt_sqlite_changeset_iterator *iterator,
    sqlite3_changeset_iter *native_iterator, int borrowed);
...
static int cpkt_sqlite_changeset_filter_trampoline(void *context,
  ...
  if (apply_context == NULL || apply_context->filter == NULL) return 1;
  return apply_context->filter(apply_context->user_context, table_name);
}

static int cpkt_sqlite_changeset_iterator_filter_trampoline(void *context,
    sqlite3_changeset_iter *native_iterator) {
  cpkt_sqlite_changeset_apply_context *apply_context;
  cpkt_sqlite_changeset_iterator iterator;
  apply_context = (cpkt_sqlite_changeset_apply_context *) context;
  if (apply_context == NULL || apply_context->iterator_filter == NULL) return 1;
...
static int cpkt_sqlite_iterator_borrowed_close(cpkt_sqlite_changeset_iterator *self) {
  (void) self;
  return CPKT_SQLITE_MISUSE;
}

static int cpkt_sqlite_changeset_conflict_trampoline(void *context, int conflict_kind,
    sqlite3_changeset_iter *native_iterator) {
  cpkt_sqlite_changeset_apply_context *apply_context;
  cpkt_sqlite_changeset_iterator iterator;
  apply_context = (cpkt_sqlite_changeset_apply_context *) context;
  if (apply_context == NULL || apply_context->conflict == NULL) return SQLITE_CHANGESET_ABORT;
  cpkt_sqlite_iterator_init(&iterator, native_iterator, 1);
  return apply_context->conflict(apply_context->user_context, conflict_kind, &iterator);
}
...
static int cpkt_sqlite_stream_input_trampoline(void *context, void *buffer,
  ...
  if (input_context == NULL || input_context->callback == NULL) return CPKT_SQLITE_MISUSE;
  return input_context->callback(input_context->user_context, buffer, byte_count);
}

static int cpkt_sqlite_stream_output_trampoline(void *context, const void *buffer,
    int byte_count) {
  ...
  return output_context->callback(output_context->user_context, buffer, byte_count);
}

static int cpkt_sqlite_iterator_stream_input_trampoline(void *context, void *buffer,
    int *byte_count) {
  cpkt_sqlite_changeset_iterator *iterator;
  iterator = (cpkt_sqlite_changeset_iterator *) context;
  if (iterator == NULL || iterator->input == NULL) return CPKT_SQLITE_MISUSE;
  return iterator->input(iterator->input_context, buffer, byte_count);
}

static int cpkt_sqlite_exec_callback(
    void *context, int column_count, char **values, char **names) {
...
static int cpkt_sqlite_statement_bind_null(cpkt_sqlite_statement *self, int parameter_index) {
  return sqlite3_bind_null(cpkt_sqlite_native_statement(self), parameter_index);
}

static int cpkt_sqlite_statement_bind_int(cpkt_sqlite_statement *self, int parameter_index, int value) {
  return sqlite3_bind_int(cpkt_sqlite_native_statement(self), parameter_index, value);
}

static int cpkt_sqlite_statement_bind_i64(
    cpkt_sqlite_statement *self, int parameter_index, cpkt_sqlite_i64 value) {
  return sqlite3_bind_int64(cpkt_sqlite_native_statement(self), parameter_index,
      cpkt_sqlite_native_i64(value));
}

static int cpkt_sqlite_statement_bind_double(
    cpkt_sqlite_statement *self, int parameter_index, double value) {
  return sqlite3_bind_double(cpkt_sqlite_native_statement(self), parameter_index, value);
}

static int cpkt_sqlite_statement_bind_text(
    cpkt_sqlite_statement *self, int parameter_index, const char *value, int byte_count) {
...
static int cpkt_sqlite_statement_bind_text16(cpkt_sqlite_statement *self,
    int parameter_index, const void *value, int byte_count) {
  return sqlite3_bind_text16(cpkt_sqlite_native_statement(self), parameter_index,
      value, byte_count, SQLITE_TRANSIENT);
}

static int cpkt_sqlite_statement_bind_text_u64(cpkt_sqlite_statement *self,
    int parameter_index, const void *value, cpkt_sqlite_u64 byte_count,
    unsigned long encoding) {
  return sqlite3_bind_text64(cpkt_sqlite_native_statement(self), parameter_index,
      (const char *) value, cpkt_sqlite_native_u64(byte_count), SQLITE_TRANSIENT,
      (unsigned char) encoding);
}

static int cpkt_sqlite_statement_bind_text_owned(cpkt_sqlite_statement *self,
    int parameter_index, const char *value, int byte_count,
    cpkt_sqlite_destroy_callback destroy) {
  return sqlite3_bind_text(cpkt_sqlite_native_statement(self), parameter_index,
      value, byte_count, destroy);
}

static int cpkt_sqlite_statement_bind_blob(
    cpkt_sqlite_statement *self, int parameter_index, const void *value, int byte_count) {
...
static int cpkt_sqlite_statement_bind_blob_u64(cpkt_sqlite_statement *self,
    int parameter_index, const void *value, cpkt_sqlite_u64 byte_count) {
  return sqlite3_bind_blob64(cpkt_sqlite_native_statement(self), parameter_index,
      value, cpkt_sqlite_native_u64(byte_count), SQLITE_TRANSIENT);
}

static int cpkt_sqlite_statement_bind_blob_owned(cpkt_sqlite_statement *self,
    int parameter_index, const void *value, int byte_count,
    cpkt_sqlite_destroy_callback destroy) {
  return sqlite3_bind_blob(cpkt_sqlite_native_statement(self), parameter_index,
      value, byte_count, destroy);
...
static int cpkt_sqlite_statement_bind_zero_blob(cpkt_sqlite_statement *self,
    int parameter_index, int byte_count) {
  return sqlite3_bind_zeroblob(cpkt_sqlite_native_statement(self), parameter_index,
      byte_count);
}

static int cpkt_sqlite_statement_bind_zero_blob_u64(cpkt_sqlite_statement *self,
    int parameter_index, cpkt_sqlite_u64 byte_count) {
  return sqlite3_bind_zeroblob64(cpkt_sqlite_native_statement(self), parameter_index,
      cpkt_sqlite_native_u64(byte_count));
}

static int cpkt_sqlite_statement_bind_value(cpkt_sqlite_statement *self,
    int parameter_index, const cpkt_sqlite_value *value) {
  if (cpkt_sqlite_native_value(value) == NULL) return CPKT_SQLITE_MISUSE;
  return sqlite3_bind_value(cpkt_sqlite_native_statement(self), parameter_index,
      cpkt_sqlite_native_value(value));
}

static int cpkt_sqlite_statement_bind_pointer(cpkt_sqlite_statement *self,
    int parameter_index, void *value, const char *type_name,
    cpkt_sqlite_destroy_callback destroy) {
  if (type_name == NULL) return CPKT_SQLITE_MISUSE;
  return sqlite3_bind_pointer(cpkt_sqlite_native_statement(self), parameter_index,
      value, type_name, destroy);
}

static int cpkt_sqlite_statement_bind_carray(cpkt_sqlite_statement *self,
    int parameter_index, void *data, int element_count, int element_type,
    cpkt_sqlite_destroy_callback destroy) {
  return sqlite3_carray_bind(cpkt_sqlite_native_statement(self), parameter_index,
      data, element_count, element_type, destroy);
}
...
static int cpkt_sqlite_statement_bind_carray_with_context(
  ...
  return sqlite3_carray_bind_v2(cpkt_sqlite_native_statement(self), parameter_index,
      data, element_count, element_type, destroy, destroy_context);
}
...
static int cpkt_sqlite_statement_parameter_count(const cpkt_sqlite_statement *self) {
  return sqlite3_bind_parameter_count(cpkt_sqlite_native_statement(self));
}

static const char *cpkt_sqlite_statement_parameter_name(
    const cpkt_sqlite_statement *self, int parameter_index) {
  return sqlite3_bind_parameter_name(cpkt_sqlite_native_statement(self), parameter_index);
}

static int cpkt_sqlite_statement_parameter_index(const cpkt_sqlite_statement *self,
    const char *name) {
  return sqlite3_bind_parameter_index(cpkt_sqlite_native_statement(self), name);
}

static int cpkt_sqlite_statement_column_count(const cpkt_sqlite_statement *self) {
  return sqlite3_column_count(cpkt_sqlite_native_statement(self));
}

static const char *cpkt_sqlite_statement_column_name(
    const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_name(cpkt_sqlite_native_statement(self), column);
}

static const void *cpkt_sqlite_statement_column_name16(
    const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_name16(cpkt_sqlite_native_statement(self), column);
}

static int cpkt_sqlite_statement_column_type(const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_type(cpkt_sqlite_native_statement(self), column);
}

static int cpkt_sqlite_statement_column_int(const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_int(cpkt_sqlite_native_statement(self), column);
}

static cpkt_sqlite_i64 cpkt_sqlite_statement_column_i64(
    const cpkt_sqlite_statement *self, int column) {
  return cpkt_sqlite_public_i64(sqlite3_column_int64(
      cpkt_sqlite_native_statement(self), column));
}

static double cpkt_sqlite_statement_column_double(
    const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_double(cpkt_sqlite_native_statement(self), column);
}

static const unsigned char *cpkt_sqlite_statement_column_text(
    const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_text(cpkt_sqlite_native_statement(self), column);
}

static const void *cpkt_sqlite_statement_column_text16(
    const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_text16(cpkt_sqlite_native_statement(self), column);
}

static const void *cpkt_sqlite_statement_column_blob(
    const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_blob(cpkt_sqlite_native_statement(self), column);
}

static int cpkt_sqlite_statement_column_bytes(const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_bytes(cpkt_sqlite_native_statement(self), column);
}

static int cpkt_sqlite_statement_column_bytes16(
    const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_bytes16(cpkt_sqlite_native_statement(self), column);
}

static cpkt_sqlite_value *cpkt_sqlite_statement_column_value(
    const cpkt_sqlite_statement *self, int column) {
  cpkt_sqlite_value *value;
  sqlite3_value *native_value;
  native_value = sqlite3_column_value(cpkt_sqlite_native_statement(self), column);
  if (native_value == NULL) return NULL;
...
static const char *cpkt_sqlite_statement_column_database_name(
    const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_database_name(cpkt_sqlite_native_statement(self), column);
}

static const void *cpkt_sqlite_statement_column_database_name16(
    const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_database_name16(cpkt_sqlite_native_statement(self), column);
}

static const char *cpkt_sqlite_statement_column_table_name(
    const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_table_name(cpkt_sqlite_native_statement(self), column);
}

static const void *cpkt_sqlite_statement_column_table_name16(
    const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_table_name16(cpkt_sqlite_native_statement(self), column);
}

static const char *cpkt_sqlite_statement_column_origin_name(
    const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_origin_name(cpkt_sqlite_native_statement(self), column);
}

static const void *cpkt_sqlite_statement_column_origin_name16(
    const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_origin_name16(cpkt_sqlite_native_statement(self), column);
}

static const char *cpkt_sqlite_statement_column_declared_type(
    const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_decltype(cpkt_sqlite_native_statement(self), column);
}

static const void *cpkt_sqlite_statement_column_declared_type16(
    const cpkt_sqlite_statement *self, int column) {
  return sqlite3_column_decltype16(cpkt_sqlite_native_statement(self), column);
}

static const char *cpkt_sqlite_statement_sql(const cpkt_sqlite_statement *self) {
  return sqlite3_sql(cpkt_sqlite_native_statement(self));
}

static char *cpkt_sqlite_statement_expanded_sql(const cpkt_sqlite_statement *self) {
  return sqlite3_expanded_sql(cpkt_sqlite_native_statement(self));
}

static const char *cpkt_sqlite_statement_normalized_sql(
    const cpkt_sqlite_statement *self) {
...
static int cpkt_sqlite_statement_explain(cpkt_sqlite_statement *self, int mode) {
...
static int cpkt_sqlite_statement_status(const cpkt_sqlite_statement *self,
  ...
  if (self == NULL || cpkt_sqlite_native_statement(self) == NULL) return 0;
  return sqlite3_stmt_status(cpkt_sqlite_native_statement(self), operation, reset);
}
...
static int cpkt_sqlite_statement_scan_status_i64(
  ...
  sqlite3_int64 value;
  int status;
  if (self == NULL || value_out == NULL || flags > 0xffffffffUL) return SQLITE_MISUSE;
  value = 0;
...
static int cpkt_sqlite_statement_scan_status_int(const cpkt_sqlite_statement *self,
    int index, int operation, unsigned long flags, int *value_out) {
  if (self == NULL || value_out == NULL || flags > 0xffffffffUL) return SQLITE_MISUSE;
  return sqlite3_stmt_scanstatus_v2(cpkt_sqlite_native_statement(self), index,
      operation, (int) flags, value_out);
}
...
static void cpkt_sqlite_statement_scan_status_reset(cpkt_sqlite_statement *self) {
  if (self != NULL) sqlite3_stmt_scanstatus_reset(cpkt_sqlite_native_statement(self));
}

static cpkt_sqlite *cpkt_sqlite_statement_database(
    const cpkt_sqlite_statement *self) {
...
int cpkt_sqlite_statement_finalize(cpkt_sqlite_statement *self) {
  cpkt_sqlite *database;
  int status;
  if (self == NULL) return CPKT_SQLITE_MISUSE;
  database = self->owner;
  status = self->borrowed ? CPKT_SQLITE_OK :
...
static const char *cpkt_sqlite_table_column_name(const cpkt_sqlite_table *self,
    int column) {
  if (self == NULL || self->values == NULL || column < 0 || column >= self->columns) {
    return NULL;
  }
...
static const char *cpkt_sqlite_table_value(const cpkt_sqlite_table *self,
  ...
  if (self == NULL || self->values == NULL || row < 0 || row >= self->rows ||
      column < 0 || column >= self->columns) return NULL;
...
static void cpkt_sqlite_table_close(cpkt_sqlite_table *self) {
  if (self == NULL) return;
  if (self->values != NULL) sqlite3_free_table(self->values);
  self->values = NULL;
  free(self);
}
...
int cpkt_sqlite_get_table(cpkt_sqlite *self, const char *sql,
  ...
  int columns;
  int status;
  if (table_out != NULL) *table_out = NULL;
  if (error_out != NULL) *error_out = NULL;
  if (self == NULL || cpkt_sqlite_native(self) == NULL || sql == NULL ||
      table_out == NULL) return SQLITE_MISUSE;
  values = NULL;
  rows = 0;
  columns = 0;
  status = sqlite3_get_table(cpkt_sqlite_native(self), sql, &values, &rows,
      &columns, error_out);
  if (status != SQLITE_OK) {
    if (values != NULL) sqlite3_free_table(values);
    return status;
  }
...
int cpkt_sqlite_statement_transfer_bindings(cpkt_sqlite_statement *source,
  ...
  if (target == NULL || source == NULL || cpkt_sqlite_native_statement(target) == NULL ||
      cpkt_sqlite_native_statement(source) == NULL) return SQLITE_MISUSE;
...
static int cpkt_sqlite_wrap_statement(cpkt_sqlite *database,
    sqlite3_stmt *native_statement, int borrowed,
    cpkt_sqlite_statement **statement_out) {
  cpkt_sqlite_statement *public_statement;
  if (database == NULL || statement_out == NULL) return SQLITE_MISUSE;
  *statement_out = NULL;
  if (native_statement == NULL) return SQLITE_OK;
  public_statement = (cpkt_sqlite_statement *) calloc(1, sizeof(*public_statement));
  if (public_statement == NULL) {
    if (!borrowed) sqlite3_finalize(native_statement);
  ...
  public_statement->bind_zero_blob_u64 = cpkt_sqlite_statement_bind_zero_blob_u64;
  ...
  public_statement->column_database_name = cpkt_sqlite_statement_column_database_name;
  public_statement->column_database_name16 = cpkt_sqlite_statement_column_database_name16;
  public_statement->column_table_name = cpkt_sqlite_statement_column_table_name;
  public_statement->column_table_name16 = cpkt_sqlite_statement_column_table_name16;
  public_statement->column_origin_name = cpkt_sqlite_statement_column_origin_name;
  public_statement->column_origin_name16 = cpkt_sqlite_statement_column_origin_name16;
  public_statement->column_declared_type = cpkt_sqlite_statement_column_declared_type;
  public_statement->column_declared_type16 = cpkt_sqlite_statement_column_declared_type16;
  ...
  public_statement->scan_status_double = cpkt_sqlite_statement_scan_status_double;
...
int cpkt_sqlite_prepare(cpkt_sqlite *self, const char *sql, int byte_count,
  ...
  int status;
  if (statement_out != NULL) *statement_out = NULL;
  if (tail_out != NULL) *tail_out = NULL;
  if (self == NULL || cpkt_sqlite_native(self) == NULL || sql == NULL ||
      statement_out == NULL) return CPKT_SQLITE_MISUSE;
  native_statement = NULL;
  native_tail = NULL;
  status = sqlite3_prepare_v3(cpkt_sqlite_native(self), sql, byte_count,
      (unsigned int) flags, &native_statement, &native_tail);
  if (tail_out != NULL) *tail_out = native_tail;
  if (status != SQLITE_OK) return status;
...
int cpkt_sqlite_busy_timeout(cpkt_sqlite *self, int milliseconds) {
  if (self == NULL || cpkt_sqlite_native(self) == NULL) return CPKT_SQLITE_MISUSE;
...
int cpkt_sqlite_wal_checkpoint(cpkt_sqlite *self, const char *schema, int mode,
  ...
  if (log_frames_out != NULL) *log_frames_out = 0;
  if (checkpointed_frames_out != NULL) *checkpointed_frames_out = 0;
  if (self == NULL || cpkt_sqlite_native(self) == NULL) return CPKT_SQLITE_MISUSE;
...
int cpkt_sqlite_load_extension(cpkt_sqlite *self, const char *path,
  ...
  return sqlite3_load_extension(cpkt_sqlite_native(self), path, entry_point, error_out);
}
...
int cpkt_sqlite_unlock_notify(cpkt_sqlite *self,
  ...
  int status;
  if (self == NULL || cpkt_sqlite_native(self) == NULL) return SQLITE_MISUSE;
  ...
  if (binding == NULL) return SQLITE_NOMEM;
  binding->state = state;
...
int cpkt_sqlite_overload_function(cpkt_sqlite *self, const char *name,
  ...
  return sqlite3_overload_function(cpkt_sqlite_native(self), name, argument_count);
}
...
int cpkt_sqlite_drop_modules(cpkt_sqlite *self, int kept_name_count,
  ...
  if (self == NULL || cpkt_sqlite_native(self) == NULL || kept_name_count < 0 ||
      (kept_name_count > 0 && kept_names == NULL)) return SQLITE_MISUSE;
  names = NULL;
  if (kept_name_count > 0) {
    names = (const char **) calloc((size_t) kept_name_count + 1, sizeof(*names));
    if (names == NULL) return SQLITE_NOMEM;
...
static int cpkt_sqlite_virtual_table_set_error(cpkt_sqlite_virtual_table *self,
  ...
  if (self == NULL || self->internal == NULL || message == NULL) return SQLITE_MISUSE;
...
static int cpkt_sqlite_module_connect(sqlite3 *native_database, void *auxiliary,
    int argument_count, const char *const *arguments, sqlite3_vtab **out,
    char **error_out, int is_create) {
  cpkt_sqlite_module_binding *module;
  cpkt_sqlite_native_virtual_table *native_table;
  cpkt_sqlite_virtual_table *public_table;
  ...
  *out = NULL;
  ...
  native_table = (cpkt_sqlite_native_virtual_table *) calloc(1, sizeof(*native_table));
...
static int cpkt_sqlite_module_create_trampoline(sqlite3 *database, void *auxiliary,
    int argument_count, const char *const *arguments, sqlite3_vtab **out,
    char **error_out) {
  return cpkt_sqlite_module_connect(database, auxiliary, argument_count, arguments,
      out, error_out, 1);
}

static int cpkt_sqlite_module_connect_trampoline(sqlite3 *database, void *auxiliary,
    int argument_count, const char *const *arguments, sqlite3_vtab **out,
    char **error_out) {
  return cpkt_sqlite_module_connect(database, auxiliary, argument_count, arguments,
      out, error_out, 0);
...
static int cpkt_sqlite_module_best_index_trampoline(sqlite3_vtab *table,
  ...
  int index;
  int status;
  if (table == NULL || native_info == NULL) return SQLITE_MISUSE;
  native_table = (cpkt_sqlite_native_virtual_table *) table;
  module = native_table->module;
  if (module == NULL || module->methods.best_index == NULL) return SQLITE_MISUSE;
  constraints = NULL;
  orders = NULL;
  usages = NULL;
  ...
  if (status == SQLITE_OK) {
    for (index = 0; index < native_info->nConstraint; ++index) {
      native_info->aConstraintUsage[index].argvIndex = usages[index].argument_index;
      native_info->aConstraintUsage[index].omit = (unsigned char) usages[index].omit;
    }
    ...
    if (info.index_string != NULL) {
      ...
      if (native_info->idxStr == NULL) status = SQLITE_NOMEM;
      else native_info->needToFreeIdxStr = 1;
    }
  }
  free(orders);
  free(usages);
...
static int cpkt_sqlite_module_table_trampoline(sqlite3_vtab *table, int destroy) {
  cpkt_sqlite_native_virtual_table *native_table;
  cpkt_sqlite_module_binding *module;
  ...
  int status;
  if (table == NULL) return SQLITE_MISUSE;
  native_table = (cpkt_sqlite_native_virtual_table *) table;
  module = native_table->module;
  callback = destroy ? module->methods.destroy_table : module->methods.disconnect;
  status = callback == NULL ? SQLITE_OK : callback(module->methods.context,
  ...
  if (native_table->public_table != NULL) native_table->public_table->internal = NULL;
...
static int cpkt_sqlite_module_find_function_trampoline(sqlite3_vtab *table,
  ...
  void *user_data;
  int found;
  if (table == NULL || function_out == NULL || user_data_out == NULL || name == NULL) {
    return 0;
  }
  native_table = (cpkt_sqlite_native_virtual_table *) table;
  module = native_table->module;
  if (module == NULL || module->methods.find_function == NULL) return 0;
  function = NULL;
  user_data = NULL;
  found = module->methods.find_function(module->methods.context,
      native_table->public_table, argument_count, name, &function, &user_data);
  if (found == 0 || function == NULL) return 0;
...
static int cpkt_sqlite_module_open_trampoline(sqlite3_vtab *table,
  ...
  cpkt_sqlite_module_binding *module;
  int status;
  if (table == NULL || out == NULL) return SQLITE_MISUSE;
  *out = NULL;
  native_table = (cpkt_sqlite_native_virtual_table *) table;
  module = native_table->module;
  if (module == NULL || module->methods.open == NULL) return SQLITE_MISUSE;
  public_cursor = NULL;
  status = module->methods.open(module->methods.context, native_table->public_table,
      &public_cursor);
  if (status != SQLITE_OK) {
    free(public_cursor);
    return status;
  }
  if (public_cursor == NULL || public_cursor->table != native_table->public_table ||
      public_cursor->internal != NULL) {
    free(public_cursor);
    return SQLITE_MISUSE;
  }
  native_cursor = (cpkt_sqlite_native_virtual_cursor *) calloc(1,
      sizeof(*native_cursor));
...
static int cpkt_sqlite_module_close_trampoline(sqlite3_vtab_cursor *cursor) {
  ...
  status = module->methods.close == NULL ? SQLITE_OK : module->methods.close(
      module->methods.context, native_cursor->public_cursor);
  if (native_cursor->public_cursor != NULL) native_cursor->public_cursor->internal = NULL;
  free(native_cursor->public_cursor);
  free(native_cursor);
  return status;
}

static int cpkt_sqlite_module_filter_trampoline(sqlite3_vtab_cursor *cursor,
    int index_number, const char *index_string, int argument_count,
  ...
  native_cursor = (cpkt_sqlite_native_virtual_cursor *) cursor;
  module = native_cursor->table->module;
  if (module->methods.filter == NULL) return SQLITE_MISUSE;
  values = NULL;
  arguments = NULL;
  if (argument_count > 0) {
    values = (cpkt_sqlite_value *) calloc((size_t) argument_count, sizeof(*values));
  ...
  status = module->methods.filter(module->methods.context, native_cursor->public_cursor,
      index_number, index_string, argument_count, arguments);
  free(arguments);
  free(values);
  return status;
}
...
static int cpkt_sqlite_module_next_trampoline(sqlite3_vtab_cursor *cursor) {
  ...
  native_cursor = (cpkt_sqlite_native_virtual_cursor *) cursor;
  module = native_cursor->table->module;
  return module->methods.next == NULL ? SQLITE_MISUSE : module->methods.next(
      module->methods.context, native_cursor->public_cursor);
}
...
static int cpkt_sqlite_module_eof_trampoline(sqlite3_vtab_cursor *cursor) {
  ...
  native_cursor = (cpkt_sqlite_native_virtual_cursor *) cursor;
  module = native_cursor->table->module;
  return module->methods.eof == NULL ? 1 : module->methods.eof(
      module->methods.context, native_cursor->public_cursor) != 0;
}

static int cpkt_sqlite_module_column_trampoline(sqlite3_vtab_cursor *cursor,
    sqlite3_context *native_context, int column) {
  cpkt_sqlite_native_virtual_cursor *native_cursor;
  cpkt_sqlite_module_binding *module;
  cpkt_sqlite_context public_context;
  if (cursor == NULL || native_context == NULL) return SQLITE_MISUSE;
  native_cursor = (cpkt_sqlite_native_virtual_cursor *) cursor;
  module = native_cursor->table->module;
  if (module->methods.column == NULL) return SQLITE_MISUSE;
  public_context.context = native_context;
  public_context.database = module->database;
  return module->methods.column(module->methods.context, native_cursor->public_cursor,
      &public_context, column);
}
...
static int cpkt_sqlite_module_rowid_trampoline(sqlite3_vtab_cursor *cursor,
  ...
  int status;
  if (cursor == NULL || rowid_out == NULL) return SQLITE_MISUSE;
  native_cursor = (cpkt_sqlite_native_virtual_cursor *) cursor;
  module = native_cursor->table->module;
  if (module->methods.rowid == NULL) return SQLITE_MISUSE;
  rowid.high = 0;
  rowid.low = 0;
  status = module->methods.rowid(module->methods.context, native_cursor->public_cursor,
      &rowid);
  if (status == SQLITE_OK) *rowid_out = cpkt_sqlite_native_i64(rowid);
  return status;
}

static int cpkt_sqlite_module_update_trampoline(sqlite3_vtab *table,
    int argument_count, sqlite3_value **native_arguments, sqlite3_int64 *rowid_out) {
  cpkt_sqlite_native_virtual_table *native_table;
  cpkt_sqlite_module_binding *module;
  cpkt_sqlite_value *values;
  cpkt_sqlite_value **arguments;
  ...
  int index;
  int status;
  if (table == NULL || rowid_out == NULL) return SQLITE_MISUSE;
  native_table = (cpkt_sqlite_native_virtual_table *) table;
  module = native_table->module;
  if (module->methods.update == NULL) return SQLITE_MISUSE;
  values = NULL;
  arguments = NULL;
  if (argument_count > 0) {
    values = (cpkt_sqlite_value *) calloc((size_t) argument_count, sizeof(*values));
  ...
  status = module->methods.update(module->methods.context, native_table->public_table,
      argument_count, arguments, &rowid);
  if (status == SQLITE_OK) *rowid_out = cpkt_sqlite_native_i64(rowid);
  free(arguments);
  free(values);
  return status;
...
static int cpkt_sqlite_module_table_operation(sqlite3_vtab *table, int operation,
    int argument) {
  cpkt_sqlite_native_virtual_table *native_table;
  cpkt_sqlite_module_binding *module;
  ...
  if (operation == 0) table_callback = module->methods.begin;
  else if (operation == 1) table_callback = module->methods.sync;
  else if (operation == 2) table_callback = module->methods.commit;
  else if (operation == 3) table_callback = module->methods.rollback;
  else if (operation == 4) savepoint_callback = module->methods.savepoint;
  else if (operation == 5) savepoint_callback = module->methods.release;
  else if (operation == 6) savepoint_callback = module->methods.rollback_to;
  if (savepoint_callback != NULL) return savepoint_callback(module->methods.context,
      native_table->public_table, argument);
  return table_callback == NULL ? SQLITE_OK : table_callback(module->methods.context,
      native_table->public_table);
}
...
static int cpkt_sqlite_module_savepoint_trampoline(sqlite3_vtab *table, int value) {
  return cpkt_sqlite_module_table_operation(table, 4, value);
}
static int cpkt_sqlite_module_release_trampoline(sqlite3_vtab *table, int value) {
  return cpkt_sqlite_module_table_operation(table, 5, value);
}
static int cpkt_sqlite_module_rollback_to_trampoline(sqlite3_vtab *table, int value) {
...
static int cpkt_sqlite_module_rename_trampoline(sqlite3_vtab *table,
  ...
  cpkt_sqlite_native_virtual_table *native_table;
  cpkt_sqlite_module_binding *module;
  if (table == NULL || name == NULL) return SQLITE_MISUSE;
  native_table = (cpkt_sqlite_native_virtual_table *) table;
  module = native_table->module;
  return module->methods.rename == NULL ? SQLITE_OK : module->methods.rename(
      module->methods.context, native_table->public_table, name);
}

static int cpkt_sqlite_module_integrity_trampoline(sqlite3_vtab *table,
    const char *schema, const char *name, int flags, char **error_out) {
  cpkt_sqlite_native_virtual_table *native_table;
  cpkt_sqlite_module_binding *module;
  if (table == NULL) return SQLITE_MISUSE;
  native_table = (cpkt_sqlite_native_virtual_table *) table;
  module = native_table->module;
  return module->methods.integrity == NULL ? SQLITE_OK : module->methods.integrity(
      module->methods.context, native_table->public_table, schema, name, flags,
      error_out);
}
...
static void cpkt_sqlite_module_destroy_binding(void *context) {
  ...
  if (module->methods.destroy != NULL) module->methods.destroy(module->methods.context);
  free(module);
}

int cpkt_sqlite_declare_virtual_table(cpkt_sqlite *database, const char *schema) {
  if (database == NULL || cpkt_sqlite_native(database) == NULL || schema == NULL) {
    return SQLITE_MISUSE;
  }
...
int cpkt_sqlite_create_module(cpkt_sqlite *database, const char *name,
  ...
  cpkt_sqlite_module_binding *module;
  int status;
  if (database == NULL || cpkt_sqlite_native(database) == NULL || name == NULL ||
      methods == NULL || methods->best_index == NULL || methods->open == NULL ||
      methods->close == NULL || methods->filter == NULL || methods->next == NULL ||
      methods->eof == NULL || methods->column == NULL || methods->rowid == NULL) {
    return SQLITE_MISUSE;
  }
  if (methods->create == NULL && methods->connect == NULL) return SQLITE_MISUSE;
  module = (cpkt_sqlite_module_binding *) calloc(1, sizeof(*module));
  if (module == NULL) return SQLITE_NOMEM;
  module->database = database;
  module->methods = *methods;
  module->module.iVersion = 4;
  module->module.xCreate = methods->create == NULL ? NULL :
      cpkt_sqlite_module_create_trampoline;
  module->module.xConnect = methods->connect == NULL ? NULL :
  ...
  module->module.xUpdate = methods->update == NULL ? NULL :
      cpkt_sqlite_module_update_trampoline;
  ...
  module->module.xFindFunction = methods->find_function == NULL ? NULL :
  ...
  status = sqlite3_create_module_v2(cpkt_sqlite_native(database), name,
      &module->module, module, cpkt_sqlite_module_destroy_binding);
  if (status != SQLITE_OK) return status;
  return SQLITE_OK;
}

int cpkt_sqlite_virtual_table_config_none(cpkt_sqlite *database, int operation) {
  if (database == NULL || cpkt_sqlite_native(database) == NULL) return SQLITE_MISUSE;
...
int cpkt_sqlite_context_no_change(const cpkt_sqlite_context *context) {
  if (context == NULL || cpkt_sqlite_native_context(context) == NULL) return 0;
...
int cpkt_sqlite_index_info_rhs_value(const cpkt_sqlite_index_info *self,
  ...
  sqlite3_value *value;
  int status;
  if (value_out != NULL) memset(value_out, 0, sizeof(*value_out));
  if (self == NULL || self->internal == NULL || value_out == NULL) return SQLITE_MISUSE;
  value = NULL;
  status = sqlite3_vtab_rhs_value((sqlite3_index_info *) self->internal,
      constraint_index, &value);
  if (status == SQLITE_OK) value_out->value = value;
  return status;
}

const char *cpkt_sqlite_index_info_collation(const cpkt_sqlite_index_info *self,
    int constraint_index) {
  if (self == NULL || self->internal == NULL) return NULL;
...
int cpkt_sqlite_index_info_distinct(const cpkt_sqlite_index_info *self) {
  if (self == NULL || self->internal == NULL) return 0;
...
int cpkt_sqlite_index_info_set_in(cpkt_sqlite_index_info *self,
  ...
  if (self == NULL || self->internal == NULL) return SQLITE_MISUSE;
  return sqlite3_vtab_in((sqlite3_index_info *) self->internal,
      constraint_index, enabled);
}
...
int cpkt_sqlite_virtual_table_in_first(const cpkt_sqlite_value *input,
  ...
  int status;
  if (value_out != NULL) memset(value_out, 0, sizeof(*value_out));
  if (input == NULL || cpkt_sqlite_native_value(input) == NULL || value_out == NULL) {
    return SQLITE_MISUSE;
  }
  value = NULL;
...
int cpkt_sqlite_database_config_int(cpkt_sqlite *self, int operation, int value,
  ...
  return sqlite3_db_config(cpkt_sqlite_native(self), operation, value, result_out);
}
...
int cpkt_sqlite_database_config_main_name(cpkt_sqlite *self, const char *name) {
  ...
  return sqlite3_db_config(cpkt_sqlite_native(self), SQLITE_DBCONFIG_MAINDBNAME, name);
}

int cpkt_sqlite_changes(const cpkt_sqlite *self) {
  if (self == NULL || cpkt_sqlite_native(self) == NULL) return 0;
...
cpkt_sqlite_i64 cpkt_sqlite_last_insert_rowid(const cpkt_sqlite *self) {
  ...
  result.high = 0;
  result.low = 0;
  if (self == NULL || cpkt_sqlite_native(self) == NULL) return result;
  return cpkt_sqlite_public_i64(sqlite3_last_insert_rowid(cpkt_sqlite_native(self)));
}

const char *cpkt_sqlite_error(const cpkt_sqlite *self) {
  if (self == NULL || cpkt_sqlite_native(self) == NULL) return "invalid SQLite receiver";
  return sqlite3_errmsg(cpkt_sqlite_native(self));
}

const void *cpkt_sqlite_error16(const cpkt_sqlite *self) {
  if (self == NULL || cpkt_sqlite_native(self) == NULL) return NULL;
...
static void cpkt_sqlite_destroy(cpkt_sqlite *self) {
  ...
  if (state != NULL) {
    if (state->listed_wrapper) {
      ...
      if (*state_link == state) *state_link = state->next_wrapper;
...
void cpkt_sqlite_close(cpkt_sqlite *self) {
  ...
  state->closing = 1;
  if (state->child_count == 0) cpkt_sqlite_finish_close(self);
}
...
int cpkt_sqlite_close_strict(cpkt_sqlite *self) {
  ...
  if (self == NULL || cpkt_sqlite_native(self) == NULL) return CPKT_SQLITE_MISUSE;
  state = cpkt_sqlite_state_for(self);
  if (state == NULL || state->closing) return CPKT_SQLITE_MISUSE;
  if (state->child_count != 0) return CPKT_SQLITE_BUSY;
...
static void cpkt_sqlite_finish_close(cpkt_sqlite *self) {
  ...
  if (self == NULL) return;
  state = cpkt_sqlite_state_for(self);
  if (state == NULL || !state->closing || state->child_count != 0) return;
...
static cpkt_sqlite *cpkt_sqlite_wrap_database(sqlite3 *database,
  ...
  if (self == NULL) {
    if (close_on_failure) sqlite3_close_v2(database);
    return NULL;
  }
...
cpkt_sqlite_auto_extension *cpkt_sqlite_auto_extension_new(
  ...
  public_extension = (cpkt_sqlite_auto_extension *) calloc(1,
      sizeof(*public_extension));
...
cpkt_sqlite_vfs *cpkt_sqlite_vfs_new(const char *name,
  ...
  if (name == NULL || name[0] == '\0' || maximum_pathname_bytes <= 0 ||
      ...
      methods->current_time == NULL) return NULL;
  ...
  binding->native.xCurrentTimeInt64 = methods->version >= 2 &&
      methods->current_time_i64 != NULL ? cpkt_sqlite_vfs_current_time_i64_native : NULL;
  binding->native.xSetSystemCall = methods->version >= 3 &&
      methods->set_system_call != NULL ? cpkt_sqlite_vfs_set_system_call_native : NULL;
  binding->native.xGetSystemCall = methods->version >= 3 &&
      methods->get_system_call != NULL ? cpkt_sqlite_vfs_get_system_call_native : NULL;
  binding->native.xNextSystemCall = methods->version >= 3 &&
      methods->next_system_call != NULL ? cpkt_sqlite_vfs_next_system_call_native : NULL;
  {
    sqlite3_mutex *mutex;
    mutex = cpkt_sqlite_global_mutex();
    cpkt_sqlite_global_lock(mutex);
...
cpkt_sqlite_vfs *cpkt_sqlite_vfs_find(const char *name) {
  ...
  if (native_vfs == NULL) return NULL;
  mutex = cpkt_sqlite_global_mutex();
  cpkt_sqlite_global_lock(mutex);
...
int cpkt_sqlite_vfs_register(cpkt_sqlite_vfs *self, int make_default) {
  ...
  if (status == SQLITE_OK) binding->registered = 1;
  return status;
}
...
int cpkt_sqlite_vfs_unregister(cpkt_sqlite_vfs *self) {
  ...
  if (status == SQLITE_OK) binding->registered = 0;
  return status;
}
...
cpkt_sqlite *cpkt_sqlite_new(const char *filename) {
  return cpkt_sqlite_open(filename,
      CPKT_SQLITE_OPEN_READWRITE | CPKT_SQLITE_OPEN_CREATE, NULL);
}
...
int cpkt_sqlite_i64_compare(cpkt_sqlite_i64 left, cpkt_sqlite_i64 right) {
  ...
  if (left_high < right_high) return -1;
  if (left_high > right_high) return 1;
...
int cpkt_sqlite_library_version_number(void) { return sqlite3_libversion_number(); }
int cpkt_sqlite_compile_option_used(const char *name) { return sqlite3_compileoption_used(name); }
const char *cpkt_sqlite_compile_option(int index) { return sqlite3_compileoption_get(index); }
...
int cpkt_sqlite_global_config_memory_methods_set(
  ...
  if (methods == NULL || methods->allocate == NULL || methods->free == NULL ||
      methods->reallocate == NULL || methods->size == NULL || methods->roundup == NULL ||
      methods->initialize == NULL || methods->shutdown == NULL) return SQLITE_MISUSE;
...
int cpkt_sqlite_global_config_memory_methods_get(
  ...
  sqlite3_mem_methods native_methods;
  int status;
  if (methods_out == NULL) return SQLITE_MISUSE;
...
static int cpkt_sqlite_page_cache_initialize(void *context) {
  ...
  if (methods == NULL || methods->initialize == NULL) return SQLITE_OK;
...
static void cpkt_sqlite_page_cache_shutdown(void *context) {
  cpkt_sqlite_page_cache_methods *methods;
  methods = (cpkt_sqlite_page_cache_methods *) context;
  if (methods != NULL && methods->shutdown != NULL) methods->shutdown(methods->context);
}
...
static sqlite3_pcache *cpkt_sqlite_page_cache_create(int page_byte_count,
  ...
  if (!cpkt_sqlite_page_cache_methods_are_set ||
      cpkt_sqlite_page_cache_methods_current.create == NULL) return NULL;
...
static cpkt_sqlite_page_cache_binding *cpkt_sqlite_page_cache_native_binding(
    sqlite3_pcache *cache) {
  return (cpkt_sqlite_page_cache_binding *) cache;
}

static cpkt_sqlite_page_binding *cpkt_sqlite_page_cache_find_page(
    cpkt_sqlite_page_cache_binding *cache, cpkt_sqlite_page *page) {
...
static int cpkt_sqlite_page_cache_page_count(sqlite3_pcache *cache) {
  cpkt_sqlite_page_cache_binding *binding;
  binding = cpkt_sqlite_page_cache_native_binding(cache);
  if (binding == NULL || binding->methods->page_count == NULL) return 0;
...
static sqlite3_pcache_page *cpkt_sqlite_page_cache_fetch(sqlite3_pcache *cache,
    unsigned int key, int create_flag) {
  cpkt_sqlite_page_cache_binding *binding;
  cpkt_sqlite_page_binding *page_binding;
  cpkt_sqlite_page *page;
  binding = cpkt_sqlite_page_cache_native_binding(cache);
  if (binding == NULL || binding->methods->fetch == NULL) return NULL;
  page = binding->methods->fetch(binding->cache, (unsigned long) key, create_flag);
  ...
  if (page_binding == NULL) {
    if (binding->methods->unpin != NULL) binding->methods->unpin(binding->cache, page, 1);
    return NULL;
  }
  page_binding->native_page.pBuf = page->buffer;
  page_binding->native_page.pExtra = page->extra;
...
static void cpkt_sqlite_page_cache_destroy(sqlite3_pcache *cache) {
  ...
  binding = cpkt_sqlite_page_cache_native_binding(cache);
  if (binding == NULL) return;
  if (binding->methods->destroy != NULL) binding->methods->destroy(binding->cache);
...
int cpkt_sqlite_global_config_page_cache_methods_set(
  ...
  int status;
  if (methods == NULL || methods->create == NULL || methods->cache_size == NULL ||
      methods->page_count == NULL || methods->fetch == NULL ||
      methods->unpin == NULL || methods->rekey == NULL || methods->truncate == NULL ||
      methods->destroy == NULL) return SQLITE_MISUSE;
...
int cpkt_sqlite_global_config_page_cache_methods_get(
  ...
  if (!cpkt_sqlite_page_cache_methods_are_set ||
      native_methods.pArg != &cpkt_sqlite_page_cache_methods_current ||
      native_methods.xCreate != cpkt_sqlite_page_cache_create) return SQLITE_NOTFOUND;
...
static void cpkt_sqlite_mutex_enter(cpkt_sqlite_mutex *self) {
  ...
  if (native_mutex != NULL) sqlite3_mutex_enter(native_mutex);
}

static int cpkt_sqlite_mutex_try_enter(cpkt_sqlite_mutex *self) {
  sqlite3_mutex *native_mutex;
  native_mutex = cpkt_sqlite_native_mutex(self);
  if (native_mutex == NULL) return SQLITE_MISUSE;
...
static void cpkt_sqlite_mutex_leave(cpkt_sqlite_mutex *self) {
  sqlite3_mutex *native_mutex;
  native_mutex = cpkt_sqlite_native_mutex(self);
  if (native_mutex != NULL) sqlite3_mutex_leave(native_mutex);
}
...
cpkt_sqlite_mutex *cpkt_sqlite_database_mutex(const cpkt_sqlite *database) {
  sqlite3_mutex *native_mutex;
  cpkt_sqlite_mutex *public_mutex;
  if (database == NULL || cpkt_sqlite_native(database) == NULL) return NULL;
  native_mutex = sqlite3_db_mutex(cpkt_sqlite_native(database));
  if (native_mutex == NULL) return NULL;
  public_mutex = (cpkt_sqlite_mutex *) calloc(1, sizeof(*public_mutex));
  if (public_mutex == NULL) return NULL;
  public_mutex->enter = cpkt_sqlite_mutex_enter;
  public_mutex->try_enter = cpkt_sqlite_mutex_try_enter;
  public_mutex->leave = cpkt_sqlite_mutex_leave;
  public_mutex->held = cpkt_sqlite_mutex_held;
...
int cpkt_sqlite_complete_utf16(const void *sql) { return sqlite3_complete16(sql); }
...
char *cpkt_sqlite_format_into_v(int byte_count, char *buffer, const char *format,
    va_list arguments) {
  if (byte_count <= 0 || buffer == NULL || format == NULL) return buffer;
...
void *cpkt_sqlite_allocate(int byte_count) { return sqlite3_malloc(byte_count); }
...
int cpkt_sqlite_memory_alarm(cpkt_sqlite_memory_alarm_callback callback,
  ...
  status = sqlite3_memory_alarm(callback == NULL ? NULL :
      cpkt_sqlite_memory_alarm_trampoline,
...
int cpkt_sqlite_release_memory(int byte_count) { return sqlite3_release_memory(byte_count); }
cpkt_sqlite_i64 cpkt_sqlite_soft_heap_limit(cpkt_sqlite_i64 byte_count) {
  return cpkt_sqlite_public_i64(sqlite3_soft_heap_limit64(
      cpkt_sqlite_native_i64(byte_count)));
}
cpkt_sqlite_i64 cpkt_sqlite_hard_heap_limit(cpkt_sqlite_i64 byte_count) {
  return cpkt_sqlite_public_i64(sqlite3_hard_heap_limit64(
      cpkt_sqlite_native_i64(byte_count)));
}
...
static const char *cpkt_sqlite_filename_database(const cpkt_sqlite_filename *self) {
  return sqlite3_filename_database(cpkt_sqlite_native_filename(self));
}

static const char *cpkt_sqlite_filename_journal(const cpkt_sqlite_filename *self) {
...
static const char *cpkt_sqlite_filename_uri_parameter(const cpkt_sqlite_filename *self,
    const char *name) {
  if (name == NULL) return NULL;
...
static int cpkt_sqlite_filename_uri_boolean(const cpkt_sqlite_filename *self,
  ...
  return sqlite3_uri_boolean(cpkt_sqlite_native_filename(self), name, default_value);
}

static cpkt_sqlite_i64 cpkt_sqlite_filename_uri_i64(const cpkt_sqlite_filename *self,
    const char *name, cpkt_sqlite_i64 default_value) {
  if (name == NULL) return default_value;
  return cpkt_sqlite_public_i64(sqlite3_uri_int64(cpkt_sqlite_native_filename(self),
      name, cpkt_sqlite_native_i64(default_value)));
}

static const char *cpkt_sqlite_filename_uri_key(const cpkt_sqlite_filename *self,
    int index) {
...
cpkt_sqlite_filename *cpkt_sqlite_filename_new(const char *database,
  ...
  native_filename = sqlite3_create_filename(database, journal, wal, parameter_count,
      (const char **) parameters);
  if (native_filename == NULL) return NULL;
...
static int cpkt_sqlite_fts5_token_trampoline(void *context, int flags,
  ...
  if (token_context == NULL || token_context->token == NULL) return SQLITE_MISUSE;
  return token_context->token(token_context->context, flags, token, token_byte_count,
      start_offset, end_offset);
}

static int cpkt_sqlite_fts5_tokenizer_create_trampoline(void *context,
    const char **arguments, int argument_count, Fts5Tokenizer **out) {
  ...
  if (out != NULL) *out = NULL;
  if (binding == NULL || binding->create == NULL || out == NULL) return SQLITE_MISUSE;
  tokenizer = NULL;
  status = binding->create(binding->context, (const char *const *) arguments,
      argument_count, &tokenizer);
  if (status != SQLITE_OK) return status;
  if (tokenizer == NULL || tokenizer->facade != NULL) return SQLITE_MISUSE;
  tokenizer->facade = binding;
  *out = (Fts5Tokenizer *) tokenizer;
  return SQLITE_OK;
}

static void cpkt_sqlite_fts5_tokenizer_delete_trampoline(Fts5Tokenizer *native_tokenizer) {
  cpkt_sqlite_fts5_tokenizer *tokenizer;
  cpkt_sqlite_fts5_tokenizer_binding *binding;
...
static int cpkt_sqlite_fts5_tokenizer_tokenize_trampoline(Fts5Tokenizer *native_tokenizer,
    void *context, int flags, const char *text, int text_byte_count,
  ...
  cpkt_sqlite_fts5_token_context token_context;
  tokenizer = (cpkt_sqlite_fts5_tokenizer *) native_tokenizer;
  if (tokenizer == NULL || token == NULL) return SQLITE_MISUSE;
  binding = (cpkt_sqlite_fts5_tokenizer_binding *) tokenizer->facade;
  if (binding == NULL || binding->tokenize == NULL) return SQLITE_MISUSE;
  token_context.context = context;
  token_context.token = token;
  return binding->tokenize(tokenizer, &token_context, flags, text, text_byte_count,
      locale, locale_byte_count, cpkt_sqlite_fts5_token_trampoline);
}
...
static void cpkt_sqlite_fts5_tokenizer_binding_destroy(void *context) {
  ...
  binding = (cpkt_sqlite_fts5_tokenizer_binding *) context;
  if (binding == NULL) return;
  if (binding->binding_destroy != NULL) binding->binding_destroy(binding->context);
  free(binding);
}

static void cpkt_sqlite_fts5_context_initialize(cpkt_sqlite_fts5_context *self,
    const Fts5ExtensionApi *api, Fts5Context *context, cpkt_sqlite *database);
...
static int cpkt_sqlite_fts5_context_column_count(const cpkt_sqlite_fts5_context *self) {
  const Fts5ExtensionApi *api;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->xColumnCount == NULL) return 0;
  return api->xColumnCount(cpkt_sqlite_native_fts5_context(self));
}

static int cpkt_sqlite_fts5_context_row_count(const cpkt_sqlite_fts5_context *self,
    cpkt_sqlite_i64 *out) {
  const Fts5ExtensionApi *api;
  sqlite3_int64 value;
  int status;
  if (out != NULL) *out = cpkt_sqlite_i64_make(0, 0);
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->xRowCount == NULL || out == NULL) return SQLITE_MISUSE;
  value = 0;
  status = api->xRowCount(cpkt_sqlite_native_fts5_context(self), &value);
  if (status == SQLITE_OK) *out = cpkt_sqlite_public_i64(value);
  return status;
}

static int cpkt_sqlite_fts5_context_column_total_size(const cpkt_sqlite_fts5_context *self,
  ...
  if (out != NULL) *out = cpkt_sqlite_i64_make(0, 0);
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->xColumnTotalSize == NULL || out == NULL) return SQLITE_MISUSE;
  value = 0;
  status = api->xColumnTotalSize(cpkt_sqlite_native_fts5_context(self), column, &value);
  if (status == SQLITE_OK) *out = cpkt_sqlite_public_i64(value);
  return status;
}

static int cpkt_sqlite_fts5_context_tokenize(cpkt_sqlite_fts5_context *self,
    const char *text, int text_byte_count, void *context,
  ...
  return api->xTokenize(cpkt_sqlite_native_fts5_context(self), text, text_byte_count,
      &token_context, cpkt_sqlite_fts5_public_token_trampoline);
}

static int cpkt_sqlite_fts5_context_tokenize_locale(cpkt_sqlite_fts5_context *self,
    const char *text, int text_byte_count, const char *locale, int locale_byte_count,
    void *context, cpkt_sqlite_fts5_token_callback token) {
  const Fts5ExtensionApi *api;
  cpkt_sqlite_fts5_public_token_context token_context;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->iVersion < 4 || api->xTokenize_v2 == NULL || text == NULL ||
      (locale == NULL && locale_byte_count != 0) || token == NULL) return SQLITE_MISUSE;
  token_context.user_context = context;
  token_context.callback = token;
  return api->xTokenize_v2(cpkt_sqlite_native_fts5_context(self), text, text_byte_count,
      locale, locale_byte_count, &token_context, cpkt_sqlite_fts5_public_token_trampoline);
}

static int cpkt_sqlite_fts5_context_phrase_count(const cpkt_sqlite_fts5_context *self) {
  const Fts5ExtensionApi *api;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->xPhraseCount == NULL) return 0;
  return api->xPhraseCount(cpkt_sqlite_native_fts5_context(self));
}

static int cpkt_sqlite_fts5_context_phrase_size(const cpkt_sqlite_fts5_context *self,
    int phrase) {
  const Fts5ExtensionApi *api;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->xPhraseSize == NULL) return 0;
  return api->xPhraseSize(cpkt_sqlite_native_fts5_context(self), phrase);
}

static int cpkt_sqlite_fts5_context_instance_count(const cpkt_sqlite_fts5_context *self,
    int *out) {
  const Fts5ExtensionApi *api;
  if (out != NULL) *out = 0;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->xInstCount == NULL || out == NULL) return SQLITE_MISUSE;
  return api->xInstCount(cpkt_sqlite_native_fts5_context(self), out);
}

static int cpkt_sqlite_fts5_context_instance(const cpkt_sqlite_fts5_context *self,
    int index, int *phrase_out, int *column_out, int *offset_out) {
  const Fts5ExtensionApi *api;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->xInst == NULL || phrase_out == NULL || column_out == NULL ||
      offset_out == NULL) return SQLITE_MISUSE;
  return api->xInst(cpkt_sqlite_native_fts5_context(self), index, phrase_out,
      column_out, offset_out);
}

static cpkt_sqlite_i64 cpkt_sqlite_fts5_context_rowid(const cpkt_sqlite_fts5_context *self) {
  const Fts5ExtensionApi *api;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->xRowid == NULL) return cpkt_sqlite_i64_make(0, 0);
  return cpkt_sqlite_public_i64(api->xRowid(cpkt_sqlite_native_fts5_context(self)));
}

static int cpkt_sqlite_fts5_context_column_text(const cpkt_sqlite_fts5_context *self,
    int column, const char **text_out, int *byte_count_out) {
  const Fts5ExtensionApi *api;
  if (text_out != NULL) *text_out = NULL;
  if (byte_count_out != NULL) *byte_count_out = 0;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->xColumnText == NULL || text_out == NULL ||
      byte_count_out == NULL) return SQLITE_MISUSE;
  return api->xColumnText(cpkt_sqlite_native_fts5_context(self), column, text_out,
      byte_count_out);
}

static int cpkt_sqlite_fts5_context_column_size(const cpkt_sqlite_fts5_context *self,
    int column, int *token_count_out) {
  const Fts5ExtensionApi *api;
  if (token_count_out != NULL) *token_count_out = 0;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->xColumnSize == NULL || token_count_out == NULL) {
    return SQLITE_MISUSE;
  }
  return api->xColumnSize(cpkt_sqlite_native_fts5_context(self), column, token_count_out);
}

static int cpkt_sqlite_fts5_query_phrase_trampoline(const Fts5ExtensionApi *api,
    Fts5Context *context, void *user_data) {
  ...
  if (query_context == NULL || query_context->callback == NULL) return SQLITE_MISUSE;
...
static int cpkt_sqlite_fts5_context_query_phrase(cpkt_sqlite_fts5_context *self,
    int phrase, void *user_data, cpkt_sqlite_fts5_query_phrase_callback callback) {
  const Fts5ExtensionApi *api;
  cpkt_sqlite_fts5_query_phrase_context query_context;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->xQueryPhrase == NULL || callback == NULL) return SQLITE_MISUSE;
  ...
  return api->xQueryPhrase(cpkt_sqlite_native_fts5_context(self), phrase,
      &query_context, cpkt_sqlite_fts5_query_phrase_trampoline);
}

static int cpkt_sqlite_fts5_context_set_auxdata(cpkt_sqlite_fts5_context *self,
    void *data, cpkt_sqlite_destroy_callback destroy) {
  const Fts5ExtensionApi *api;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->xSetAuxdata == NULL) return SQLITE_MISUSE;
  return api->xSetAuxdata(cpkt_sqlite_native_fts5_context(self), data, destroy);
}

static void *cpkt_sqlite_fts5_context_get_auxdata(cpkt_sqlite_fts5_context *self,
    int clear) {
  const Fts5ExtensionApi *api;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->xGetAuxdata == NULL) return NULL;
  return api->xGetAuxdata(cpkt_sqlite_native_fts5_context(self), clear);
}

static int cpkt_sqlite_fts5_context_phrase_first(cpkt_sqlite_fts5_context *self,
    int phrase, cpkt_sqlite_fts5_phrase_iterator *iterator, int *column_out,
    int *offset_out) {
  const Fts5ExtensionApi *api;
  Fts5PhraseIter native_iterator;
  int status;
  if (iterator == NULL || column_out == NULL || offset_out == NULL) return SQLITE_MISUSE;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->xPhraseFirst == NULL) return SQLITE_MISUSE;
...
static void cpkt_sqlite_fts5_context_phrase_next(cpkt_sqlite_fts5_context *self,
    cpkt_sqlite_fts5_phrase_iterator *iterator, int *column_out, int *offset_out) {
  const Fts5ExtensionApi *api;
  Fts5PhraseIter native_iterator;
  if (iterator == NULL || column_out == NULL || offset_out == NULL) return;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->xPhraseNext == NULL) return;
  native_iterator.a = iterator->first;
  native_iterator.b = iterator->second;
...
static int cpkt_sqlite_fts5_context_phrase_first_column(cpkt_sqlite_fts5_context *self,
    int phrase, cpkt_sqlite_fts5_phrase_iterator *iterator, int *column_out) {
  const Fts5ExtensionApi *api;
  Fts5PhraseIter native_iterator;
  int status;
  if (iterator == NULL || column_out == NULL) return SQLITE_MISUSE;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->xPhraseFirstColumn == NULL) return SQLITE_MISUSE;
  native_iterator.a = iterator->first;
  native_iterator.b = iterator->second;
  status = api->xPhraseFirstColumn(cpkt_sqlite_native_fts5_context(self), phrase,
...
static void cpkt_sqlite_fts5_context_phrase_next_column(cpkt_sqlite_fts5_context *self,
    cpkt_sqlite_fts5_phrase_iterator *iterator, int *column_out) {
  const Fts5ExtensionApi *api;
  Fts5PhraseIter native_iterator;
  if (iterator == NULL || column_out == NULL) return;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->xPhraseNextColumn == NULL) return;
  native_iterator.a = iterator->first;
  native_iterator.b = iterator->second;
  api->xPhraseNextColumn(cpkt_sqlite_native_fts5_context(self), &native_iterator,
      column_out);
  iterator->first = native_iterator.a;
  iterator->second = native_iterator.b;
}
...
static int cpkt_sqlite_fts5_context_query_token(cpkt_sqlite_fts5_context *self,
    int phrase, int token, const char **text_out, int *byte_count_out) {
  const Fts5ExtensionApi *api;
  if (text_out != NULL) *text_out = NULL;
  if (byte_count_out != NULL) *byte_count_out = 0;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->iVersion < 3 || api->xQueryToken == NULL ||
      text_out == NULL || byte_count_out == NULL) return SQLITE_MISUSE;
  return api->xQueryToken(cpkt_sqlite_native_fts5_context(self), phrase, token,
      text_out, byte_count_out);
}

static int cpkt_sqlite_fts5_context_instance_token(cpkt_sqlite_fts5_context *self,
    int instance, int token, const char **text_out, int *byte_count_out) {
  const Fts5ExtensionApi *api;
  if (text_out != NULL) *text_out = NULL;
  if (byte_count_out != NULL) *byte_count_out = 0;
  api = cpkt_sqlite_native_fts5_extension_api(self);
...
static int cpkt_sqlite_fts5_context_column_locale(cpkt_sqlite_fts5_context *self,
    int column, const char **locale_out, int *byte_count_out) {
  const Fts5ExtensionApi *api;
  if (locale_out != NULL) *locale_out = NULL;
  if (byte_count_out != NULL) *byte_count_out = 0;
  api = cpkt_sqlite_native_fts5_extension_api(self);
  if (api == NULL || api->iVersion < 4 || api->xColumnLocale == NULL ||
      locale_out == NULL || byte_count_out == NULL) return SQLITE_MISUSE;
...
static void cpkt_sqlite_fts5_auxiliary_trampoline(const Fts5ExtensionApi *api,
    Fts5Context *fts_context, sqlite3_context *sql_context, int argument_count,
  ...
  int index;
  binding = api == NULL || api->xUserData == NULL ? NULL :
  ...
  if (argument_count > 0) {
    ...
    public_value_pointers = (cpkt_sqlite_value **) calloc((size_t) argument_count,
        sizeof(*public_value_pointers));
...
static int cpkt_sqlite_fts5_api_create_tokenizer(cpkt_sqlite_fts5_api *self,
    const char *name, void *context, cpkt_sqlite_fts5_tokenizer_create_callback create,
    cpkt_sqlite_fts5_tokenizer_destroy_callback destroy,
    cpkt_sqlite_fts5_tokenizer_tokenize_callback tokenize,
    cpkt_sqlite_destroy_callback binding_destroy) {
  fts5_api *api;
  ...
  if (api == NULL || name == NULL || create == NULL || destroy == NULL ||
      tokenize == NULL || api->iVersion < 3 || api->xCreateTokenizer_v2 == NULL) {
    return SQLITE_MISUSE;
  }
  ...
  binding->native_tokenizer.xCreate = cpkt_sqlite_fts5_tokenizer_create_trampoline;
  binding->native_tokenizer.xDelete = cpkt_sqlite_fts5_tokenizer_delete_trampoline;
  binding->native_tokenizer.xTokenize = cpkt_sqlite_fts5_tokenizer_tokenize_trampoline;
  status = api->xCreateTokenizer_v2(api, name, binding, &binding->native_tokenizer,
...
static int cpkt_sqlite_fts5_api_create_auxiliary(cpkt_sqlite_fts5_api *self,
    const char *name, void *user_data, cpkt_sqlite_fts5_auxiliary_callback callback,
    cpkt_sqlite_destroy_callback destroy) {
  fts5_api *api;
  cpkt_sqlite_fts5_auxiliary_binding *binding;
  int status;
  api = cpkt_sqlite_native_fts5_api(self);
  if (api == NULL || name == NULL || callback == NULL || api->xCreateFunction == NULL) {
    return SQLITE_MISUSE;
  }
  ...
  binding->callback = callback;
  binding->destroy = destroy;
  status = api->xCreateFunction(api, name, binding, cpkt_sqlite_fts5_auxiliary_trampoline,
...
int cpkt_sqlite_fts5_api_open(cpkt_sqlite *database, cpkt_sqlite_fts5_api **out) {
...
cpkt_sqlite_fts5_tokenizer *cpkt_sqlite_fts5_tokenizer_new(void *state) {
  ...
  if (tokenizer != NULL) tokenizer->state = state;
  return tokenizer;
}
...
int cpkt_sqlite_glob(const char *pattern, const char *text) { return sqlite3_strglob(pattern, text); }
...
int cpkt_sqlite_status_i64(int category, cpkt_sqlite_i64 *current,
  ...
  native_current = 0;
  native_highwater = 0;
  status = sqlite3_status64(category, &native_current, &native_highwater, reset);
  if (current != NULL) *current = cpkt_sqlite_public_i64(native_current);
  if (highwater != NULL) *highwater = cpkt_sqlite_public_i64(native_highwater);
  return status;
}

cpkt_sqlite_i64 cpkt_sqlite_changes64(const cpkt_sqlite *self) {
  if (self == NULL || cpkt_sqlite_native(self) == NULL) return cpkt_sqlite_i64_make(0, 0);
...
cpkt_sqlite_i64 cpkt_sqlite_total_changes64(const cpkt_sqlite *self) {
  if (self == NULL || cpkt_sqlite_native(self) == NULL) return cpkt_sqlite_i64_make(0, 0);
  return cpkt_sqlite_public_i64(sqlite3_total_changes64(cpkt_sqlite_native(self)));
}

void cpkt_sqlite_set_last_insert_rowid(cpkt_sqlite *self, cpkt_sqlite_i64 value) {
  if (self != NULL && cpkt_sqlite_native(self) != NULL) {
    sqlite3_set_last_insert_rowid(cpkt_sqlite_native(self), cpkt_sqlite_native_i64(value));
  }
}

void cpkt_sqlite_interrupt(cpkt_sqlite *self) {
  if (self != NULL && cpkt_sqlite_native(self) != NULL) sqlite3_interrupt(cpkt_sqlite_native(self));
}
...
int cpkt_sqlite_database_readonly(const cpkt_sqlite *self, const char *name) {
  if (self == NULL || cpkt_sqlite_native(self) == NULL) return -1;
...
int cpkt_sqlite_table_column_metadata(cpkt_sqlite *self, const char *database_name,
  ...
  primary_key = 0;
  auto_increment = 0;
  status = sqlite3_table_column_metadata(cpkt_sqlite_native(self), database_name,
      table_name, column_name, metadata_out == NULL ? NULL : &declared_type,
  ...
  if (status != SQLITE_OK || metadata_out == NULL) return status;
...
int cpkt_sqlite_file_control(cpkt_sqlite *self, const char *database_name,
  ...
  if (self == NULL || cpkt_sqlite_native(self) == NULL) return SQLITE_MISUSE;
  return sqlite3_file_control(cpkt_sqlite_native(self), database_name, operation,
      argument);
}

int cpkt_sqlite_set_lock_timeout(cpkt_sqlite *self, int milliseconds,
    unsigned long flags) {
  if (self == NULL || cpkt_sqlite_native(self) == NULL || flags > 0xffffffffUL) {
    return SQLITE_MISUSE;
  }
  return sqlite3_setlk_timeout(cpkt_sqlite_native(self), milliseconds, (int) flags);
}

int cpkt_sqlite_status_database(const cpkt_sqlite *self, int category,
    int *current, int *highwater, int reset) {
  if (self == NULL || cpkt_sqlite_native(self) == NULL) return CPKT_SQLITE_MISUSE;
  return sqlite3_db_status(cpkt_sqlite_native(self), category, current, highwater, reset);
}
...
int cpkt_sqlite_set_authorizer(cpkt_sqlite *self,
  ...
  return sqlite3_set_authorizer(cpkt_sqlite_native(self),
...
int cpkt_sqlite_set_collation_needed(cpkt_sqlite *self,
  ...
  return sqlite3_collation_needed(cpkt_sqlite_native(self),
      callback == NULL ? NULL : state,
...
int cpkt_sqlite_set_collation_needed16(cpkt_sqlite *self,
  ...
  state->collation_needed_context = context;
  return sqlite3_collation_needed16(cpkt_sqlite_native(self),
      callback == NULL ? NULL : state,
...
int cpkt_sqlite_set_autovacuum_callback(cpkt_sqlite *self,
  ...
  if (self == NULL || cpkt_sqlite_native(self) == NULL) return SQLITE_MISUSE;
  if (callback == NULL) {
    if (context != NULL || destroy != NULL) return SQLITE_MISUSE;
  ...
  status = sqlite3_autovacuum_pages(cpkt_sqlite_native(self),
      cpkt_sqlite_autovacuum_trampoline, binding, cpkt_sqlite_autovacuum_destroy);
  if (status != SQLITE_OK) cpkt_sqlite_autovacuum_destroy(binding);
  return status;
}

void *cpkt_sqlite_client_data(const cpkt_sqlite *self, const char *name) {
  if (self == NULL || cpkt_sqlite_native(self) == NULL || name == NULL) return NULL;
...
void *cpkt_sqlite_set_legacy_trace(cpkt_sqlite *self,
  ...
  previous = state->trace_callback != NULL ? state->trace_context :
      (state->legacy_trace_callback != NULL ? state->legacy_trace_context :
  ...
  (void) sqlite3_trace(cpkt_sqlite_native(self),
      callback == NULL ? NULL : cpkt_sqlite_legacy_trace_trampoline,
      callback == NULL ? NULL : state);
  return previous;
}
...
void *cpkt_sqlite_set_legacy_profile(cpkt_sqlite *self,
  ...
  (void) sqlite3_profile(cpkt_sqlite_native(self),
...
int cpkt_sqlite_register_rtree_geometry(cpkt_sqlite *self, const char *name,
  ...
  int status;
  if (self == NULL || cpkt_sqlite_native(self) == NULL || name == NULL ||
      callback == NULL) return SQLITE_MISUSE;
  state = cpkt_sqlite_state_for(self);
  if (state == NULL) return SQLITE_NOMEM;
  ...
  binding->context = context;
  binding->callback = callback;
  status = sqlite3_rtree_geometry_callback(cpkt_sqlite_native(self), name,
      cpkt_sqlite_rtree_geometry_trampoline, binding);
  if (status != SQLITE_OK) {
    free(binding);
    return status;
  }
...
int cpkt_sqlite_register_rtree_query(cpkt_sqlite *self, const char *name,
  ...
  binding->callback = callback;
  binding->destroy = destroy;
  status = sqlite3_rtree_query_callback(cpkt_sqlite_native(self), name,
      cpkt_sqlite_rtree_query_trampoline, binding, cpkt_sqlite_rtree_query_destroy);
  if (status != SQLITE_OK) free(binding);
  return status;
}
...
void cpkt_sqlite_set_progress_handler(cpkt_sqlite *self, int instruction_count,
  ...
  cpkt_sqlite_state *state;
  if (self == NULL || cpkt_sqlite_native(self) == NULL) return;
  state = cpkt_sqlite_state_for(self);
  if (state == NULL) return;
  ...
  sqlite3_progress_handler(cpkt_sqlite_native(self), instruction_count,
      callback == NULL ? NULL : cpkt_sqlite_progress_trampoline,
      callback == NULL ? NULL : state);
}
...
void cpkt_sqlite_set_rollback_hook(cpkt_sqlite *self,
  ...
  sqlite3_rollback_hook(cpkt_sqlite_native(self),
      callback == NULL ? NULL : cpkt_sqlite_rollback_trampoline,
      callback == NULL ? NULL : state);
}
...
int cpkt_sqlite_set_preupdate_hook(cpkt_sqlite *self,
  ...
  state = cpkt_sqlite_state_for(self);
  if (state == NULL) return CPKT_SQLITE_NOMEM;
  if (callback != NULL && state->session_count != 0) return CPKT_SQLITE_MISUSE;
  state->preupdate_callback = callback;
  state->preupdate_context = context;
  sqlite3_preupdate_hook(cpkt_sqlite_native(self),
      callback == NULL ? NULL : cpkt_sqlite_preupdate_trampoline,
      callback == NULL ? NULL : state);
  return CPKT_SQLITE_OK;
}
...
static int cpkt_sqlite_preupdate_value(cpkt_sqlite *self, int column,
  ...
  int status;
  if (out != NULL) *out = NULL;
  if (self == NULL || cpkt_sqlite_native(self) == NULL || out == NULL) return CPKT_SQLITE_MISUSE;
  native_value = NULL;
  status = old_value ? sqlite3_preupdate_old(cpkt_sqlite_native(self), column, &native_value) :
      sqlite3_preupdate_new(cpkt_sqlite_native(self), column, &native_value);
  if (status != SQLITE_OK || native_value == NULL) return status;
  public_value = (cpkt_sqlite_value *) calloc(1, sizeof(*public_value));
  if (public_value == NULL) return CPKT_SQLITE_NOMEM;
...
static int cpkt_sqlite_blob_read(cpkt_sqlite_blob *self, void *buffer,
  ...
  if (cpkt_sqlite_native_blob(self) == NULL) return CPKT_SQLITE_MISUSE;
  return sqlite3_blob_read(cpkt_sqlite_native_blob(self), buffer, byte_count, offset);
}

static int cpkt_sqlite_blob_write(cpkt_sqlite_blob *self, const void *buffer,
    int byte_count, int offset) {
  if (cpkt_sqlite_native_blob(self) == NULL) return CPKT_SQLITE_MISUSE;
  return sqlite3_blob_write(cpkt_sqlite_native_blob(self), buffer, byte_count, offset);
}

static int cpkt_sqlite_blob_reopen(cpkt_sqlite_blob *self, cpkt_sqlite_i64 row_id) {
  if (cpkt_sqlite_native_blob(self) == NULL) return CPKT_SQLITE_MISUSE;
  return sqlite3_blob_reopen(cpkt_sqlite_native_blob(self), cpkt_sqlite_native_i64(row_id));
}

static int cpkt_sqlite_blob_bytes(const cpkt_sqlite_blob *self) {
  if (cpkt_sqlite_native_blob(self) == NULL) return 0;
...
static int cpkt_sqlite_blob_close(cpkt_sqlite_blob *self) {
  ...
  status = cpkt_sqlite_native_blob(self) == NULL ? CPKT_SQLITE_OK :
...
int cpkt_sqlite_open_blob(cpkt_sqlite *self, const char *database_name,
  ...
  int status;
  if (out != NULL) *out = NULL;
  if (self == NULL || cpkt_sqlite_native(self) == NULL || database_name == NULL ||
      table_name == NULL || column_name == NULL || out == NULL) return CPKT_SQLITE_MISUSE;
  native_blob = NULL;
  status = sqlite3_blob_open(cpkt_sqlite_native(self), database_name, table_name,
      column_name, cpkt_sqlite_native_i64(row_id), writable, &native_blob);
  if (status != SQLITE_OK) return status;
...
static int cpkt_sqlite_backup_step(cpkt_sqlite_backup *self, int page_count) {
  if (cpkt_sqlite_native_backup(self) == NULL) return CPKT_SQLITE_MISUSE;
...
static int cpkt_sqlite_backup_remaining(const cpkt_sqlite_backup *self) {
  if (cpkt_sqlite_native_backup(self) == NULL) return 0;
...
static int cpkt_sqlite_backup_close(cpkt_sqlite_backup *self) {
  ...
  source = self->source;
  status = cpkt_sqlite_native_backup(self) == NULL ? CPKT_SQLITE_OK :
...
int cpkt_sqlite_backup_start(cpkt_sqlite *destination, const char *destination_name,
  ...
  if (destination == NULL || source == NULL || destination_name == NULL ||
      source_name == NULL || out == NULL) return CPKT_SQLITE_MISUSE;
  native_backup = sqlite3_backup_init(cpkt_sqlite_native(destination), destination_name,
      cpkt_sqlite_native(source), source_name);
  if (native_backup == NULL) return sqlite3_errcode(cpkt_sqlite_native(destination));
...
int cpkt_sqlite_snapshot_get(cpkt_sqlite *self, const char *schema,
  ...
  int status;
  if (out != NULL) *out = NULL;
  if (self == NULL || cpkt_sqlite_native(self) == NULL || schema == NULL || out == NULL) {
    return CPKT_SQLITE_MISUSE;
  }
  native_snapshot = NULL;
  status = sqlite3_snapshot_get(cpkt_sqlite_native(self), schema, &native_snapshot);
  if (status != SQLITE_OK) return status;
...
int cpkt_sqlite_snapshot_open(cpkt_sqlite *self, const char *schema,
  ...
  if (self == NULL || schema == NULL || snapshot == NULL || snapshot->snapshot == NULL) {
    return CPKT_SQLITE_MISUSE;
  }
...
void cpkt_sqlite_snapshot_free(cpkt_sqlite_snapshot *snapshot) {
  ...
  if (snapshot->snapshot != NULL) sqlite3_snapshot_free((sqlite3_snapshot *) snapshot->snapshot);
...
unsigned char *cpkt_sqlite_serialize(cpkt_sqlite *self, const char *schema,
  ...
  if (byte_count != NULL) *byte_count = cpkt_sqlite_i64_make(0, 0);
  if (self == NULL || cpkt_sqlite_native(self) == NULL) return NULL;
  native_count = 0;
  result = sqlite3_serialize(cpkt_sqlite_native(self), schema, &native_count,
      (unsigned int) flags);
  if (byte_count != NULL) *byte_count = cpkt_sqlite_public_i64(native_count);
  return result;
}
...
int cpkt_sqlite_deserialize(cpkt_sqlite *self, const char *schema,
  ...
  if (self == NULL || cpkt_sqlite_native(self) == NULL || data == NULL) return CPKT_SQLITE_MISUSE;
  return sqlite3_deserialize(cpkt_sqlite_native(self), schema, data,
      cpkt_sqlite_native_i64(database_size), cpkt_sqlite_native_i64(buffer_size),
      (unsigned int) flags);
}
...
int cpkt_sqlite_create_function(cpkt_sqlite *self, const char *name,
  ...
  if (binding == NULL) return CPKT_SQLITE_NOMEM;
  ...
  binding->final = final;
  binding->destroy = destroy;
  status = sqlite3_create_function_v2(cpkt_sqlite_native(self), name, argument_count,
      (int) text_representation, binding,
      scalar == NULL ? NULL : cpkt_sqlite_function_scalar_trampoline,
...
int cpkt_sqlite_create_function16(cpkt_sqlite *self, const void *name,
  ...
  binding->step = step;
  binding->final = final;
  status = sqlite3_create_function16(cpkt_sqlite_native(self), name, argument_count,
      (int) text_representation, binding,
      scalar == NULL ? NULL : cpkt_sqlite_function_scalar_trampoline,
      step == NULL ? NULL : cpkt_sqlite_function_step_trampoline,
      final == NULL ? NULL : cpkt_sqlite_function_final_trampoline);
...
int cpkt_sqlite_create_window_function(cpkt_sqlite *self, const char *name,
  ...
  binding->destroy = destroy;
  status = sqlite3_create_window_function(cpkt_sqlite_native(self), name,
      argument_count, (int) text_representation, binding,
      cpkt_sqlite_function_step_trampoline, cpkt_sqlite_function_final_trampoline,
      cpkt_sqlite_function_value_trampoline, cpkt_sqlite_function_inverse_trampoline,
      cpkt_sqlite_function_destroy);
  if (status != SQLITE_OK) return status;
  return CPKT_SQLITE_OK;
}
...
int cpkt_sqlite_create_collation(cpkt_sqlite *self, const char *name,
  ...
  binding->destroy = destroy;
  status = sqlite3_create_collation_v2(cpkt_sqlite_native(self), name,
      (int) text_representation, binding,
...
void *cpkt_sqlite_context_user_data(cpkt_sqlite_context *context) {
  cpkt_sqlite_function_binding *binding;
  if (cpkt_sqlite_native_context(context) == NULL) return NULL;
...
void *cpkt_sqlite_context_aggregate(cpkt_sqlite_context *context, int byte_count) {
  if (cpkt_sqlite_native_context(context) == NULL) return NULL;
  return sqlite3_aggregate_context(cpkt_sqlite_native_context(context), byte_count);
}

int cpkt_sqlite_context_aggregate_count(cpkt_sqlite_context *context) {
  if (cpkt_sqlite_native_context(context) == NULL) return 0;
...
int cpkt_sqlite_context_set_auxdata(cpkt_sqlite_context *context, int argument_index,
  ...
  binding->data = data;
  binding->destroy = destroy;
  sqlite3_set_auxdata(cpkt_sqlite_native_context(context), argument_index, binding,
      cpkt_sqlite_auxdata_destroy);
  return CPKT_SQLITE_OK;
}

void cpkt_sqlite_context_result_null(cpkt_sqlite_context *context) {
  if (cpkt_sqlite_native_context(context) != NULL) sqlite3_result_null(cpkt_sqlite_native_context(context));
}

void cpkt_sqlite_context_result_int(cpkt_sqlite_context *context, int value) {
  if (cpkt_sqlite_native_context(context) != NULL) sqlite3_result_int(cpkt_sqlite_native_context(context), value);
}
...
void cpkt_sqlite_context_result_i64(cpkt_sqlite_context *context, cpkt_sqlite_i64 value) {
  if (cpkt_sqlite_native_context(context) != NULL) {
    sqlite3_result_int64(cpkt_sqlite_native_context(context), cpkt_sqlite_native_i64(value));
  }
}

void cpkt_sqlite_context_result_double(cpkt_sqlite_context *context, double value) {
  if (cpkt_sqlite_native_context(context) != NULL) sqlite3_result_double(cpkt_sqlite_native_context(context), value);
}

void cpkt_sqlite_context_result_text(cpkt_sqlite_context *context,
    const char *value, int byte_count) {
  if (cpkt_sqlite_native_context(context) != NULL) {
    sqlite3_result_text(cpkt_sqlite_native_context(context), value, byte_count, SQLITE_TRANSIENT);
  }
}
...
void cpkt_sqlite_context_result_text16(cpkt_sqlite_context *context,
  ...
  if (cpkt_sqlite_native_context(context) != NULL) {
    sqlite3_result_text16(cpkt_sqlite_native_context(context), value, byte_count,
        SQLITE_TRANSIENT);
  }
}
...
void cpkt_sqlite_context_result_text16le(cpkt_sqlite_context *context,
    const void *value, int byte_count) {
  if (cpkt_sqlite_native_context(context) != NULL) {
    sqlite3_result_text16le(cpkt_sqlite_native_context(context), value, byte_count,
        SQLITE_TRANSIENT);
  }
}
...
void cpkt_sqlite_context_result_text16be(cpkt_sqlite_context *context,
    const void *value, int byte_count) {
  if (cpkt_sqlite_native_context(context) != NULL) {
    sqlite3_result_text16be(cpkt_sqlite_native_context(context), value, byte_count,
        SQLITE_TRANSIENT);
  }
}
...
void cpkt_sqlite_context_result_text_u64(cpkt_sqlite_context *context,
  ...
  if (cpkt_sqlite_native_context(context) != NULL) {
    sqlite3_result_text64(cpkt_sqlite_native_context(context), value,
        cpkt_sqlite_native_u64(byte_count), SQLITE_TRANSIENT, (unsigned char) encoding);
  }
}
...
void cpkt_sqlite_context_result_blob(cpkt_sqlite_context *context,
    const void *value, int byte_count) {
  if (cpkt_sqlite_native_context(context) != NULL) {
    sqlite3_result_blob(cpkt_sqlite_native_context(context), value, byte_count, SQLITE_TRANSIENT);
  }
}
...
int cpkt_sqlite_context_result_zero_blob(cpkt_sqlite_context *context, int byte_count) {
  if (cpkt_sqlite_native_context(context) == NULL) return CPKT_SQLITE_MISUSE;
...
void cpkt_sqlite_context_result_pointer(cpkt_sqlite_context *context,
  ...
  if (cpkt_sqlite_native_context(context) != NULL && type_name != NULL) {
    sqlite3_result_pointer(cpkt_sqlite_native_context(context), value, type_name, destroy);
  }
}
...
void cpkt_sqlite_context_result_error(cpkt_sqlite_context *context,
  ...
  if (cpkt_sqlite_native_context(context) != NULL) {
    sqlite3_result_error(cpkt_sqlite_native_context(context), message, byte_count);
  }
}
...
void cpkt_sqlite_context_result_error16(cpkt_sqlite_context *context,
  ...
  if (cpkt_sqlite_native_context(context) != NULL) {
    sqlite3_result_error16(cpkt_sqlite_native_context(context), message, byte_count);
  }
}

void cpkt_sqlite_context_result_error_code(cpkt_sqlite_context *context, int code) {
  if (cpkt_sqlite_native_context(context) != NULL) sqlite3_result_error_code(cpkt_sqlite_native_context(context), code);
}

void cpkt_sqlite_context_result_value(cpkt_sqlite_context *context,
    const cpkt_sqlite_value *value) {
  if (cpkt_sqlite_native_context(context) != NULL && cpkt_sqlite_native_value(value) != NULL) {
    sqlite3_result_value(cpkt_sqlite_native_context(context), cpkt_sqlite_native_value(value));
  }
}
...
void cpkt_sqlite_context_result_subtype(cpkt_sqlite_context *context,
  ...
  if (cpkt_sqlite_native_context(context) != NULL) {
    sqlite3_result_subtype(cpkt_sqlite_native_context(context), (unsigned int) subtype);
  }
}

int cpkt_sqlite_value_type(const cpkt_sqlite_value *value) {
  return cpkt_sqlite_native_value(value) == NULL ? CPKT_SQLITE_NULL :
...
int cpkt_sqlite_value_int(const cpkt_sqlite_value *value) {
  return cpkt_sqlite_native_value(value) == NULL ? 0 : sqlite3_value_int(cpkt_sqlite_native_value(value));
}

cpkt_sqlite_i64 cpkt_sqlite_value_i64(const cpkt_sqlite_value *value) {
  if (cpkt_sqlite_native_value(value) == NULL) return cpkt_sqlite_i64_make(0, 0);
  return cpkt_sqlite_public_i64(sqlite3_value_int64(cpkt_sqlite_native_value(value)));
}

double cpkt_sqlite_value_double(const cpkt_sqlite_value *value) {
  return cpkt_sqlite_native_value(value) == NULL ? 0.0 : sqlite3_value_double(cpkt_sqlite_native_value(value));
}

const void *cpkt_sqlite_value_blob(const cpkt_sqlite_value *value) {
  return cpkt_sqlite_native_value(value) == NULL ? NULL : sqlite3_value_blob(cpkt_sqlite_native_value(value));
}

const unsigned char *cpkt_sqlite_value_text(const cpkt_sqlite_value *value) {
  return cpkt_sqlite_native_value(value) == NULL ? NULL : sqlite3_value_text(cpkt_sqlite_native_value(value));
}

const void *cpkt_sqlite_value_text16(const cpkt_sqlite_value *value) {
  return cpkt_sqlite_native_value(value) == NULL ? NULL :
...
void *cpkt_sqlite_value_pointer(const cpkt_sqlite_value *value, const char *type_name) {
  if (cpkt_sqlite_native_value(value) == NULL || type_name == NULL) return NULL;
...
int cpkt_sqlite_value_bytes(const cpkt_sqlite_value *value) {
  return cpkt_sqlite_native_value(value) == NULL ? 0 : sqlite3_value_bytes(cpkt_sqlite_native_value(value));
}

int cpkt_sqlite_value_bytes16(const cpkt_sqlite_value *value) {
  return cpkt_sqlite_native_value(value) == NULL ? 0 :
...
unsigned long cpkt_sqlite_value_subtype(const cpkt_sqlite_value *value) {
  return cpkt_sqlite_native_value(value) == NULL ? 0UL :
      (unsigned long) sqlite3_value_subtype(cpkt_sqlite_native_value(value));
}
...
cpkt_sqlite_value *cpkt_sqlite_value_duplicate(const cpkt_sqlite_value *value) {
  ...
  if (cpkt_sqlite_native_value(value) == NULL) return NULL;
  native_copy = sqlite3_value_dup(cpkt_sqlite_native_value(value));
  if (native_copy == NULL) return NULL;
...
void cpkt_sqlite_value_free(cpkt_sqlite_value *value) {
  ...
  if (!value->shell_owned) return;
  if (value->owned && value->value != NULL) sqlite3_value_free((sqlite3_value *) value->value);
  value->value = NULL;
  free(value);
}

static int cpkt_sqlite_session_attach(cpkt_sqlite_session *self, const char *table_name) {
  if (cpkt_sqlite_native_session(self) == NULL) return CPKT_SQLITE_MISUSE;
...
static int cpkt_sqlite_session_enable(cpkt_sqlite_session *self, int enabled) {
  if (cpkt_sqlite_native_session(self) == NULL) return 0;
  return sqlite3session_enable(cpkt_sqlite_native_session(self), enabled);
}

static int cpkt_sqlite_session_indirect(cpkt_sqlite_session *self, int indirect) {
  if (cpkt_sqlite_native_session(self) == NULL) return 0;
...
static int cpkt_sqlite_session_empty(const cpkt_sqlite_session *self) {
  if (cpkt_sqlite_native_session(self) == NULL) return 1;
...
static int cpkt_sqlite_session_object_config(cpkt_sqlite_session *self,
  ...
  return sqlite3session_object_config(cpkt_sqlite_native_session(self), operation, value);
}

static int cpkt_sqlite_session_filter_trampoline(void *context, const char *table_name) {
  cpkt_sqlite_session *self;
  self = (cpkt_sqlite_session *) context;
  if (self == NULL || self->filter == NULL) return 1;
  return self->filter(self->filter_context, table_name);
}

static void cpkt_sqlite_session_table_filter(cpkt_sqlite_session *self,
    cpkt_sqlite_session_filter_callback filter, void *context) {
  if (cpkt_sqlite_native_session(self) == NULL) return;
  self->filter = filter;
  self->filter_context = context;
  sqlite3session_table_filter(cpkt_sqlite_native_session(self),
      filter == NULL ? NULL : cpkt_sqlite_session_filter_trampoline,
      filter == NULL ? NULL : self);
}

static cpkt_sqlite_i64 cpkt_sqlite_session_changeset_size(
    const cpkt_sqlite_session *self) {
  if (cpkt_sqlite_native_session(self) == NULL) return cpkt_sqlite_i64_make(0, 0);
  return cpkt_sqlite_public_i64(sqlite3session_changeset_size(
      cpkt_sqlite_native_session(self)));
}

static cpkt_sqlite_i64 cpkt_sqlite_session_memory_used(
    const cpkt_sqlite_session *self) {
  if (cpkt_sqlite_native_session(self) == NULL) return cpkt_sqlite_i64_make(0, 0);
  return cpkt_sqlite_public_i64(sqlite3session_memory_used(
      cpkt_sqlite_native_session(self)));
}

static int cpkt_sqlite_session_diff(cpkt_sqlite_session *self,
    const char *from_database, const char *table_name, char **error_out) {
  if (error_out != NULL) *error_out = NULL;
  if (cpkt_sqlite_native_session(self) == NULL || from_database == NULL ||
      table_name == NULL) return CPKT_SQLITE_MISUSE;
...
static int cpkt_sqlite_session_stream(cpkt_sqlite_session *self,
    cpkt_sqlite_stream_output_callback output, void *context, int patchset) {
  cpkt_sqlite_stream_output_context output_context;
  ...
  if (patchset) {
    return sqlite3session_patchset_strm(cpkt_sqlite_native_session(self),
        cpkt_sqlite_stream_output_trampoline, &output_context);
  }
  return sqlite3session_changeset_strm(cpkt_sqlite_native_session(self),
      cpkt_sqlite_stream_output_trampoline, &output_context);
}

static int cpkt_sqlite_session_changeset_stream(cpkt_sqlite_session *self,
    cpkt_sqlite_stream_output_callback output, void *context) {
  return cpkt_sqlite_session_stream(self, output, context, 0);
}

static int cpkt_sqlite_session_patchset_stream(cpkt_sqlite_session *self,
    cpkt_sqlite_stream_output_callback output, void *context) {
...
static void cpkt_sqlite_changeset_free(cpkt_sqlite_changeset *self) {
  if (self == NULL) return;
  if (self->owned_data != NULL) sqlite3_free(self->owned_data);
  self->data = NULL;
...
static int cpkt_sqlite_session_changeset(cpkt_sqlite_session *self,
  ...
  int status;
  if (out != NULL) *out = NULL;
  if (cpkt_sqlite_native_session(self) == NULL || out == NULL) return CPKT_SQLITE_MISUSE;
  data = NULL;
  byte_count = 0;
  status = sqlite3session_changeset(cpkt_sqlite_native_session(self), &byte_count, &data);
  if (status != SQLITE_OK) return status;
...
static int cpkt_sqlite_session_patchset(cpkt_sqlite_session *self,
  ...
  data = NULL;
  byte_count = 0;
  status = sqlite3session_patchset(cpkt_sqlite_native_session(self), &byte_count, &data);
  if (status != SQLITE_OK) return status;
  return cpkt_sqlite_changeset_new_owned(data, byte_count, out);
}
...
static void cpkt_sqlite_session_close(cpkt_sqlite_session *self) {
  cpkt_sqlite_state *state;
  if (self == NULL) return;
  if (cpkt_sqlite_native_session(self) != NULL) sqlite3session_delete(cpkt_sqlite_native_session(self));
  state = cpkt_sqlite_state_for(self->database);
  if (state != NULL && state->session_count > 0) --state->session_count;
  self->session = NULL;
  self->database = NULL;
  free(self);
}
...
int cpkt_sqlite_session_new(cpkt_sqlite *database, const char *schema,
  ...
  if (cpkt_sqlite_state_for(database) == NULL) return CPKT_SQLITE_NOMEM;
  if (cpkt_sqlite_state_for(database)->preupdate_callback != NULL) {
    return CPKT_SQLITE_MISUSE;
  }
  native_session = NULL;
  status = sqlite3session_create(cpkt_sqlite_native(database), schema, &native_session);
  if (status != SQLITE_OK) return status;
...
static int cpkt_sqlite_iterator_operation(cpkt_sqlite_changeset_iterator *self,
    const char **table_name, int *column_count, int *operation, int *indirect) {
  if (cpkt_sqlite_native_iterator(self) == NULL) return CPKT_SQLITE_MISUSE;
  return sqlite3changeset_op(cpkt_sqlite_native_iterator(self), table_name,
      column_count, operation, indirect);
}

static int cpkt_sqlite_iterator_primary_key(cpkt_sqlite_changeset_iterator *self,
  ...
  native_columns = NULL;
  status = sqlite3changeset_pk(cpkt_sqlite_native_iterator(self), &native_columns,
      column_count);
  if (status != SQLITE_OK) return status;
...
static int cpkt_sqlite_iterator_value(cpkt_sqlite_changeset_iterator *self,
    int column, cpkt_sqlite_value **out, int kind) {
  sqlite3_value *native_value;
  cpkt_sqlite_value *public_value;
  int status;
  if (out != NULL) *out = NULL;
  if (cpkt_sqlite_native_iterator(self) == NULL || out == NULL) return CPKT_SQLITE_MISUSE;
  native_value = NULL;
  if (kind == 0) status = sqlite3changeset_old(cpkt_sqlite_native_iterator(self), column, &native_value);
  else if (kind == 1) status = sqlite3changeset_new(cpkt_sqlite_native_iterator(self), column, &native_value);
  else status = sqlite3changeset_conflict(cpkt_sqlite_native_iterator(self), column, &native_value);
  if (status != SQLITE_OK) return status;
  if (native_value == NULL) return CPKT_SQLITE_OK;
  public_value = (cpkt_sqlite_value *) calloc(1, sizeof(*public_value));
  if (public_value == NULL) return CPKT_SQLITE_NOMEM;
  public_value->value = native_value;
  public_value->owned = 0;
...
static int cpkt_sqlite_iterator_conflict_value(cpkt_sqlite_changeset_iterator *self,
    int column, cpkt_sqlite_value **out) {
  return cpkt_sqlite_iterator_value(self, column, out, 2);
}

static int cpkt_sqlite_iterator_foreign_key_conflicts(
    cpkt_sqlite_changeset_iterator *self, int *count_out) {
  if (count_out != NULL) *count_out = 0;
  if (cpkt_sqlite_native_iterator(self) == NULL || count_out == NULL) {
    return CPKT_SQLITE_MISUSE;
  }
  return sqlite3changeset_fk_conflicts(cpkt_sqlite_native_iterator(self), count_out);
}

static int cpkt_sqlite_iterator_close(cpkt_sqlite_changeset_iterator *self) {
  int status;
  if (self == NULL) return CPKT_SQLITE_MISUSE;
  status = cpkt_sqlite_native_iterator(self) == NULL ? CPKT_SQLITE_OK :
...
static void cpkt_sqlite_iterator_init(cpkt_sqlite_changeset_iterator *iterator,
  ...
  iterator->close = borrowed ? cpkt_sqlite_iterator_borrowed_close : cpkt_sqlite_iterator_close;
...
int cpkt_sqlite_changeset_start_ex(const cpkt_sqlite_changeset *changeset,
  ...
  int status;
  if (out != NULL) *out = NULL;
  if (changeset == NULL || changeset->data == NULL || out == NULL) return CPKT_SQLITE_MISUSE;
  native_iterator = NULL;
  status = sqlite3changeset_start_v2(&native_iterator, changeset->byte_count,
      (void *) changeset->data, flags);
  if (status != SQLITE_OK) return status;
  public_iterator = (cpkt_sqlite_changeset_iterator *) calloc(1, sizeof(*public_iterator));
...
int cpkt_sqlite_changeset_start_stream(cpkt_sqlite_stream_input_callback input,
  ...
  int status;
  if (out != NULL) *out = NULL;
  if (input == NULL || out == NULL) return CPKT_SQLITE_MISUSE;
  public_iterator = (cpkt_sqlite_changeset_iterator *) calloc(1,
      sizeof(*public_iterator));
  if (public_iterator == NULL) return CPKT_SQLITE_NOMEM;
  public_iterator->input = input;
  public_iterator->input_context = context;
  native_iterator = NULL;
  status = sqlite3changeset_start_v2_strm(&native_iterator,
      cpkt_sqlite_iterator_stream_input_trampoline, public_iterator, flags);
  if (status != SQLITE_OK) {
...
int cpkt_sqlite_changeset_invert(const cpkt_sqlite_changeset *input,
  ...
  int status;
  if (out != NULL) *out = NULL;
  if (input == NULL || input->data == NULL || out == NULL) return CPKT_SQLITE_MISUSE;
  data = NULL;
  byte_count = 0;
  status = sqlite3changeset_invert(input->byte_count, input->data,
      &byte_count, &data);
  if (status != SQLITE_OK) return status;
  return cpkt_sqlite_changeset_new_owned(data, byte_count, out);
}
...
int cpkt_sqlite_changeset_concat(const cpkt_sqlite_changeset *left,
  ...
  int status;
  if (out != NULL) *out = NULL;
  if (left == NULL || left->data == NULL || right == NULL || right->data == NULL ||
      out == NULL) return CPKT_SQLITE_MISUSE;
  data = NULL;
  byte_count = 0;
  status = sqlite3changeset_concat(left->byte_count, (void *) left->data,
      right->byte_count, (void *) right->data, &byte_count, &data);
  if (status != SQLITE_OK) return status;
  return cpkt_sqlite_changeset_new_owned(data, byte_count, out);
}
...
int cpkt_sqlite_changeset_invert_stream(cpkt_sqlite_stream_input_callback input,
  ...
  if (input == NULL || output == NULL) return CPKT_SQLITE_MISUSE;
  ...
  return sqlite3changeset_invert_strm(cpkt_sqlite_stream_input_trampoline,
      &public_input, cpkt_sqlite_stream_output_trampoline, &public_output);
}
...
int cpkt_sqlite_changeset_concat_stream(cpkt_sqlite_stream_input_callback left_input,
  ...
  public_output.callback = output;
  public_output.user_context = output_context;
  return sqlite3changeset_concat_strm(cpkt_sqlite_stream_input_trampoline,
      &public_left, cpkt_sqlite_stream_input_trampoline, &public_right,
...
int cpkt_sqlite_changeset_apply(cpkt_sqlite *database,
  ...
  cpkt_sqlite_changeset_apply_context apply_context;
  if (database == NULL || cpkt_sqlite_native(database) == NULL || changeset == NULL ||
      changeset->data == NULL) return CPKT_SQLITE_MISUSE;
  ...
  return sqlite3changeset_apply(cpkt_sqlite_native(database), changeset->byte_count,
      (void *) changeset->data,
...
int cpkt_sqlite_changeset_apply_stream(cpkt_sqlite *database,
  ...
  cpkt_sqlite_changeset_apply_context apply_context;
  if (database == NULL || cpkt_sqlite_native(database) == NULL || input == NULL) {
    return CPKT_SQLITE_MISUSE;
  }
  ...
  apply_context.conflict = conflict;
  apply_context.user_context = context;
  return sqlite3changeset_apply_strm(cpkt_sqlite_native(database),
      cpkt_sqlite_stream_input_trampoline, &stream_context,
      filter == NULL ? NULL : cpkt_sqlite_changeset_filter_trampoline,
      conflict == NULL ? NULL : cpkt_sqlite_changeset_conflict_trampoline,
      &apply_context);
}

static int cpkt_sqlite_changeset_rebase_result(void *data, int byte_count,
...
int cpkt_sqlite_changeset_apply_ex(cpkt_sqlite *database,
  ...
  int rebase_size;
  int status;
  if (rebase_out != NULL) *rebase_out = NULL;
  if (database == NULL || cpkt_sqlite_native(database) == NULL || changeset == NULL ||
      changeset->data == NULL) return CPKT_SQLITE_MISUSE;
  apply_context.filter = filter;
  apply_context.iterator_filter = NULL;
  ...
  rebase_data = NULL;
  rebase_size = 0;
  status = sqlite3changeset_apply_v2(cpkt_sqlite_native(database),
      changeset->byte_count, (void *) changeset->data,
      filter == NULL ? NULL : cpkt_sqlite_changeset_filter_trampoline,
      conflict == NULL ? NULL : cpkt_sqlite_changeset_conflict_trampoline,
      &apply_context, &rebase_data, &rebase_size, flags);
  if (status != SQLITE_OK) {
    if (rebase_data != NULL) sqlite3_free(rebase_data);
    return status;
  }
  return cpkt_sqlite_changeset_rebase_result(rebase_data, rebase_size, rebase_out);
}
...
int cpkt_sqlite_changeset_apply_stream_ex(cpkt_sqlite *database,
  ...
  rebase_data = NULL;
  rebase_size = 0;
  status = sqlite3changeset_apply_v2_strm(cpkt_sqlite_native(database),
      cpkt_sqlite_stream_input_trampoline, &stream_context,
      filter == NULL ? NULL : cpkt_sqlite_changeset_filter_trampoline,
      conflict == NULL ? NULL : cpkt_sqlite_changeset_conflict_trampoline,
      &apply_context, &rebase_data, &rebase_size, flags);
...
int cpkt_sqlite_changeset_apply_v3(cpkt_sqlite *database,
  ...
  rebase_data = NULL;
  rebase_size = 0;
  status = sqlite3changeset_apply_v3(cpkt_sqlite_native(database),
      changeset->byte_count, (void *) changeset->data,
...
int cpkt_sqlite_changeset_apply_v3_stream(cpkt_sqlite *database,
  ...
  rebase_data = NULL;
  rebase_size = 0;
  status = sqlite3changeset_apply_v3_strm(cpkt_sqlite_native(database),
      cpkt_sqlite_stream_input_trampoline, &stream_context,
      filter == NULL ? NULL : cpkt_sqlite_changeset_iterator_filter_trampoline,
      conflict == NULL ? NULL : cpkt_sqlite_changeset_conflict_trampoline,
      &apply_context, &rebase_data, &rebase_size, flags);
...
static int cpkt_sqlite_rebaser_configure(cpkt_sqlite_rebaser *self,
  ...
  if (cpkt_sqlite_native_rebaser(self) == NULL || data == NULL) return CPKT_SQLITE_MISUSE;
...
static int cpkt_sqlite_rebaser_rebase(cpkt_sqlite_rebaser *self,
    const void *data, int byte_count, cpkt_sqlite_changeset **out) {
  void *result;
  int result_size;
  int status;
  if (out != NULL) *out = NULL;
...
static int cpkt_sqlite_rebaser_rebase_stream(cpkt_sqlite_rebaser *self,
    cpkt_sqlite_stream_input_callback input, void *input_context,
  ...
  if (cpkt_sqlite_native_rebaser(self) == NULL || input == NULL || output == NULL) {
    return CPKT_SQLITE_MISUSE;
  }
  ...
  return sqlite3rebaser_rebase_strm(cpkt_sqlite_native_rebaser(self),
      cpkt_sqlite_stream_input_trampoline, &input_state,
      cpkt_sqlite_stream_output_trampoline, &output_state);
}

static void cpkt_sqlite_rebaser_close(cpkt_sqlite_rebaser *self) {
  if (self == NULL) return;
  if (cpkt_sqlite_native_rebaser(self) != NULL) sqlite3rebaser_delete(cpkt_sqlite_native_rebaser(self));
  self->rebaser = NULL;
  free(self);
}
...
int cpkt_sqlite_rebaser_new(cpkt_sqlite_rebaser **out) {
  ...
  int status;
  if (out != NULL) *out = NULL;
  if (out == NULL) return CPKT_SQLITE_MISUSE;
  native_rebaser = NULL;
...
static int cpkt_sqlite_changegroup_config(cpkt_sqlite_changegroup *self,
  ...
  return sqlite3changegroup_config(cpkt_sqlite_native_changegroup(self), operation, value);
}

static int cpkt_sqlite_changegroup_schema(cpkt_sqlite_changegroup *self,
    cpkt_sqlite *database, const char *schema_name) {
  if (cpkt_sqlite_native_changegroup(self) == NULL || cpkt_sqlite_native(database) == NULL) {
    return CPKT_SQLITE_MISUSE;
  }
...
static int cpkt_sqlite_changegroup_add_stream(cpkt_sqlite_changegroup *self,
  ...
  return sqlite3changegroup_add_strm(cpkt_sqlite_native_changegroup(self),
      cpkt_sqlite_stream_input_trampoline, &input_context);
}

static int cpkt_sqlite_changegroup_add_change(cpkt_sqlite_changegroup *self,
    cpkt_sqlite_changeset_iterator *iterator) {
  if (cpkt_sqlite_native_changegroup(self) == NULL ||
      cpkt_sqlite_native_iterator(iterator) == NULL) return CPKT_SQLITE_MISUSE;
...
static int cpkt_sqlite_changegroup_output_stream(cpkt_sqlite_changegroup *self,
    cpkt_sqlite_stream_output_callback output, void *context) {
  cpkt_sqlite_stream_output_context output_context;
...
static int cpkt_sqlite_changegroup_change_begin(cpkt_sqlite_changegroup *self,
    int operation, const char *table_name, int indirect, char **error_out) {
  if (error_out != NULL) *error_out = NULL;
  ...
  return sqlite3changegroup_change_begin(cpkt_sqlite_native_changegroup(self),
      operation, table_name, indirect, error_out);
}

static int cpkt_sqlite_changegroup_change_i64(cpkt_sqlite_changegroup *self,
    int is_new, int column, cpkt_sqlite_i64 value) {
  if (cpkt_sqlite_native_changegroup(self) == NULL) return CPKT_SQLITE_MISUSE;
  return sqlite3changegroup_change_int64(cpkt_sqlite_native_changegroup(self),
      is_new, column, cpkt_sqlite_native_i64(value));
}
...
static int cpkt_sqlite_changegroup_change_double(cpkt_sqlite_changegroup *self,
    int is_new, int column, double value) {
  if (cpkt_sqlite_native_changegroup(self) == NULL) return CPKT_SQLITE_MISUSE;
...
static int cpkt_sqlite_changegroup_change_text(cpkt_sqlite_changegroup *self,
    int is_new, int column, const char *value, int byte_count) {
  if (cpkt_sqlite_native_changegroup(self) == NULL || value == NULL) {
    return CPKT_SQLITE_MISUSE;
  }
...
static int cpkt_sqlite_changegroup_change_blob(cpkt_sqlite_changegroup *self,
    int is_new, int column, const void *value, int byte_count) {
  if (cpkt_sqlite_native_changegroup(self) == NULL || value == NULL) {
    return CPKT_SQLITE_MISUSE;
  }
...
int cpkt_sqlite_changegroup_new(cpkt_sqlite_changegroup **out) {
  ...
  if (status != SQLITE_OK) return status;
  public_changegroup = (cpkt_sqlite_changegroup *) calloc(1,
      sizeof(*public_changegroup));
...
static void cpkt_sqlite_string_append(cpkt_sqlite_string *self, const char *text,
    int byte_count) {
...
static void cpkt_sqlite_string_append_all(cpkt_sqlite_string *self, const char *text) {
  if (cpkt_sqlite_native_string(self) != NULL) sqlite3_str_appendall(cpkt_sqlite_native_string(self), text);
}
...
static void cpkt_sqlite_string_reset(cpkt_sqlite_string *self) {
  if (cpkt_sqlite_native_string(self) != NULL) sqlite3_str_reset(cpkt_sqlite_native_string(self));
}

static void cpkt_sqlite_string_truncate(cpkt_sqlite_string *self, int byte_count) {
  if (cpkt_sqlite_native_string(self) != NULL) sqlite3_str_truncate(cpkt_sqlite_native_string(self), byte_count);
}

static int cpkt_sqlite_string_error_code(const cpkt_sqlite_string *self) {
  return cpkt_sqlite_native_string(self) == NULL ? CPKT_SQLITE_MISUSE :
      sqlite3_str_errcode(cpkt_sqlite_native_string(self));
}

static int cpkt_sqlite_string_length(const cpkt_sqlite_string *self) {
  return cpkt_sqlite_native_string(self) == NULL ? 0 : sqlite3_str_length(cpkt_sqlite_native_string(self));
}

static const char *cpkt_sqlite_string_value(const cpkt_sqlite_string *self) {
  return cpkt_sqlite_native_string(self) == NULL ? NULL : sqlite3_str_value(cpkt_sqlite_native_string(self));
}

static char *cpkt_sqlite_string_finish(cpkt_sqlite_string *self) {
  char *value;
  if (self == NULL || cpkt_sqlite_native_string(self) == NULL) return NULL;
...
static void cpkt_sqlite_string_close(cpkt_sqlite_string *self) {
  if (self == NULL) return;
  if (cpkt_sqlite_native_string(self) != NULL) sqlite3_str_free(cpkt_sqlite_native_string(self));
  self->string = NULL;
  free(self);
}
...
cpkt_sqlite_string *cpkt_sqlite_string_new(cpkt_sqlite *database) {
  ...
  native_string = sqlite3_str_new(database == NULL ? NULL : cpkt_sqlite_native(database));
  if (native_string == NULL) return NULL;
