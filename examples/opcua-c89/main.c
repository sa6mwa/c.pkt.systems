...
static int browse_for_name(const cpkt_opcua_browse_entry *entry, void *user) {
  ...
  if (entry->browse_name != NULL &&
      strcmp(entry->browse_name, state->expected_name) == 0) {
    state->matched = 1;
  }
  return 0;
}
...
int main(void) {
  ...
  failed = expect_ok(
      cpkt_opcua_server_add_variable(server, value_node, "exampleValue",
                                     "Example Value", &value, &status),
      "add variable");

  if (!failed) {
    failed = expect_ok(cpkt_opcua_server_add_object(
                           server, object_node, objects_folder, "exampleObject",
  ...
  if (!failed) {
    ...
    failed = expect_ok(cpkt_opcua_server_add_variable_under(
                           server, child_node, object_node, "exampleChild",
                           "Example Child", &value, &status),
                       "add child variable");
  }

  if (!failed) {
    failed = expect_ok(cpkt_opcua_server_add_method(
                           server, method_node, object_node, "exampleMultiply",
                           "Example Multiply", method_input_types, 1,
  ...
  if (!failed) {
    ...
    failed =
        expect_ok(cpkt_opcua_server_write(server, value_node, &value, &status),
                  "write variable");
  }

  if (!failed) {
    failed = expect_ok(cpkt_opcua_server_read(server, value_node, &out, NULL, 0,
                                              NULL, &status),
                       "read variable");
    if (!failed &&
        (out.type != CPKT_OPCUA_VALUE_INTEGER || out.integer_value != 42)) {
  ...
  if (!failed) {
    ...
    state.matched = 0;
    failed =
        expect_ok(cpkt_opcua_server_browse_children(
                      server, objects_folder, browse_for_name, &state, &status),
                  "browse objects");
  ...
  if (!failed) {
    ...
    failed =
        expect_ok(cpkt_opcua_server_browse_children(
                      server, object_node, browse_for_name, &state, &status),
                  "browse object");
    if (!failed && !state.matched) {
  ...
  if (!failed) {
    failed = expect_ok(cpkt_opcua_server_endpoint_url(
                           server, endpoint, sizeof(endpoint), &required),
                       "endpoint url");
