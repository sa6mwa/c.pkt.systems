...
static const unsigned char cpkt_c89_facade_guid_node_id[16] = {
    ...
    0x0f, 0xed, 0xcb, 0xa9, 0x87, 0x65, 0x43, 0x21};
static const unsigned char cpkt_c89_facade_byte_node_id[4] = {0xca, 0xfe, 0xba, 0xbe};
static const unsigned char cpkt_c89_native_bytes_value[4] = {0x01, 0x23, 0x45, 0x67};
static const unsigned char cpkt_c89_native_bytes_updated[3] = {0x10, 0x20, 0x30};
static const unsigned char cpkt_c89_facade_bytes_value[5] = {0x89, 0xab, 0xcd, 0xef, 0x01};

static void cpkt_c89_async_value_callback(
...
static void cpkt_c89_async_status_callback(
    cpkt_opcua_request_id request_id,
    cpkt_opcua_result result,
    cpkt_opcua_status status,
    void *user) {
...
static void cpkt_c89_async_browse_done_callback(
    cpkt_opcua_request_id request_id,
    cpkt_opcua_result result,
    cpkt_opcua_status status,
    void *user) {
...
static int cpkt_c89_async_browse_entry_callback(const cpkt_opcua_browse_entry *entry, void *user) {
  struct cpkt_c89_async_seen *seen;
...
static void cpkt_c89_async_call_callback(
    cpkt_opcua_request_id request_id,
    cpkt_opcua_result result,
...
static void cpkt_c89_async_node_callback(
    cpkt_opcua_request_id request_id,
    cpkt_opcua_result result,
...
static void cpkt_c89_wait_for_async(cpkt_opcua_client *client, struct cpkt_c89_async_seen *seen) {
  cpkt_opcua_status status;
  int attempt;
...
static int cpkt_c89_browse_callback(const cpkt_opcua_browse_entry *entry, void *user) {
  ...
  if (entry->browse_name != NULL && strcmp(entry->browse_name, "nativeObject") == 0) {
  ...
  if (entry->browse_name != NULL && strcmp(entry->browse_name, "nativeObjectChild") == 0) {
    if (entry->target_node_id.identifier_type != CPKT_OPCUA_NODE_ID_NUMERIC ||
  ...
  if (entry->browse_name != NULL && strcmp(entry->browse_name, "nativeInteger") == 0) {
    if (entry->target_node_id.identifier_type != CPKT_OPCUA_NODE_ID_NUMERIC ||
  ...
  if (entry->browse_name != NULL && strcmp(entry->browse_name, "nativeAsyncInteger") == 0) {
    seen->int_seen = 1;
  }
  if (entry->browse_name != NULL && strcmp(entry->browse_name, "clientAddedObject") == 0) {
    if (entry->target_node_id.identifier_type != CPKT_OPCUA_NODE_ID_NUMERIC ||
  ...
  if (entry->browse_name != NULL && strcmp(entry->browse_name, "clientAddedVariable") == 0) {
    if (entry->target_node_id.identifier_type != CPKT_OPCUA_NODE_ID_NUMERIC ||
  ...
  if (entry->browse_name != NULL && strcmp(entry->browse_name, "clientAddedChildVariable") == 0) {
    if (entry->target_node_id.identifier_type != CPKT_OPCUA_NODE_ID_NUMERIC ||
        entry->target_node_id.numeric != CPKT_OPCUA_CLIENT_ADDED_CHILD_VARIABLE_ID) {
      return 1;
    }
    seen->client_added_child_variable_seen = 1;
  }
  if (entry->browse_name != NULL && strcmp(entry->browse_name, "facadeObject") == 0) {
    if (entry->target_node_id.identifier_type != CPKT_OPCUA_NODE_ID_NUMERIC ||
  ...
  if (entry->browse_name != NULL && strcmp(entry->browse_name, "facadeObjectChild") == 0) {
    if (entry->target_node_id.identifier_type != CPKT_OPCUA_NODE_ID_NUMERIC ||
  ...
  if (entry->browse_name != NULL && strcmp(entry->browse_name, "facadeInteger") == 0) {
    if (entry->target_node_id.identifier_type != CPKT_OPCUA_NODE_ID_NUMERIC ||
...
static cpkt_opcua_status cpkt_c89_native_client_seen(void *native_client, void *user) {
...
static cpkt_opcua_status cpkt_c89_native_server_seen(void *native_server, void *user) {
...
static cpkt_opcua_status cpkt_c89_native_config_seen(void *native_config, void *user) {
...
static int cpkt_c89_history_seen_callback(
  ...
  if (data_value->has_value == 0 || data_value->value.type != CPKT_OPCUA_VALUE_INTEGER) {
    return 1;
  }
...
static int cpkt_c89_native_variant_seen(const void *native_variant, void *user) {
...
static int cpkt_c89_native_data_value_seen(const void *native_data_value, void *user) {
...
static void cpkt_c89_subscription_callback(
  ...
  if (subscription_id == 0 || monitored_item_id == 0 || value == NULL || user == NULL) {
    return;
  }
  ...
  if (status == 0 && value->type == CPKT_OPCUA_VALUE_INTEGER) {
    ...
    if (value->bytes_length == sizeof(cpkt_c89_native_bytes_updated) &&
        memcmp(value->bytes_value, cpkt_c89_native_bytes_updated, value->bytes_length) == 0) {
    ...
    if (value->qualified_name_namespace_index == CPKT_OPCUA_TEST_NS &&
        ...
        memcmp(value->qualified_name, "monitorQualified", value->qualified_name_length) == 0) {
    ...
    if (value->localized_text_locale_length == strlen("en-US") &&
        memcmp(value->localized_text_locale, "en-US", value->localized_text_locale_length) == 0 &&
        value->localized_text_length == strlen("monitor text") &&
        memcmp(value->localized_text, "monitor text", value->localized_text_length) == 0) {
      seen->saw_expected_localized_text = 1;
    }
  }
}

static void cpkt_c89_event_callback(
    cpkt_opcua_subscription_id subscription_id,
    cpkt_opcua_monitored_item_id monitored_item_id,
  ...
  if (subscription_id == 0 || monitored_item_id == 0 || event == NULL || user == NULL || status != 0) {
    return;
  }
  ...
  if (((event->severity == 321UL && strcmp(seen->message, "native method event") == 0) ||
       (event->severity == 654UL && strcmp(seen->message, "facade server event") == 0)) &&
      event->event_id_length != 0) {
    seen->saw_expected = 1;
  }
}

static int cpkt_c89_event_field_matches(const cpkt_opcua_event_field *field, const char *name) {
  size_t name_length;
  size_t offset;
  ...
  if (field->name_length > name_length && field->name[field->name_length - name_length - 1] == '/') {
...
static void cpkt_c89_event_fields_callback(
    cpkt_opcua_subscription_id subscription_id,
    cpkt_opcua_monitored_item_id monitored_item_id,
  ...
  (void)status;
  if (subscription_id == 0 || monitored_item_id == 0 || fields == NULL || user == NULL) {
    return;
  }
  seen = (struct cpkt_c89_event_seen *)user;
  saw_message = 0;
  ...
  for (i = 0; i < field_count; ++i) {
    if (cpkt_c89_event_field_matches(&fields[i], "Message") &&
      ...
      if (copy_length != 0 && fields[i].value.localized_text != NULL) {
        memcpy(seen->field_message, fields[i].value.localized_text, copy_length);
      }
  ...
  if (saw_message && saw_severity &&
      ((seen->field_severity == 321L && strcmp(seen->field_message, "native method event") == 0) ||
       (seen->field_severity == 654L && strcmp(seen->field_message, "facade server event") == 0))) {
    seen->saw_expected_fields = 1;
  }
}

static int cpkt_c89_string_array_callback(size_t index, const char *data, size_t length, void *user) {
  ...
  if ((index == 0 && length == strlen("alpha") && memcmp(data, "alpha", length) == 0) ||
      (index == 1 && length == strlen("beta") && memcmp(data, "beta", length) == 0)) {
    ++seen->matched;
  }
  ++seen->count;
  return 0;
...
static int cpkt_c89_byte_string_array_callback(
    size_t index,
  ...
  if ((index == 0 && length == 2 && data[0] == 0x10U && data[1] == 0x11U) ||
      (index == 1 && length == 3 && data[0] == 0x20U && data[1] == 0x21U && data[2] == 0x22U)) {
    ++seen->matched;
  }
  ++seen->count;
  return 0;
...
static int cpkt_c89_qualified_name_array_callback(
  ...
  if (namespace_index == CPKT_OPCUA_TEST_NS &&
      ((name_length == strlen("alphaName") && memcmp(name, "alphaName", name_length) == 0) ||
       (name_length == strlen("betaName") && memcmp(name, "betaName", name_length) == 0))) {
    ++seen->matched;
  }
  ++seen->count;
  return 0;
...
static int cpkt_c89_localized_text_array_callback(
    size_t index,
    const char *locale,
  ...
  (void)index;
  if ((locale_length == strlen("en-US") && memcmp(locale, "en-US", locale_length) == 0 &&
       text_length == strlen("alpha text") && memcmp(text, "alpha text", text_length) == 0) ||
      (locale_length == strlen("sv-SE") && memcmp(locale, "sv-SE", locale_length) == 0 &&
       text_length == strlen("beta text") && memcmp(text, "beta text", text_length) == 0)) {
    ++seen->matched;
  }
  ++seen->count;
  return 0;
...
static cpkt_opcua_result cpkt_c89_multiply_method(
...
static cpkt_opcua_result cpkt_c89_echo_string_method(
    const cpkt_opcua_value *inputs,
    size_t input_count,
    cpkt_opcua_value *output,
    void *user) {
  ...
  cpkt_opcua_value_string(output, inputs[0].string_value, inputs[0].string_length);
  return CPKT_OPCUA_OK;
}

static cpkt_opcua_result cpkt_c89_echo_bytes_method(
    const cpkt_opcua_value *inputs,
    size_t input_count,
    cpkt_opcua_value *output,
    void *user) {
  ...
  cpkt_opcua_value_byte_string(output, inputs[0].bytes_value, inputs[0].bytes_length);
  return CPKT_OPCUA_OK;
}

static cpkt_opcua_result cpkt_c89_echo_qualified_name_method(
    const cpkt_opcua_value *inputs,
    size_t input_count,
    cpkt_opcua_value *output,
    void *user) {
  ...
  cpkt_opcua_value_qualified_name(
      output,
      inputs[0].qualified_name_namespace_index,
      inputs[0].qualified_name,
      inputs[0].qualified_name_length);
  return CPKT_OPCUA_OK;
}

static cpkt_opcua_result cpkt_c89_echo_localized_text_method(
    const cpkt_opcua_value *inputs,
    size_t input_count,
    cpkt_opcua_value *output,
    void *user) {
  ...
  cpkt_opcua_value_localized_text(
      output,
      inputs[0].localized_text_locale,
...
static cpkt_opcua_result cpkt_c89_empty_method(
    const cpkt_opcua_value *inputs,
    size_t input_count,
    cpkt_opcua_value *output,
    void *user) {
...
static cpkt_opcua_result cpkt_c89_multi_method(
    const cpkt_opcua_value *inputs,
    size_t input_count,
    ...
    void *user) {
  (void)user;
  if (inputs == NULL || outputs == NULL || input_count != 1 || output_count != 2 ||
      inputs[0].type != CPKT_OPCUA_VALUE_INTEGER) {
    return CPKT_OPCUA_ERR_ARG;
  }
...
static cpkt_opcua_status cpkt_c89_login_callback(
    const char *username,
...
int cpkt_opcua_c89_client_exercise_native_server(const char *endpoint) {
  ...
  CPKT_C89_CHECK(cpkt_opcua_client_connect(client, endpoint, &status) == CPKT_OPCUA_OK);

  CPKT_C89_CHECK(
      cpkt_opcua_client_get_endpoint_count(client, endpoint, &endpoint_count, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(endpoint_count > 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_get_endpoint_url(
          client,
          endpoint,
          0,
          endpoint_buffer,
  ...
  CPKT_C89_CHECK(strncmp(endpoint_buffer, "opc.tcp://", strlen("opc.tcp://")) == 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_get_endpoint_url(
          client,
          endpoint,
          ...
          &status) == CPKT_OPCUA_ERR_RANGE);
  CPKT_C89_CHECK(
      cpkt_opcua_client_find_server_count(client, endpoint, &server_count, &status) ==
      CPKT_OPCUA_OK);
  ...
  int_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_INT_ID);
  async_int_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, 7799);
  history_value_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_HISTORY_VALUE_ID);
  bool_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_BOOL_ID);
  string_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_STRING_ID);
  native_object_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_OBJECT_ID);
  native_child_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_CHILD_ID);
  native_method_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_METHOD_ID);
  native_multi_method_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_MULTI_METHOD_ID);
  native_reference_type_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_REFERENCE_TYPE_ID);
  native_view_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_VIEW_ID);
  client_added_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_CLIENT_ADDED_OBJECT_ID);
  client_added_variable_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_CLIENT_ADDED_VARIABLE_ID);
  client_added_child_variable_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_CLIENT_ADDED_CHILD_VARIABLE_ID);
  client_added_object_type_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_CLIENT_ADDED_OBJECT_TYPE_ID);
  client_added_variable_type_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_CLIENT_ADDED_VARIABLE_TYPE_ID);
  client_added_array_variable_type_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_CLIENT_ADDED_ARRAY_VARIABLE_TYPE_ID);
  client_added_reference_type_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_CLIENT_ADDED_REFERENCE_TYPE_ID);
  client_added_data_type_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_CLIENT_ADDED_DATA_TYPE_ID);
  client_added_view_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_CLIENT_ADDED_VIEW_ID);
  bytes_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_BYTES_ID);
  array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_ARRAY_ID);
  bool_array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_BOOL_ARRAY_ID);
  double_array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_DOUBLE_ARRAY_ID);
  string_array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_STRING_ARRAY_ID);
  bytes_array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_BYTES_ARRAY_ID);
  guid_value_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_GUID_VALUE_ID);
  guid_array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_GUID_ARRAY_ID);
  status_value_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_STATUS_VALUE_ID);
  status_array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_STATUS_ARRAY_ID);
  uint64_value_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_UINT64_VALUE_ID);
  uint64_array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_UINT64_ARRAY_ID);
  datetime_value_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_DATETIME_VALUE_ID);
  datetime_array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_DATETIME_ARRAY_ID);
  qualified_name_value_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_QUALIFIED_NAME_VALUE_ID);
  qualified_name_array_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_QUALIFIED_NAME_ARRAY_ID);
  localized_text_value_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_LOCALIZED_TEXT_VALUE_ID);
  localized_text_array_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_LOCALIZED_TEXT_ARRAY_ID);
  native_guid_node = cpkt_opcua_node_id_guid(CPKT_OPCUA_TEST_NS, cpkt_c89_native_guid_node_id);
  native_byte_node = cpkt_opcua_node_id_byte_string(
      CPKT_OPCUA_TEST_NS,
      cpkt_c89_native_byte_node_id,
      sizeof(cpkt_c89_native_byte_node_id));
  constructed_string_node =
      cpkt_opcua_node_id_string(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_NATIVE_STRING_NODE_ID);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_call_method_async(
          client,
          native_object_node,
          native_multi_method_node,
          inputs,
          1,
          2,
          cpkt_c89_async_call_callback,
          &async_seen,
          &request_id,
          outputs,
          NULL,
  ...
  CPKT_C89_CHECK(async_seen.double_value > 6.4 && async_seen.double_value < 6.6);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_browse_children_async(
          ...
          cpkt_c89_async_browse_done_callback,
          &async_seen,
          &request_id,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(request_id != 0);
  ...
  native_seen = 0;
  CPKT_C89_CHECK(
      cpkt_opcua_client_native(client, cpkt_c89_native_client_seen, &native_seen) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(native_seen == 1);
  native_seen = 0;
  CPKT_C89_CHECK(
      cpkt_opcua_client_async_native(client, cpkt_c89_native_client_seen, &native_seen) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(native_seen == 1);
  native_seen = 0;
  CPKT_C89_CHECK(
      cpkt_opcua_client_history_native(client, cpkt_c89_native_client_seen, &native_seen) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(native_seen == 1);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_native_data_value(
          client,
          int_node,
          cpkt_c89_native_data_value_seen,
          &native_data_value_seen,
          &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_history_read_raw(
          ...
          0,
          16,
          cpkt_c89_history_seen_callback,
          &history_seen,
          NULL,
          0,
          NULL,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_get_namespace_index(
          client,
          CPKT_OPCUA_NATIVE_NAMESPACE_URI,
          &namespace_index,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(namespace_index != 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_get_namespace_uri(
          client,
          namespace_index,
          namespace_buffer,
          4,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_get_namespace_uri(
          ...
          namespace_index,
          namespace_buffer,
          sizeof(namespace_buffer),
          &required,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(strcmp(namespace_buffer, CPKT_OPCUA_NATIVE_NAMESPACE_URI) == 0);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_translate_browse_path(
          ...
          &required,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(cpkt_opcua_node_id_equal(translated_node, native_child_node) == 1);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_translate_browse_path(
          ...
          &required,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(cpkt_opcua_node_id_equal(translated_node, constructed_string_node) == 1);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_node_class(client, native_object_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_node_id(
          ...
          &required,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(cpkt_opcua_node_id_equal(parsed_node, native_object_node) == 1);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_node_id(
          client,
          constructed_string_node,
  ...
  CPKT_C89_CHECK(strcmp(parsed_node.string, CPKT_OPCUA_NATIVE_STRING_NODE_ID) == 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_node_class(client, native_child_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_VARIABLE);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_node_class(client, native_method_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_METHOD);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_data_type(client, int_node, &parsed_node, &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_value_rank(client, int_node, &value_rank, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(value_rank == -1);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_access_level(client, int_node, &access_level, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK((access_level & 1UL) != 0);
  CPKT_C89_CHECK((access_level & 2UL) != 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_write_mask(client, int_node, &write_mask, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK((write_mask & 1UL) != 0);
  original_write_mask = write_mask;
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_user_write_mask(client, int_node, &write_mask, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK((write_mask & 1UL) != 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_write_write_mask(client, int_node, original_write_mask, &status) ==
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_write_write_mask(client, int_node, 1UL, &status) == CPKT_OPCUA_ERR_UPSTREAM);
  CPKT_C89_CHECK(status != 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_write_mask(client, int_node, &write_mask, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(write_mask == original_write_mask);
  CPKT_C89_CHECK(
      cpkt_opcua_client_write_data_type(
          ...
          &status) == CPKT_OPCUA_ERR_UPSTREAM);
  CPKT_C89_CHECK(status != 0);
  CPKT_C89_CHECK(cpkt_opcua_client_write_value_rank(client, int_node, -1, &status) == CPKT_OPCUA_ERR_UPSTREAM);
  CPKT_C89_CHECK(status != 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_array_dimensions(client, array_node, dimensions, 0, &dimension_count, &status) ==
      CPKT_OPCUA_ERR_RANGE);
  CPKT_C89_CHECK(dimension_count == 1);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_array_dimensions(client, array_node, dimensions, 1, &dimension_count, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(dimension_count == 1);
  CPKT_C89_CHECK(dimensions[0] == 3UL);
  dimensions[0] = 4UL;
  CPKT_C89_CHECK(cpkt_opcua_client_write_array_dimensions(client, array_node, dimensions, 1, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_array_dimensions(client, array_node, dimensions, 1, &dimension_count, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(dimension_count == 1);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_integer_array(client, array_node, integer_array_values, 2, &array_count, &status) ==
      CPKT_OPCUA_ERR_RANGE);
  CPKT_C89_CHECK(array_count == 3);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_integer_array(client, array_node, integer_array_values, 3, &array_count, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(array_count == 3);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_integer_array_range(
          client,
          array_node,
          "1:2",
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_write_index_range(client, array_node, "1:2", &value, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_integer_array(client, array_node, integer_array_values, 3, &array_count, &status) ==
      CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_boolean_array(
          client,
          bool_array_node,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_double_array(client, double_array_node, double_array_values, 2, &array_count, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(array_count == 2);
  CPKT_C89_CHECK(double_array_values[0] > 1.24 && double_array_values[0] < 1.26);
  CPKT_C89_CHECK(double_array_values[1] > 2.49 && double_array_values[1] < 2.51);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_double_array_range(
          ...
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(array_count == 1);
  CPKT_C89_CHECK(double_array_values[0] > 2.49 && double_array_values[0] < 2.51);
  string_array_seen.count = 0;
  string_array_seen.matched = 0;
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_string_array(
          client,
          string_array_node,
          cpkt_c89_string_array_callback,
          &string_array_seen,
          &array_count,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(array_count == 2);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_access_level_ex(client, int_node, &access_level, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK((access_level & 1UL) != 0);
  CPKT_C89_CHECK((access_level & 2UL) != 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_write_access_level_ex(client, int_node, 1UL, &status) == CPKT_OPCUA_ERR_UPSTREAM);
  CPKT_C89_CHECK(status != 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_access_level_ex(client, int_node, &access_level, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK((access_level & 1UL) != 0);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_write_minimum_sampling_interval(client, int_node, 12.5, &status) ==
      CPKT_OPCUA_ERR_UPSTREAM);
  CPKT_C89_CHECK(status != 0);
  CPKT_C89_CHECK(cpkt_opcua_client_write_access_level(client, int_node, 1UL, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_access_level(client, int_node, &access_level, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK((access_level & 1UL) != 0);
  CPKT_C89_CHECK((access_level & 2UL) == 0);
  CPKT_C89_CHECK(cpkt_opcua_client_write_access_level(client, int_node, 3UL, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_user_access_level(client, int_node, &access_level, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK((access_level & 1UL) != 0);
  CPKT_C89_CHECK((access_level & 2UL) != 0);
  CPKT_C89_CHECK(cpkt_opcua_client_read_historizing(client, int_node, &historizing, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(historizing == 0);
  CPKT_C89_CHECK(cpkt_opcua_client_write_historizing(client, int_node, 0, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(cpkt_opcua_client_read_historizing(client, int_node, &historizing, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(historizing == 0);
  CPKT_C89_CHECK(cpkt_opcua_client_write_historizing(client, int_node, 1, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_executable(client, native_method_node, &executable, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(executable == 1);
  CPKT_C89_CHECK(cpkt_opcua_client_write_executable(client, native_method_node, 0, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_executable(client, native_method_node, &executable, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(executable == 0);
  CPKT_C89_CHECK(cpkt_opcua_client_write_executable(client, native_method_node, 1, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_user_executable(client, native_method_node, &executable, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(executable == 1);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_event_notifier(client, native_object_node, &event_notifier, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(event_notifier == 1UL);
  CPKT_C89_CHECK(
      cpkt_opcua_client_write_event_notifier(client, native_object_node, 1UL, &status) ==
      CPKT_OPCUA_ERR_UPSTREAM);
  CPKT_C89_CHECK(status != 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_event_notifier(client, native_object_node, &event_notifier, &status) ==
  ...
  CPKT_C89_CHECK(event_notifier == 1UL);
  CPKT_C89_CHECK(
      cpkt_opcua_client_write_event_notifier(client, native_object_node, 0UL, &status) ==
      CPKT_OPCUA_ERR_UPSTREAM);
  CPKT_C89_CHECK(status != 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_is_abstract(client, native_reference_type_node, &boolean_attribute, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(boolean_attribute == 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_write_is_abstract(client, native_reference_type_node, 1, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_is_abstract(client, native_reference_type_node, &boolean_attribute, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(boolean_attribute == 1);
  CPKT_C89_CHECK(
      cpkt_opcua_client_write_is_abstract(client, native_reference_type_node, 0, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_symmetric(client, native_reference_type_node, &boolean_attribute, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(boolean_attribute == 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_inverse_name(
          client,
          native_reference_type_node,
          name_buffer,
          sizeof(name_buffer),
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_write_inverse_name(
          client,
          native_reference_type_node,
          "Native Related Back",
          &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_write_inverse_name(
          ...
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_contains_no_loops(client, native_view_node, &boolean_attribute, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(boolean_attribute == 1);
  CPKT_C89_CHECK(
  ...
  CPKT_C89_CHECK(argument_data_type.identifier_type == CPKT_OPCUA_NODE_ID_NUMERIC);
  CPKT_C89_CHECK(argument_data_type.numeric == CPKT_OPCUA_STANDARD_DATA_TYPE_INTEGER);
  ...
  CPKT_C89_CHECK(argument_data_type.namespace_index == 0);
  CPKT_C89_CHECK(argument_data_type.identifier_type == CPKT_OPCUA_NODE_ID_NUMERIC);
  CPKT_C89_CHECK(argument_data_type.numeric == CPKT_OPCUA_STANDARD_DATA_TYPE_DOUBLE);
  CPKT_C89_CHECK(argument_value_rank == -1);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_method_argument(
          client,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_browse_name(
          client,
          native_object_node,
          &namespace_index,
          name_buffer,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_display_name(
          client,
          native_object_node,
          name_buffer,
          4,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_write_display_name(
          client,
          native_object_node,
          "Native Object Updated",
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_display_name(
          client,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_description(
          client,
          native_object_node,
          name_buffer,
          4,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_write_description(
          client,
          native_object_node,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_node_id_parse(
          "ns=1;s=nativeStringNode",
          &parsed_node,
          parsed_string_buffer,
          sizeof(parsed_string_buffer),
          &required) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(cpkt_opcua_node_id_equal(parsed_node, constructed_string_node) == 1);
  CPKT_C89_CHECK(
      cpkt_opcua_node_id_print(parsed_node, node_id_buffer, 4, &required) == CPKT_OPCUA_ERR_RANGE);
  ...
  cpkt_opcua_value_string(&value, "string id updated", strlen("string id updated"));
  CPKT_C89_CHECK(cpkt_opcua_client_write(client, parsed_node, &value, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read(
          client,
          constructed_string_node,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_add_variable(
          ...
          client_added_variable_node,
          "clientAddedVariable",
          "Client Added Variable",
          &value,
          &status) == CPKT_OPCUA_OK);
  cpkt_opcua_value_byte_string(&value, cpkt_c89_native_bytes_value, sizeof(cpkt_c89_native_bytes_value));
  CPKT_C89_CHECK(
      cpkt_opcua_client_add_variable_under(
          client,
          client_added_child_variable_node,
          native_object_node,
          "clientAddedChildVariable",
          "Client Added Child Variable",
          &value,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_add_object_type(
          client,
          client_added_object_type_node,
          cpkt_opcua_node_id_numeric(0, 58),
          "clientAddedObjectType",
          "Client Added Object Type",
          1,
          &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_add_variable_type(
          ...
          "clientAddedVariableType",
          "Client Added Variable Type",
          &value,
          0,
          &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_add_variable_type(
          ...
          "Client Added Array Variable Type",
          &value,
          0,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_add_reference_type(
          ...
          "clientAddedReferenceType",
          "Client Added Reference Type",
          "Client Added Reference Type Inverse",
          0,
          0,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_add_data_type(
          client,
          client_added_data_type_node,
          cpkt_opcua_node_id_numeric(0, 24),
          "clientAddedDataType",
          "Client Added Data Type",
          1,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_add_view(
          ...
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_node_class(client, client_added_object_type_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_OBJECT_TYPE);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_node_class(client, client_added_variable_type_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_VARIABLE_TYPE);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_node_class(client, client_added_array_variable_type_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_VARIABLE_TYPE);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_value_rank(client, client_added_array_variable_type_node, &value_rank, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(value_rank == 1);
  CPKT_C89_CHECK(
  ...
  CPKT_C89_CHECK(dimensions[0] == 3UL);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_node_class(client, client_added_reference_type_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_REFERENCE_TYPE);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_node_class(client, client_added_data_type_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_DATA_TYPE);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_node_class(client, client_added_view_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_VIEW);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_is_abstract(client, client_added_object_type_node, &boolean_attribute, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(boolean_attribute == 1);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_inverse_name(
          ...
          &required,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(strcmp(name_buffer, "Client Added Reference Type Inverse") == 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_contains_no_loops(client, client_added_view_node, &boolean_attribute, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(boolean_attribute == 1);

  memset(&browse_seen, 0, sizeof(browse_seen));
  CPKT_C89_CHECK(
      cpkt_opcua_client_browse_children(
          client,
          cpkt_opcua_node_id_numeric(0, 85),
          cpkt_c89_browse_callback,
          &browse_seen,
          &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read(client, client_added_variable_node, &out, NULL, 0, NULL, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(out.type == CPKT_OPCUA_VALUE_INTEGER);
  CPKT_C89_CHECK(out.integer_value == 77);
  cpkt_opcua_value_integer(&value, 78);
  CPKT_C89_CHECK(cpkt_opcua_client_write(client, client_added_variable_node, &value, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read(client, client_added_variable_node, &out, NULL, 0, NULL, &status) ==
      CPKT_OPCUA_OK);
  ...
  memset(&browse_seen, 0, sizeof(browse_seen));
  CPKT_C89_CHECK(
      cpkt_opcua_client_browse_children_ex(
          client,
          native_object_node,
          &browse_options,
          cpkt_c89_browse_callback,
  ...
  CPKT_C89_CHECK(memcmp(out.bytes_value, cpkt_c89_native_bytes_value, out.bytes_length) == 0);

  CPKT_C89_CHECK(
      cpkt_opcua_client_delete_node(client, client_added_child_variable_node, 1, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_delete_node(client, client_added_variable_node, 1, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(cpkt_opcua_client_delete_node(client, client_added_node, 1, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_node_class(client, client_added_node, &node_class, &status) ==
      CPKT_OPCUA_ERR_UPSTREAM);
  memset(&browse_seen, 0, sizeof(browse_seen));
  CPKT_C89_CHECK(
      cpkt_opcua_client_browse_children(
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_add_reference_ex(
          client,
          native_object_node,
          cpkt_opcua_node_id_numeric(0, 47),
          1,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_browse_children_page(
          ...
          cpkt_c89_browse_callback,
          &browse_seen,
          small_continuation_point,
          sizeof(small_continuation_point),
          &required,
          &status) == CPKT_OPCUA_ERR_RANGE);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_browse_next(
          ...
          sizeof(next_continuation_point),
          &next_required,
          &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(cpkt_opcua_client_read(client, int_node, &out, NULL, 0, NULL, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(out.type == CPKT_OPCUA_VALUE_INTEGER);
  ...
  CPKT_C89_CHECK(cpkt_opcua_client_write(client, int_node, &value, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(cpkt_opcua_client_read(client, int_node, &out, NULL, 0, NULL, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(out.integer_value == 42);

  CPKT_C89_CHECK(
      cpkt_opcua_client_read(client, guid_value_node, &out, NULL, 0, NULL, &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(cpkt_opcua_client_write(client, guid_value_node, &value, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read(client, guid_value_node, &out, NULL, 0, NULL, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(memcmp(out.guid_value, cpkt_c89_facade_guid_node_id, 16) == 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_guid_array(
          client,
          guid_array_node,
          guid_array_values,
          2,
          ...
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(array_count == 2);
  CPKT_C89_CHECK(memcmp(guid_array_values[0].bytes, cpkt_c89_native_guid_node_id, 16) == 0);
  CPKT_C89_CHECK(memcmp(guid_array_values[1].bytes, cpkt_c89_facade_guid_node_id, 16) == 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_guid_array_range(
          ...
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(array_count == 1);
  CPKT_C89_CHECK(memcmp(guid_array_values[0].bytes, cpkt_c89_facade_guid_node_id, 16) == 0);

  CPKT_C89_CHECK(
      cpkt_opcua_client_read(client, status_value_node, &out, NULL, 0, NULL, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(out.type == CPKT_OPCUA_VALUE_STATUS);
  CPKT_C89_CHECK(out.status_value != 0);
  cpkt_opcua_value_status(&value, 0);
  CPKT_C89_CHECK(cpkt_opcua_client_write(client, status_value_node, &value, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read(client, status_value_node, &out, NULL, 0, NULL, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(out.status_value == 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_status_array(
          client,
          status_array_node,
          status_array_values,
          2,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read(client, uint64_value_node, &out, NULL, 0, NULL, &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(cpkt_opcua_client_write(client, uint64_value_node, &value, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read(client, uint64_value_node, &out, NULL, 0, NULL, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(out.type == CPKT_OPCUA_VALUE_UINT64);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_uint64_array(
          client,
          uint64_array_node,
          uint64_array_values,
          2,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read(client, datetime_value_node, &out, NULL, 0, NULL, &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(cpkt_opcua_client_write(client, datetime_value_node, &value, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read(client, datetime_value_node, &out, NULL, 0, NULL, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(out.type == CPKT_OPCUA_VALUE_DATETIME);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_datetime_array(
          client,
          datetime_array_node,
  ...
  cpkt_opcua_value_qualified_name(&value, CPKT_OPCUA_TEST_NS, "facadeQualified", strlen("facadeQualified"));
  CPKT_C89_CHECK(cpkt_opcua_client_write(client, qualified_name_value_node, &value, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read(
          client,
          qualified_name_value_node,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_qualified_name_array(
          client,
          qualified_name_array_node,
          cpkt_c89_qualified_name_array_callback,
          &qualified_name_array_seen,
          &array_count,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(array_count == 2);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_localized_text_array(
          client,
          localized_text_array_node,
          cpkt_c89_localized_text_array_callback,
          &localized_text_array_seen,
          &array_count,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(array_count == 2);
  ...
  cpkt_opcua_value_localized_text(&value, "en-US", strlen("en-US"), "facade text", strlen("facade text"));
  CPKT_C89_CHECK(cpkt_opcua_client_write(client, localized_text_value_node, &value, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read(
          client,
          localized_text_value_node,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read(client, native_child_node, &out, NULL, 0, NULL, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(out.type == CPKT_OPCUA_VALUE_INTEGER);
  ...
  CPKT_C89_CHECK(outputs[1].double_value > 6.49 && outputs[1].double_value < 6.51);

  for (lifecycle_iteration = 0; lifecycle_iteration < 3; ++lifecycle_iteration) {
    CPKT_C89_CHECK(
        cpkt_opcua_client_create_subscription(client, 50.0, &subscription_id, &status) ==
        CPKT_OPCUA_OK);
    CPKT_C89_CHECK(subscription_id != 0);
    CPKT_C89_CHECK(
        cpkt_opcua_client_modify_subscription(client, subscription_id, 25.0, &status) ==
        CPKT_OPCUA_OK);
    ...
    CPKT_C89_CHECK(
        cpkt_opcua_client_monitor_events(
            ...
            native_object_node,
            10.0,
            cpkt_c89_event_callback,
            &event_seen,
            &event_monitored_item_id,
            &status) == CPKT_OPCUA_OK);
    CPKT_C89_CHECK(event_monitored_item_id != 0);
    cpkt_opcua_value_integer(&inputs[0], 5);
    CPKT_C89_CHECK(
        cpkt_opcua_client_call_method(
            client,
            native_object_node,
            native_method_node,
            inputs,
    ...
    CPKT_C89_CHECK(
        cpkt_opcua_client_delete_monitored_item(client, subscription_id, event_monitored_item_id, &status) ==
        CPKT_OPCUA_OK);
    ...
    CPKT_C89_CHECK(
        cpkt_opcua_client_monitor_event_fields(
            ...
            event_field_names,
            3,
            cpkt_c89_event_fields_callback,
            &event_seen,
            &event_monitored_item_id,
            &status) == CPKT_OPCUA_OK);
    CPKT_C89_CHECK(event_monitored_item_id != 0);
    ...
    CPKT_C89_CHECK(
        cpkt_opcua_client_call_method(
            ...
            NULL,
            &status) == CPKT_OPCUA_OK);
    for (attempt = 0; attempt < 100 && !event_seen.saw_expected_fields; ++attempt) {
      (void)cpkt_opcua_client_run_iterate(client, 50, &status);
    }
    CPKT_C89_CHECK(event_seen.saw_expected_fields == 1);
    CPKT_C89_CHECK(event_seen.field_severity == 321L);
    CPKT_C89_CHECK(strcmp(event_seen.field_message, "native method event") == 0);
    CPKT_C89_CHECK(
        cpkt_opcua_client_delete_monitored_item(client, subscription_id, event_monitored_item_id, &status) ==
        CPKT_OPCUA_OK);
    memset(&subscription_seen, 0, sizeof(subscription_seen));
    CPKT_C89_CHECK(
        cpkt_opcua_client_monitor_value(
            client,
            subscription_id,
            int_node,
            10.0,
            cpkt_c89_subscription_callback,
            &subscription_seen,
            &monitored_item_id,
            &status) == CPKT_OPCUA_OK);
    ...
    CPKT_C89_CHECK(cpkt_opcua_client_write(client, int_node, &value, &status) == CPKT_OPCUA_OK);
    for (attempt = 0; attempt < 100 && !subscription_seen.saw_expected; ++attempt) {
      (void)cpkt_opcua_client_run_iterate(client, 50, &status);
    }
    CPKT_C89_CHECK(subscription_seen.saw_expected == 1);
    CPKT_C89_CHECK(subscription_seen.last_integer == 43);
    CPKT_C89_CHECK(
        cpkt_opcua_client_delete_monitored_item(client, subscription_id, monitored_item_id, &status) ==
        CPKT_OPCUA_OK);
    memset(&subscription_seen, 0, sizeof(subscription_seen));
    ...
    CPKT_C89_CHECK(
        cpkt_opcua_client_delete_monitored_item(client, subscription_id, monitored_item_id, &status) ==
        CPKT_OPCUA_OK);
    CPKT_C89_CHECK(cpkt_opcua_client_delete_subscription(client, subscription_id, &status) == CPKT_OPCUA_OK);
  }

  CPKT_C89_CHECK(cpkt_opcua_client_read(client, bool_node, &out, NULL, 0, NULL, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(out.type == CPKT_OPCUA_VALUE_BOOLEAN);
  CPKT_C89_CHECK(out.boolean_value == 1);
  cpkt_opcua_value_boolean(&value, 0);
  CPKT_C89_CHECK(cpkt_opcua_client_write(client, bool_node, &value, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(cpkt_opcua_client_read(client, bool_node, &out, NULL, 0, NULL, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(out.boolean_value == 0);

  CPKT_C89_CHECK(
      cpkt_opcua_client_read(client, bytes_node, &out, string_buffer, 2, &required, &status) ==
      CPKT_OPCUA_ERR_RANGE);
  ...
  cpkt_opcua_value_byte_string(
      &value,
      cpkt_c89_native_bytes_updated,
      sizeof(cpkt_c89_native_bytes_updated));
  CPKT_C89_CHECK(cpkt_opcua_client_write(client, bytes_node, &value, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read(
          client,
          bytes_node,
  ...
  CPKT_C89_CHECK(memcmp(out.bytes_value, cpkt_c89_native_bytes_updated, out.bytes_length) == 0);

  CPKT_C89_CHECK(
      cpkt_opcua_client_create_subscription(client, 50.0, &subscription_id, &status) == CPKT_OPCUA_OK);
  memset(&subscription_seen, 0, sizeof(subscription_seen));
  CPKT_C89_CHECK(
      cpkt_opcua_client_monitor_value(
          client,
  ...
  cpkt_opcua_value_byte_string(
      ...
      sizeof(cpkt_c89_native_bytes_updated));
  CPKT_C89_CHECK(cpkt_opcua_client_write(client, bytes_node, &value, &status) == CPKT_OPCUA_OK);
  for (attempt = 0; attempt < 100 && !subscription_seen.saw_expected_bytes; ++attempt) {
    (void)cpkt_opcua_client_run_iterate(client, 50, &status);
  }
  ...
  CPKT_C89_CHECK(cpkt_opcua_client_write(client, qualified_name_value_node, &value, &status) == CPKT_OPCUA_OK);
  for (attempt = 0; attempt < 100 && !subscription_seen.saw_expected_qualified_name; ++attempt) {
    (void)cpkt_opcua_client_run_iterate(client, 50, &status);
  }
  ...
  cpkt_opcua_value_localized_text(
      ...
      strlen("monitor text"));
  CPKT_C89_CHECK(cpkt_opcua_client_write(client, localized_text_value_node, &value, &status) == CPKT_OPCUA_OK);
  for (attempt = 0; attempt < 100 && !subscription_seen.saw_expected_localized_text; ++attempt) {
    (void)cpkt_opcua_client_run_iterate(client, 50, &status);
  }
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read(client, string_node, &out, string_buffer, 4, &required, &status) ==
      CPKT_OPCUA_ERR_RANGE);
  CPKT_C89_CHECK(required > 4);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read(
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read(
          client,
          cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, 9999),
          &out,
          NULL,
          0,
          NULL,
  ...
  CPKT_C89_CHECK(cpkt_opcua_client_disconnect(client, &status) == CPKT_OPCUA_OK);
...
int cpkt_opcua_c89_client_exercise_facade_server(const char *endpoint) {
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_find_server_application_name(
          ...
          &required,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(strcmp(server_buffer, CPKT_OPCUA_FACADE_APPLICATION_NAME) == 0);
  CPKT_C89_CHECK(
      cpkt_opcua_client_find_server_application_name(
          client,
          endpoint,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_get_namespace_uri(
          ...
          &required,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(strcmp(namespace_buffer, CPKT_OPCUA_FACADE_NAMESPACE_URI) == 0);

  int_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_INT_ID);
  object_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_OBJECT_ID);
  child_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_CHILD_ID);
  method_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_METHOD_ID);
  multi_method_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_MULTI_METHOD_ID);
  uint64_value_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_UINT64_VALUE_ID);
  array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_ARRAY_ID);

  CPKT_C89_CHECK(cpkt_opcua_client_read_node_class(client, object_node, &node_class, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_OBJECT);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_display_name(
          client,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_uint64_array_range(
          client,
          cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_UINT64_ARRAY_ID),
          "1",
          &uint64_value,
          1,
          &count,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_integer_array(
          client,
          array_node,
          integer_values,
          3,
  ...
  integer_values[1] = 31;
  integer_values[2] = 32;
  CPKT_C89_CHECK(cpkt_opcua_client_write_index_range(client, array_node, "1:2", &value, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_integer_array_range(
          client,
          array_node,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_executable(client, method_node, &executable, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(executable == 1);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_user_executable(client, method_node, &executable, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(executable == 1);
  CPKT_C89_CHECK(cpkt_opcua_client_write_executable(client, method_node, 0, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_executable(client, method_node, &executable, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(executable == 0);
  CPKT_C89_CHECK(cpkt_opcua_client_write_executable(client, method_node, 1, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_read_method_argument_count(
          client,
          method_node,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_call_method(client, object_node, method_node, inputs, 1, &out, NULL, 0, NULL, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(out.type == CPKT_OPCUA_VALUE_INTEGER);
  ...
  CPKT_C89_CHECK(outputs[1].type == CPKT_OPCUA_VALUE_DOUBLE);
  CPKT_C89_CHECK(outputs[1].double_value > 7.24 && outputs[1].double_value < 7.26);
  ...
  CPKT_C89_CHECK(async_seen.double_value > 8.24 && async_seen.double_value < 8.26);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_add_object_async(
          client,
          cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_ASYNC_OBJECT_ID),
          object_node,
          "facadeAsyncObject",
          "Facade Async Object",
          cpkt_c89_async_node_callback,
          &async_seen,
          &request_id,
          &async_node_out,
          string_buffer,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_add_variable_async(
          client,
          cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_ASYNC_VARIABLE_ID),
          object_node,
          "facadeAsyncVariable",
          "Facade Async Variable",
          &value,
          cpkt_c89_async_node_callback,
          &async_seen,
          &request_id,
  ...
  CPKT_C89_CHECK(async_seen.result == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(async_seen.node_seen == 1);
  CPKT_C89_CHECK(async_seen.node_numeric == CPKT_OPCUA_FACADE_ASYNC_VARIABLE_ID);
  CPKT_C89_CHECK(async_node_out.identifier_type == CPKT_OPCUA_NODE_ID_NUMERIC);
  CPKT_C89_CHECK(async_node_out.numeric == CPKT_OPCUA_FACADE_ASYNC_VARIABLE_ID);

  memset(&subscription_seen, 0, sizeof(subscription_seen));
  CPKT_C89_CHECK(cpkt_opcua_client_create_subscription(client, 20.0, &subscription_id, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_monitor_value(
          client,
          subscription_id,
...
int cpkt_opcua_c89_client_monitor_facade_server_events(
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_client_write_event_notifier(client, object_node, 1UL, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(cpkt_opcua_client_create_subscription(client, 20.0, &subscription_id, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(subscription_id != 0);
  ...
  CPKT_C89_CHECK(cpkt_opcua_client_delete_subscription(client, subscription_id, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_client_write_event_notifier(client, object_node, 0UL, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(cpkt_opcua_client_disconnect(client, &status) == CPKT_OPCUA_OK);
  cpkt_opcua_client_free(client);
  return 0;
...
int cpkt_opcua_c89_server_peer_new(
  ...
  int native_variant_seen;
  int native_data_value_seen;
  static const unsigned char json_config[] = "{ applicationDescription: { applicationUri: \"urn:cpkt:opcua:c89-json\" } }";
  char name_buffer[64];
  unsigned char continuation_point[64];
  unsigned char next_continuation_point[64];
  unsigned char small_continuation_point[1];
  ...
  *out = NULL;
  ...
  if (cpkt_opcua_server_new_from_json(&peer->server, json_config, sizeof(json_config) - 1, &status) !=
      CPKT_OPCUA_OK) {
    free(peer);
    return CPKT_C89_FAIL(__LINE__);
  }
  CPKT_C89_CHECK(cpkt_opcua_server_set_endpoint(peer->server, "127.0.0.1", port) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_set_application_identity(
          peer->server,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_set_access_control_callback(
          peer->server,
          1,
          cpkt_c89_login_callback,
          NULL,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_endpoint_url(
          peer->server,
          endpoint_buffer,
          (size_t)endpoint_buffer_size,
          &required) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_namespace(
          peer->server,
          CPKT_OPCUA_FACADE_NAMESPACE_URI,
  ...
  int_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_INT_ID);
  double_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_DOUBLE_ID);
  string_node = cpkt_opcua_node_id_string(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_STRING_NODE_ID);
  bytes_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_BYTES_ID);
  array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_ARRAY_ID);
  bool_array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_BOOL_ARRAY_ID);
  double_array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_DOUBLE_ARRAY_ID);
  string_array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_STRING_ARRAY_ID);
  bytes_array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_BYTES_ARRAY_ID);
  guid_value_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_GUID_VALUE_ID);
  guid_array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_GUID_ARRAY_ID);
  status_value_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_STATUS_VALUE_ID);
  status_array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_STATUS_ARRAY_ID);
  uint64_value_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_UINT64_VALUE_ID);
  uint64_array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_UINT64_ARRAY_ID);
  datetime_value_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_DATETIME_VALUE_ID);
  datetime_array_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_DATETIME_ARRAY_ID);
  qualified_name_value_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_QUALIFIED_NAME_VALUE_ID);
  qualified_name_array_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_QUALIFIED_NAME_ARRAY_ID);
  localized_text_value_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_LOCALIZED_TEXT_VALUE_ID);
  localized_text_array_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_LOCALIZED_TEXT_ARRAY_ID);
  object_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_OBJECT_ID);
  child_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_CHILD_ID);
  deleted_object_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_DELETED_OBJECT_ID);
  method_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_METHOD_ID);
  echo_method_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_ECHO_METHOD_ID);
  qualified_name_method_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_QUALIFIED_NAME_METHOD_ID);
  localized_text_method_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_LOCALIZED_TEXT_METHOD_ID);
  empty_method_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_EMPTY_METHOD_ID);
  multi_method_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_MULTI_METHOD_ID);
  object_type_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_OBJECT_TYPE_ID);
  variable_type_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_VARIABLE_TYPE_ID);
  array_variable_type_node =
      cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_ARRAY_VARIABLE_TYPE_ID);
  reference_type_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_REFERENCE_TYPE_ID);
  data_type_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_DATA_TYPE_ID);
  view_node = cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_VIEW_ID);
  guid_node = cpkt_opcua_node_id_guid(CPKT_OPCUA_TEST_NS, cpkt_c89_facade_guid_node_id);
  byte_node = cpkt_opcua_node_id_byte_string(
      CPKT_OPCUA_TEST_NS,
      cpkt_c89_facade_byte_node_id,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_mqtt_pubsub_connection(
          peer->server,
          &mqtt_options,
          &pubsub_connection_node,
          name_buffer,
          sizeof(name_buffer),
          &required,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(pubsub_connection_node.identifier_type == CPKT_OPCUA_NODE_ID_NUMERIC);
  CPKT_C89_CHECK(
  ...
  CPKT_C89_CHECK(pubsub_writer_node.identifier_type == CPKT_OPCUA_NODE_ID_NUMERIC);
  CPKT_C89_CHECK(pubsub_reader_node.identifier_type == CPKT_OPCUA_NODE_ID_NUMERIC);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_variable(
          ...
          &value,
          &status) == CPKT_OPCUA_OK);
  cpkt_opcua_value_string(&value, "facade string id", strlen("facade string id"));
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_variable(
          peer->server,
          string_node,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_variable(
          peer->server,
          array_node,
          "facadeArrayDimensions",
          "Facade Array Dimensions",
          &value,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_array_dimensions(peer->server, array_node, dimensions, 0, &dimension_count, &status) ==
      CPKT_OPCUA_ERR_RANGE);
  CPKT_C89_CHECK(dimension_count == 1);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_array_dimensions(peer->server, array_node, dimensions, 1, &dimension_count, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(dimension_count == 1);
  CPKT_C89_CHECK(dimensions[0] == 3UL);
  CPKT_C89_CHECK(
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_write_index_range(peer->server, array_node, "1:2", &value, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_integer_array(peer->server, array_node, array_values, 3, &dimension_count, &status) ==
      CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_variable(
          ...
          bool_array_node,
          "facadeBooleanArray",
          "Facade Boolean Array",
          &value,
          &status) == CPKT_OPCUA_OK);
  bool_array_values[0] = 0;
  bool_array_values[1] = 0;
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_boolean_array(
          peer->server,
          bool_array_node,
          bool_array_values,
          2,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_double_array(
          peer->server,
          double_array_node,
          double_array_values,
          2,
          ...
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(dimension_count == 2);
  CPKT_C89_CHECK(double_array_values[0] > 7.24 && double_array_values[0] < 7.26);
  CPKT_C89_CHECK(double_array_values[1] > 8.49 && double_array_values[1] < 8.51);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_double_array_range(
          ...
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(dimension_count == 1);
  CPKT_C89_CHECK(double_array_values[0] > 8.49 && double_array_values[0] < 8.51);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_string_array(
          peer->server,
          string_array_node,
          cpkt_c89_string_array_callback,
          &string_array_seen,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_variable(
          peer->server,
          bytes_array_node,
          "facadeByteStringArray",
          "Facade Byte String Array",
          &value,
          &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_variable(
          ...
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_guid_array(
          peer->server,
          guid_array_node,
          guid_array_values,
          2,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_variable(
          ...
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_status_array(
          peer->server,
          status_array_node,
          status_array_values,
          2,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_variable(
          ...
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_uint64_array(
          peer->server,
          uint64_array_node,
          uint64_array_values,
          2,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_variable(
          ...
          datetime_array_node,
          "facadeDateTimeArray",
          "Facade DateTime Array",
          &value,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_datetime_array(
          peer->server,
          datetime_array_node,
          datetime_array_values,
          3,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_variable(
          peer->server,
          qualified_name_value_node,
          "facadeQualifiedNameValue",
          "Facade Qualified Name Value",
          &value,
          &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_variable(
          peer->server,
          qualified_name_array_node,
          "facadeQualifiedNameArray",
          "Facade Qualified Name Array",
          &value,
          &status) == CPKT_OPCUA_OK);
  memset(&qualified_name_array_seen, 0, sizeof(qualified_name_array_seen));
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_qualified_name_array(
          peer->server,
          qualified_name_array_node,
          cpkt_c89_qualified_name_array_callback,
          &qualified_name_array_seen,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_variable(
          peer->server,
          localized_text_value_node,
          "facadeLocalizedTextValue",
          "Facade Localized Text Value",
          &value,
          &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_variable(
          peer->server,
          localized_text_array_node,
          "facadeLocalizedTextArray",
          "Facade Localized Text Array",
          &value,
          &status) == CPKT_OPCUA_OK);
  memset(&localized_text_array_seen, 0, sizeof(localized_text_array_seen));
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_localized_text_array(
          peer->server,
          localized_text_array_node,
          cpkt_c89_localized_text_array_callback,
          &localized_text_array_seen,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_object(
          ...
          cpkt_opcua_node_id_numeric(0, 85),
          "facadeDeletedObject",
          "Facade Deleted Object",
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(cpkt_opcua_server_delete_node(peer->server, deleted_object_node, 1, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_node_class(peer->server, deleted_object_node, &node_class, &status) ==
      CPKT_OPCUA_ERR_UPSTREAM);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_variable_under(
          ...
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_node_class(peer->server, object_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_OBJECT);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_node_class(peer->server, child_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_VARIABLE);
  CPKT_C89_CHECK(
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_variable_type(
          ...
          array_variable_type_node,
          cpkt_opcua_node_id_numeric(0, 62),
          "facadeArrayVariableType",
          "Facade Array Variable Type",
          &value,
          0,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_reference_type(
          ...
          cpkt_opcua_node_id_numeric(0, 31),
          "facadeReferenceType",
          "Facade Reference Type",
          "Facade Reference Type Inverse",
          0,
          0,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_data_type(
          peer->server,
          data_type_node,
          cpkt_opcua_node_id_numeric(0, 24),
          "facadeDataType",
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_view(
          ...
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_node_class(peer->server, object_type_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_OBJECT_TYPE);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_node_class(peer->server, variable_type_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_VARIABLE_TYPE);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_node_class(peer->server, array_variable_type_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_VARIABLE_TYPE);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_value_rank(peer->server, array_variable_type_node, &value_rank, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(value_rank == 1);
  CPKT_C89_CHECK(
  ...
  CPKT_C89_CHECK(dimensions[0] == 3UL);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_node_class(peer->server, reference_type_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_REFERENCE_TYPE);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_node_class(peer->server, data_type_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_DATA_TYPE);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_node_class(peer->server, view_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_VIEW);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_is_abstract(peer->server, object_type_node, &boolean_attribute, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(boolean_attribute == 1);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_is_abstract(peer->server, data_type_node, &boolean_attribute, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(boolean_attribute == 1);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_inverse_name(
          peer->server,
          reference_type_node,
          name_buffer,
          sizeof(name_buffer),
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_symmetric(peer->server, reference_type_node, &boolean_attribute, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(boolean_attribute == 0);
  CPKT_C89_CHECK(cpkt_opcua_server_write_symmetric(peer->server, reference_type_node, 1, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_symmetric(peer->server, reference_type_node, &boolean_attribute, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(boolean_attribute == 1);
  CPKT_C89_CHECK(cpkt_opcua_server_write_symmetric(peer->server, reference_type_node, 0, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_contains_no_loops(peer->server, view_node, &boolean_attribute, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(boolean_attribute == 1);
  CPKT_C89_CHECK(cpkt_opcua_server_write_contains_no_loops(peer->server, view_node, 0, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_contains_no_loops(peer->server, view_node, &boolean_attribute, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(boolean_attribute == 0);
  CPKT_C89_CHECK(cpkt_opcua_server_write_contains_no_loops(peer->server, view_node, 1, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_event_notifier(peer->server, view_node, &event_notifier, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(event_notifier == 0UL);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_data_type(peer->server, int_node, &data_type_node, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(data_type_node.namespace_index == 0);
  CPKT_C89_CHECK(data_type_node.identifier_type == CPKT_OPCUA_NODE_ID_NUMERIC);
  CPKT_C89_CHECK(data_type_node.numeric == CPKT_OPCUA_STANDARD_DATA_TYPE_INTEGER);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_data_type(peer->server, bytes_node, &data_type_node, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(data_type_node.namespace_index == 0);
  CPKT_C89_CHECK(data_type_node.identifier_type == CPKT_OPCUA_NODE_ID_NUMERIC);
  CPKT_C89_CHECK(data_type_node.numeric == CPKT_OPCUA_STANDARD_DATA_TYPE_BYTE_STRING);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_value_rank(peer->server, int_node, &value_rank, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(value_rank == -1);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_access_level(peer->server, int_node, &access_level, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK((access_level & 1UL) != 0);
  CPKT_C89_CHECK((access_level & 2UL) != 0);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_user_access_level(peer->server, int_node, &access_level, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK((access_level & 1UL) != 0);
  CPKT_C89_CHECK((access_level & 2UL) != 0);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_write_mask(peer->server, int_node, &write_mask, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK((write_mask & 1UL) != 0);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_user_write_mask(peer->server, int_node, &write_mask, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK((write_mask & 1UL) != 0);
  original_write_mask = write_mask;
  CPKT_C89_CHECK(
      cpkt_opcua_server_write_write_mask(peer->server, int_node, 1UL, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_write_mask(peer->server, int_node, &write_mask, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(write_mask == 1UL);
  CPKT_C89_CHECK(
      cpkt_opcua_server_write_write_mask(peer->server, int_node, original_write_mask, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_write_data_type(
          ...
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_write_value_rank(peer->server, int_node, -1, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_access_level_ex(peer->server, int_node, &access_level, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK((access_level & 1UL) != 0);
  CPKT_C89_CHECK((access_level & 2UL) != 0);
  CPKT_C89_CHECK(
      cpkt_opcua_server_write_access_level_ex(peer->server, int_node, 1UL, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_access_level_ex(peer->server, int_node, &access_level, &status) ==
      CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK((access_level & 2UL) == 0);
  CPKT_C89_CHECK(
      cpkt_opcua_server_write_access_level_ex(peer->server, int_node, 3UL, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
  ...
  CPKT_C89_CHECK(minimum_sampling_interval >= 0.0);
  CPKT_C89_CHECK(
      cpkt_opcua_server_write_minimum_sampling_interval(peer->server, int_node, 8.5, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_minimum_sampling_interval(
          peer->server,
          ...
          &minimum_sampling_interval,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(minimum_sampling_interval > 8.4 && minimum_sampling_interval < 8.6);
  CPKT_C89_CHECK(cpkt_opcua_server_write_access_level(peer->server, int_node, 1UL, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_access_level(peer->server, int_node, &access_level, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK((access_level & 1UL) != 0);
  CPKT_C89_CHECK((access_level & 2UL) == 0);
  CPKT_C89_CHECK(cpkt_opcua_server_write_access_level(peer->server, int_node, 3UL, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(cpkt_opcua_server_read_historizing(peer->server, int_node, &historizing, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(historizing == 0);
  CPKT_C89_CHECK(cpkt_opcua_server_write_historizing(peer->server, int_node, 1, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(cpkt_opcua_server_read_historizing(peer->server, int_node, &historizing, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(historizing == 1);
  CPKT_C89_CHECK(cpkt_opcua_server_write_historizing(peer->server, int_node, 0, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_browse_name(
          peer->server,
          object_node,
          &browse_namespace_index,
          name_buffer,
          sizeof(name_buffer),
          &required,
          &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_display_name(
          peer->server,
          object_node,
          name_buffer,
          4,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_write_display_name(
          peer->server,
          object_node,
          "Facade Object Updated",
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_display_name(
          peer->server,
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_write_description(
          peer->server,
          object_node,
          "Facade object description",
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_event_notifier(peer->server, object_node, &event_notifier, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(event_notifier == 0UL);
  CPKT_C89_CHECK(
      cpkt_opcua_server_write_event_notifier(peer->server, object_node, 1UL, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_event_notifier(peer->server, object_node, &event_notifier, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(event_notifier == 1UL);
  CPKT_C89_CHECK(
      cpkt_opcua_server_write_event_notifier(peer->server, object_node, 0UL, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_description(
          peer->server,
          object_node,
          name_buffer,
          sizeof(name_buffer),
  ...
  CPKT_C89_CHECK(strcmp(translated_node.string, CPKT_OPCUA_FACADE_STRING_NODE_ID) == 0);
  missing_path[0].namespace_index = CPKT_OPCUA_TEST_NS;
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_method(
          ...
          cpkt_c89_multiply_method,
          &method_factor,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_node_class(peer->server, method_node, &node_class, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(node_class == CPKT_OPCUA_NODE_CLASS_METHOD);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_executable(peer->server, method_node, &executable, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(executable == 1);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_user_executable(peer->server, method_node, &executable, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(executable == 1);
  CPKT_C89_CHECK(cpkt_opcua_server_write_executable(peer->server, method_node, 0, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_read_executable(peer->server, method_node, &executable, &status) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(executable == 0);
  CPKT_C89_CHECK(cpkt_opcua_server_write_executable(peer->server, method_node, 1, &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_method(
          ...
          "facadeEcho",
          "Facade Echo",
          string_method_input_types,
          1,
          CPKT_OPCUA_VALUE_STRING,
          cpkt_c89_echo_string_method,
          NULL,
          &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_method(
          peer->server,
          cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_BYTES_METHOD_ID),
          object_node,
          "facadeBytesEcho",
          "Facade Bytes Echo",
          bytes_method_input_types,
          1,
          CPKT_OPCUA_VALUE_BYTE_STRING,
          cpkt_c89_echo_bytes_method,
          NULL,
          &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_method(
          ...
          qualified_name_method_node,
          object_node,
          "facadeQualifiedNameEcho",
          "Facade QualifiedName Echo",
          qualified_name_method_input_types,
          1,
          CPKT_OPCUA_VALUE_QUALIFIED_NAME,
          cpkt_c89_echo_qualified_name_method,
          NULL,
          &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_method(
          ...
          localized_text_method_node,
          object_node,
          "facadeLocalizedTextEcho",
          "Facade LocalizedText Echo",
          localized_text_method_input_types,
          1,
          CPKT_OPCUA_VALUE_LOCALIZED_TEXT,
          cpkt_c89_echo_localized_text_method,
          NULL,
          &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_add_method(
...
int cpkt_opcua_c89_server_peer_start(cpkt_opcua_c89_server_peer *peer) {
  ...
  CPKT_C89_CHECK(peer != NULL);
  CPKT_C89_CHECK(cpkt_opcua_server_startup(peer->server, &status) == CPKT_OPCUA_OK);
  native_seen = 0;
  CPKT_C89_CHECK(
      cpkt_opcua_server_pubsub_native(peer->server, cpkt_c89_native_server_seen, &native_seen) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(native_seen == 1);
  native_seen = 0;
  CPKT_C89_CHECK(
      cpkt_opcua_server_history_native(peer->server, cpkt_c89_native_server_seen, &native_seen) ==
      CPKT_OPCUA_OK);
  CPKT_C89_CHECK(native_seen == 1);
  return 0;
}
...
int cpkt_opcua_c89_server_peer_iterate(cpkt_opcua_c89_server_peer *peer) {
  CPKT_C89_CHECK(peer != NULL);
  CPKT_C89_CHECK(cpkt_opcua_server_iterate(peer->server, 1, NULL) == CPKT_OPCUA_OK);
  return 0;
}
...
int cpkt_opcua_c89_server_peer_trigger_event(cpkt_opcua_c89_server_peer *peer) {
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_create_event(
          cpkt_opcua_node_id_numeric(CPKT_OPCUA_TEST_NS, CPKT_OPCUA_FACADE_OBJECT_ID),
          cpkt_opcua_node_id_numeric(0, 2041),
          654UL,
          "facade server event",
          &event,
          &status) == CPKT_OPCUA_OK);
  ...
  CPKT_C89_CHECK(
      cpkt_opcua_server_event_set_field(event, 0, "Message", &value, &status) == CPKT_OPCUA_OK);
  CPKT_C89_CHECK(
      cpkt_opcua_server_event_trigger(
          peer->server,
          event,
          event_id,
          sizeof(event_id),
...
int cpkt_opcua_c89_server_peer_shutdown(cpkt_opcua_c89_server_peer *peer) {
  ...
  CPKT_C89_CHECK(peer != NULL);
  CPKT_C89_CHECK(cpkt_opcua_server_shutdown(peer->server, &status) == CPKT_OPCUA_OK);
  return 0;
}
