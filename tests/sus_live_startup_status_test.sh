...
if ! grep -F -- "status source=default-capture mode=vox model_path=$missing_model language=en threshold=0.010 hang_ms=1500 prebuffer_ms=50" "$output" >/dev/null 2>&1; then
...
if ! grep -F -- "status source=default-capture mode=vox model=tiny cache=$cache_dir/ggml-tiny.bin cache_state=missing-offline language=en threshold=0.010 hang_ms=1500 prebuffer_ms=50" "$cache_output" >/dev/null 2>&1; then
...
if ! grep -F -- "status source=default-capture mode=vox model=tiny cache=$download_dir/ggml-tiny.bin cache_state=download language=en threshold=0.010 hang_ms=1500 prebuffer_ms=50" "$download_output" >/dev/null 2>&1; then
