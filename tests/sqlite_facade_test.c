...
static int vfs_probe_open(cpkt_sqlite_vfs *vfs, const char *name,
  ...
  if (state != 0) state->open_count += 1;
  return CPKT_SQLITE_CANTOPEN;
}

static int vfs_probe_delete(cpkt_sqlite_vfs *vfs, const char *name, int sync_directory) {
  (void) vfs;
  (void) name;
  (void) sync_directory;
...
static int vfs_probe_access(cpkt_sqlite_vfs *vfs, const char *name,
    int flags, int *result_out) {
  (void) vfs;
  (void) name;
  (void) flags;
  if (result_out != 0) *result_out = 0;
  return CPKT_SQLITE_OK;
}
...
static int vfs_probe_full_path(cpkt_sqlite_vfs *vfs, const char *name,
  ...
  (void) vfs;
  if (output_byte_count <= 0 || output == 0 || name == 0) return CPKT_SQLITE_CANTOPEN;
  output[0] = '\0';
  return CPKT_SQLITE_OK;
}

static int vfs_probe_randomness(cpkt_sqlite_vfs *vfs, int byte_count, char *output) {
  (void) vfs;
  if (byte_count > 0 && output != 0) memset(output, 0, (size_t) byte_count);
  return byte_count;
}
...
static int vfs_probe_current_time(cpkt_sqlite_vfs *vfs, double *julian_day_out) {
  (void) vfs;
  if (julian_day_out != 0) *julian_day_out = 0.0;
  return CPKT_SQLITE_OK;
}
...
static void page_cache_test_remove(page_cache_test_cache *cache,
  ...
  if (cache == 0 || page == 0) return;
  link = &cache->pages;
  while (*link != 0 && *link != page) link = &(*link)->next;
  if (*link == page) {
    *link = page->next;
    cache->page_count -= 1;
...
static int page_cache_test_initialize(void *context) {
  ...
  if (state == 0) return CPKT_SQLITE_MISUSE;
...
static void page_cache_test_shutdown(void *context) {
  page_cache_test_state *state;
  state = (page_cache_test_state *) context;
  if (state != 0) state->shutdown_count += 1;
}

static cpkt_sqlite_page_cache *page_cache_test_create(void *context,
    int page_byte_count, int extra_byte_count, int purgeable) {
  page_cache_test_state *state;
  page_cache_test_cache *cache;
  (void) purgeable;
  if (page_byte_count <= 0 || extra_byte_count < 0) return 0;
  state = (page_cache_test_state *) context;
  cache = (page_cache_test_cache *) calloc(1, sizeof(*cache));
  if (cache == 0) return 0;
  cache->cache.state = state;
  if (state != 0) state->create_count += 1;
  return &cache->cache;
}
...
static cpkt_sqlite_page *page_cache_test_fetch(cpkt_sqlite_page_cache *cache,
  ...
  page_cache_test_state *state;
  test_cache = (page_cache_test_cache *) cache;
  if (test_cache == 0 || key == 0) return 0;
  state = (page_cache_test_state *) test_cache->cache.state;
  if (state != 0) state->fetch_count += 1;
  page = page_cache_test_find(test_cache, key);
  if (page != 0 || create_flag == 0) return page == 0 ? 0 : &page->page;
...
static void page_cache_test_unpin(cpkt_sqlite_page_cache *cache,
  ...
  page_cache_test_state *state;
  test_cache = (page_cache_test_cache *) cache;
  if (test_cache == 0 || page == 0) return;
  state = (page_cache_test_state *) test_cache->cache.state;
  if (state != 0) state->unpin_count += 1;
  if (discard) page_cache_test_remove(test_cache, (page_cache_test_page *) page);
}

static void page_cache_test_rekey(cpkt_sqlite_page_cache *cache,
    cpkt_sqlite_page *page, unsigned long old_key, unsigned long new_key) {
  page_cache_test_page *test_page;
  (void) cache;
  (void) old_key;
  test_page = (page_cache_test_page *) page;
  if (test_page != 0) test_page->key = new_key;
}
...
static void page_cache_test_destroy(cpkt_sqlite_page_cache *cache) {
  ...
  page_cache_test_page *next;
  test_cache = (page_cache_test_cache *) cache;
  if (test_cache == 0) return;
  state = (page_cache_test_state *) test_cache->cache.state;
  if (state != 0) state->destroy_count += 1;
...
static int stream_input(void *context, void *buffer, int *byte_count) {
  ...
  if (count > 0) memcpy(buffer, (const char *) state->input + state->input_offset, count);
...
static int stream_output(void *context, const void *buffer, int byte_count) {
  stream_state *state;
  state = (stream_state *) context;
  if (state == 0 || buffer == 0 || byte_count < 0) return CPKT_SQLITE_MISUSE;
...
static int callback_count(void *context, int column_count,
  ...
  if (column_count != 2 || values == 0 || names == 0 ||
      strcmp(names[0], "name") != 0 || strcmp(values[0], "alpha") != 0) return 1;
  count = (int *) context;
  *count += 1;
  return 0;
}
...
static int needed_collation_compare(void *context, int left_byte_count,
  ...
  int byte_count;
  (void) context;
  byte_count = left_byte_count < right_byte_count ? left_byte_count : right_byte_count;
  compare = memcmp(left, right, (size_t) byte_count);
  if (compare != 0) return compare;
  if (left_byte_count < right_byte_count) return -1;
  if (left_byte_count > right_byte_count) return 1;
  return 0;
}

static void collation_needed(void *context, cpkt_sqlite *database,
    unsigned long text_representation, const char *name) {
  collation_needed_state *state;
  state = (collation_needed_state *) context;
  if (state == 0 || database == 0 || name == 0) return;
  state->count += 1;
  state->status = cpkt_sqlite_create_collation(database, name,
      text_representation, 0, needed_collation_compare, 0);
}
...
static unsigned long autovacuum_callback(void *context, const char *schema_name,
  ...
  if (state != 0) state->callback_count += 1;
...
static void trace_callback(void *context, cpkt_sqlite *database,
  ...
  if (state == 0) return;
  if (event == CPKT_SQLITE_TRACE_STATEMENT && sql != 0) state->statement_count += 1;
  if (event == CPKT_SQLITE_TRACE_ROW && sql != 0) state->row_count += 1;
  if (event == CPKT_SQLITE_TRACE_PROFILE && sql != 0) state->profile_count += 1;
}

static void legacy_trace_callback(void *context, const char *sql) {
  trace_state *state;
  state = (trace_state *) context;
  if (state != 0 && sql != 0) state->statement_count += 1;
}
...
static void legacy_profile_callback(void *context, const char *sql,
  ...
  (void) elapsed_nanoseconds;
  state = (trace_state *) context;
  if (state != 0 && sql != 0) state->profile_count += 1;
}
...
static void unlock_notify_callback(void *context, int context_count,
  ...
  state->context_matches = context_count == 1 && contexts != 0 &&
      contexts[0] == context;
}
...
static void virtual_table_module_destroy(void *context) {
  ...
  if (state != 0) state->module_destroy_count += 1;
}
...
static int virtual_table_connect(void *context, cpkt_sqlite *database,
  ...
  (void) arguments;
  (void) error_out;
  if (context == 0 || database == 0 || out == 0) return CPKT_SQLITE_MISUSE;
  table = cpkt_sqlite_virtual_table_new(context);
  if (table == 0) return CPKT_SQLITE_NOMEM;
  if (cpkt_sqlite_declare_virtual_table(database,
      "create table x(value integer)") != CPKT_SQLITE_OK ||
      cpkt_sqlite_virtual_table_config_int(database,
      CPKT_SQLITE_VTAB_CONSTRAINT_SUPPORT, 1) != CPKT_SQLITE_OK) {
    free(table);
...
static int virtual_table_best_index(void *context, cpkt_sqlite_virtual_table *table,
    cpkt_sqlite_index_info *index_info) {
  virtual_table_state *state;
  state = (virtual_table_state *) context;
  if (state == 0 || table == 0 || index_info == 0) return CPKT_SQLITE_MISUSE;
...
static int virtual_table_open(void *context, cpkt_sqlite_virtual_table *table,
  ...
  if (context == 0 || table == 0 || out == 0) return CPKT_SQLITE_MISUSE;
  *out = cpkt_sqlite_virtual_cursor_new(table, context);
  return *out == 0 ? CPKT_SQLITE_NOMEM : CPKT_SQLITE_OK;
}

static int virtual_table_close(void *context, cpkt_sqlite_virtual_cursor *cursor) {
  (void) context;
  return cursor == 0 ? CPKT_SQLITE_MISUSE : CPKT_SQLITE_OK;
}

static int virtual_table_filter(void *context, cpkt_sqlite_virtual_cursor *cursor,
  ...
  (void) arguments;
  state = (virtual_table_state *) context;
  if (state == 0 || cursor == 0) return CPKT_SQLITE_MISUSE;
  state->filter_count += 1;
  state->eof = 0;
  return CPKT_SQLITE_OK;
}

static int virtual_table_next(void *context, cpkt_sqlite_virtual_cursor *cursor) {
  virtual_table_state *state;
  state = (virtual_table_state *) context;
  if (state == 0 || cursor == 0) return CPKT_SQLITE_MISUSE;
  state->eof = 1;
...
static int virtual_table_eof(void *context, cpkt_sqlite_virtual_cursor *cursor) {
  virtual_table_state *state;
  state = (virtual_table_state *) context;
  if (state == 0 || cursor == 0) return 1;
  return state->eof;
}

static int virtual_table_column(void *context, cpkt_sqlite_virtual_cursor *cursor,
...
static int virtual_table_rowid(void *context, cpkt_sqlite_virtual_cursor *cursor,
    cpkt_sqlite_i64 *rowid_out) {
  if (context == 0 || cursor == 0 || rowid_out == 0) return CPKT_SQLITE_MISUSE;
...
static int virtual_table_update(void *context, cpkt_sqlite_virtual_table *table,
  ...
  if (state == 0 || table == 0 || argument_count < 3 || arguments == 0 ||
      rowid_out == 0) return CPKT_SQLITE_MISUSE;
  state->update_count += 1;
  *rowid_out = cpkt_sqlite_i64_make(0, 2);
  return CPKT_SQLITE_OK;
}

static int virtual_table_disconnect(void *context, cpkt_sqlite_virtual_table *table) {
  virtual_table_state *state;
  state = (virtual_table_state *) context;
  if (state == 0 || table == 0) return CPKT_SQLITE_MISUSE;
  state->disconnect_count += 1;
  return CPKT_SQLITE_OK;
}

static int rtree_contains_point(void *context, cpkt_sqlite_rtree_geometry *geometry,
    int coordinate_count, double *coordinates, int *within_out) {
  rtree_state *state;
  state = (rtree_state *) context;
  if (state == 0 || geometry == 0 || coordinate_count != 4 ||
      geometry->parameter_count != 2 || geometry->parameters == 0 ||
      coordinates == 0 || within_out == 0) return CPKT_SQLITE_MISUSE;
  state->callback_count += 1;
...
static int rtree_score_point(void *context, cpkt_sqlite_rtree_query *query) {
  ...
  if (state == 0 || query == 0 || query->parameter_count != 2 ||
      ...
      query->coordinate_count != 4) return CPKT_SQLITE_MISUSE;
  state->query_count += 1;
...
static void plus_one(cpkt_sqlite_context *context, int argument_count,
  ...
  int increment;
  if (argument_count != 1 || arguments == 0 || arguments[0] == 0 || user_data == 0) {
  ...
  cpkt_sqlite_context_result_int(context,
      cpkt_sqlite_value_int(arguments[0]) + increment);
}
...
static void bound_argument(cpkt_sqlite_context *context, int argument_count,
    cpkt_sqlite_value *const *arguments, void *user_data) {
  int *was_bound;
  if (context == 0 || argument_count != 1 || arguments == 0 || arguments[0] == 0 ||
      user_data == 0) {
    cpkt_sqlite_context_result_error_code(context, CPKT_SQLITE_MISUSE);
    return;
  }
...
static void auxiliary_data(cpkt_sqlite_context *context, int argument_count,
  ...
  if (count == 0) {
    count = (int *) user_data;
    if (cpkt_sqlite_context_set_auxdata(context, 0, count, 0) != CPKT_SQLITE_OK) {
...
static void aggregate_counter_step(cpkt_sqlite_context *context, int argument_count,
    cpkt_sqlite_value *const *arguments, void *user_data) {
  int *last_count;
  (void) arguments;
  if (context == 0 || argument_count != 1 || user_data == 0) return;
  last_count = (int *) user_data;
  *last_count = cpkt_sqlite_context_aggregate_count(context);
}

static void aggregate_counter_final(cpkt_sqlite_context *context, int argument_count,
    cpkt_sqlite_value *const *arguments, void *user_data) {
  (void) arguments;
  (void) user_data;
  if (context == 0 || argument_count != 0) return;
  cpkt_sqlite_context_result_int(context, cpkt_sqlite_context_aggregate_count(context));
}
...
static int virtual_table_find_function(void *context,
  ...
  if (state == 0 || table == 0 || function_out == 0 || user_data_out == 0 ||
      name == 0 || argument_count != 1 || strcmp(name, "cpkt_vtab_plus_one") != 0) {
    return 0;
  }
...
static void update_count(void *context, int operation, const char *database_name,
    const char *table_name, cpkt_sqlite_i64 row_id) {
  int *count;
  (void) database_name;
  (void) row_id;
  count = (int *) context;
  if (operation == 18 && table_name != 0 && strcmp(table_name, "item") == 0) *count += 1;
}

static void preupdate_noop(void *context, cpkt_sqlite *database, int operation,
    const char *database_name, const char *table_name, cpkt_sqlite_i64 old_row_id,
...
static void preupdate_blob_write(void *context, cpkt_sqlite *database, int operation,
    const char *database_name, const char *table_name, cpkt_sqlite_i64 old_row_id,
    cpkt_sqlite_i64 new_row_id) {
...
static int fts5_tokenizer_create(void *context, const char *const *arguments,
    int argument_count, cpkt_sqlite_fts5_tokenizer **out) {
  fts5_state *state;
  (void) arguments;
  (void) argument_count;
  state = (fts5_state *) context;
  if (state == 0 || out == 0) return CPKT_SQLITE_MISUSE;
  *out = cpkt_sqlite_fts5_tokenizer_new(state);
  if (*out == 0) return CPKT_SQLITE_NOMEM;
...
static int fts5_tokenizer_tokenize(cpkt_sqlite_fts5_tokenizer *tokenizer,
    ...
    const char *locale, int locale_byte_count, cpkt_sqlite_fts5_token_callback token) {
  fts5_state *state;
  (void) flags;
  (void) locale;
  (void) locale_byte_count;
  state = (fts5_state *) cpkt_sqlite_fts5_tokenizer_state(tokenizer);
  if (state == 0 || context == 0 || text == 0 || text_byte_count < 0 || token == 0) {
    return CPKT_SQLITE_MISUSE;
  }
...
static void fts5_binding_destroy(void *context) {
  fts5_state *state;
  state = (fts5_state *) context;
  if (state != 0) state->binding_destroy_count += 1;
}
...
static void fts5_auxiliary(cpkt_sqlite_fts5_context *fts_context,
  ...
  if (state == 0 || fts_context == 0 || sql_context == 0 || argument_count != 0 ||
      arguments != 0 || fts_context->user_data(fts_context) != state ||
      ...
      fts_context->instance_count(fts_context, &instance_count) != CPKT_SQLITE_OK ||
      instance_count != 1 ||
      fts_context->instance(fts_context, 0, &phrase, &column, &offset) != CPKT_SQLITE_OK ||
      phrase != 0 || column != 0 || offset != 0 ||
      fts_context->column_text(fts_context, 0, &text, &text_byte_count) != CPKT_SQLITE_OK ||
      text == 0 || text_byte_count != 5 || memcmp(text, "alpha", 5) != 0 ||
      fts_context->query_token(fts_context, 0, 0, &text, &text_byte_count) != CPKT_SQLITE_OK ||
      text == 0 || text_byte_count != 5 || memcmp(text, "alpha", 5) != 0) {
...
int main(void) {
  ...
  if (cpkt_sqlite_shutdown() != CPKT_SQLITE_OK ||
      cpkt_sqlite_global_config_memory_methods_get(&memory_methods) != CPKT_SQLITE_OK ||
      ...
      memory_methods.shutdown == 0) return 36;
  if (cpkt_sqlite_memory_alarm(0, 0, cpkt_sqlite_i64_make(0, 0))
      != CPKT_SQLITE_OK) return 81;
  log.call_count = 0;
  log.error_code = 0;
  if (cpkt_sqlite_global_config_log(record_log, &log) != CPKT_SQLITE_OK) return 82;
  cpkt_sqlite_log(CPKT_SQLITE_NOTICE, "facade log %d", 7);
  if (log.call_count != 1 || log.error_code != CPKT_SQLITE_NOTICE) return 83;
  ...
  if (cpkt_sqlite_global_config_page_cache_methods_set(&page_cache_methods)
      ...
      configured_page_cache_methods.create != page_cache_test_create) return 39;
  (void) remove("cpkt-sqlite-page-cache-test.db");
  page_cache_database = cpkt_sqlite_new("cpkt-sqlite-page-cache-test.db");
  if (page_cache_database == 0 || page_cache_database->tx(page_cache_database,
      "create table cache_test (id integer primary key, value text);"
      "insert into cache_test values(1, 'page cache');", 0, 0) != CPKT_SQLITE_OK) {
    if (page_cache_database != 0) page_cache_database->close(page_cache_database);
    return 40;
  }
  ...
  if (page_cache_state.initialize_count != 1 ||
      ...
      page_cache_state.unpin_count == 0 || page_cache_state.destroy_count == 0) return 41;
  mutex = cpkt_sqlite_mutex_new(CPKT_SQLITE_MUTEX_FAST);
  if (mutex == 0 || mutex->try_enter(mutex) != CPKT_SQLITE_OK ||
      mutex->held(mutex) == 0) return 37;
  mutex->leave(mutex);
  if (mutex->not_held(mutex) == 0) return 38;
  mutex->close(mutex);
  formatted = cpkt_sqlite_format("%q:%d", "a'b", 7);
  if (formatted == 0 || strcmp(formatted, "a''b:7") != 0 ||
      cpkt_sqlite_format_into((int) sizeof(format_buffer), format_buffer,
      "%s", "format") != format_buffer ||
      strcmp(format_buffer, "format") != 0) return 45;
  ...
  if (vfs == 0 || vfs->register_vfs(vfs, 0) != CPKT_SQLITE_OK) return 74;
  if (cpkt_sqlite_vfs_find("cpkt-vfs-probe") != vfs) return 85;
  replica = cpkt_sqlite_open("cpkt-vfs-probe.db", CPKT_SQLITE_OPEN_READWRITE |
      CPKT_SQLITE_OPEN_CREATE, "cpkt-vfs-probe");
  if (replica == 0 || replica->error_code(replica) != CPKT_SQLITE_CANTOPEN ||
      vfs_probe.open_count != 1) return 75;
  replica->close(replica);
  if (vfs->unregister_vfs(vfs) != CPKT_SQLITE_OK) return 76;
  vfs->close(vfs);
  ...
  if (auto_extension == 0 || auto_extension->register_extension(auto_extension)
      != CPKT_SQLITE_OK) return 79;
  db = cpkt_sqlite_open(":memory:", CPKT_SQLITE_OPEN_READWRITE |
      CPKT_SQLITE_OPEN_CREATE | CPKT_SQLITE_OPEN_FULLMUTEX, 0);
  if (db == 0 || db->error_code(db) != CPKT_SQLITE_OK ||
      cpkt_sqlite_error16(db) == 0) return 1;
  string = cpkt_sqlite_string_new(db);
  if (string == 0) return 86;
  string->append_format(string, "%s:%d", "string", 7);
  if (string->value(string) == 0 || strcmp(string->value(string), "string:7") != 0 ||
      string->error_code(string) != CPKT_SQLITE_OK) return 87;
  string->close(string);
  statement = 0;
  if (auto_extension_registration.call_count != 1 ||
      db->prepare(db, "select auto_plus_one(4)", -1, 0, &statement, 0)
      != CPKT_SQLITE_OK || statement == 0 || statement->step(statement)
      != CPKT_SQLITE_ROW || statement->column_int(statement, 0) != 5 ||
      statement->finalize(statement) != CPKT_SQLITE_OK ||
      auto_extension->cancel(auto_extension) != 1) return 80;
  auto_extension->close(auto_extension);
  mutex = cpkt_sqlite_database_mutex(db);
  if (mutex == 0 || mutex->try_enter(mutex) != CPKT_SQLITE_OK ||
      mutex->held(mutex) == 0) return 63;
  mutex->leave(mutex);
  mutex->close(mutex);
  ...
  if (cpkt_sqlite_create_collation16(db, utf16_collation_name,
      CPKT_SQLITE_UTF16, 0, needed_collation_compare) != CPKT_SQLITE_OK ||
      db->tx(db, "select 'a' union all select 'b' order by 1 collate utf16_cmp",
      0, 0) != CPKT_SQLITE_OK) return 71;
  ...
  if (cpkt_sqlite_create_function16(db, utf16_function_name, 1,
      CPKT_SQLITE_UTF16, &increment, plus_one, 0, 0) != CPKT_SQLITE_OK ||
      db->prepare(db, "select utf16_function(4)", -1, 0, &statement, 0)
      != CPKT_SQLITE_OK || statement == 0 || statement->step(statement)
      != CPKT_SQLITE_ROW || statement->column_int(statement, 0) != 5 ||
      statement->finalize(statement) != CPKT_SQLITE_OK) return 78;
  ...
  if (cpkt_sqlite_create_function(db, "utf16_error", 0, CPKT_SQLITE_UTF8,
      utf16_error_text, utf16_error, 0, 0, 0) != CPKT_SQLITE_OK ||
      db->prepare(db, "select utf16_error()", -1, 0, &statement, 0)
      != CPKT_SQLITE_OK || statement == 0 || statement->step(statement)
      != CPKT_SQLITE_ERROR || strcmp(db->error(db), "wide error") != 0 ||
      statement->finalize(statement) != CPKT_SQLITE_ERROR) return 76;
  utf16le_result[0] = 'l';
  utf16le_result[1] = 'e';
  utf16le_result[2] = 0;
  utf16be_result[0] = 0;
  ...
  if (cpkt_sqlite_create_function(db, "utf16le_text", 0, CPKT_SQLITE_UTF8,
      utf16le_result, utf16le_text, 0, 0, 0) != CPKT_SQLITE_OK ||
      cpkt_sqlite_create_function(db, "utf16be_text", 0, CPKT_SQLITE_UTF8,
      utf16be_result, utf16be_text, 0, 0, 0) != CPKT_SQLITE_OK ||
      db->prepare(db, "select utf16le_text(), utf16be_text()", -1, 0,
      &statement, 0) != CPKT_SQLITE_OK || statement == 0 ||
      ...
      statement->finalize(statement) != CPKT_SQLITE_OK) return 77;
  if (db->set_error(db, CPKT_SQLITE_ERROR, "facade error")
      != CPKT_SQLITE_OK || db->error_code(db) != CPKT_SQLITE_ERROR ||
      strcmp(db->error(db), "facade error") != 0) return 72;
  was_bound = 0;
  statement = 0;
  if (cpkt_sqlite_create_function(db, "bound_argument", 1, CPKT_SQLITE_UTF8,
      &was_bound, bound_argument, 0, 0, 0) != CPKT_SQLITE_OK ||
      db->prepare(db, "select bound_argument(?1)", -1, 0, &statement, 0)
      != CPKT_SQLITE_OK || statement == 0 || statement->bind_int(statement, 1, 3)
      != CPKT_SQLITE_OK || statement->step(statement) != CPKT_SQLITE_ROW ||
      statement->column_int(statement, 0) != 1 || was_bound != 1 ||
      statement->finalize(statement) != CPKT_SQLITE_OK) return 75;
  auxdata_count = 0;
  statement = 0;
  if (cpkt_sqlite_create_function(db, "auxiliary_data", 1, CPKT_SQLITE_UTF8,
      &auxdata_count, auxiliary_data, 0, 0, 0) != CPKT_SQLITE_OK ||
      db->prepare(db, "select auxiliary_data(?1) from (select 1 union all select 2)",
      -1, 0, &statement, 0) != CPKT_SQLITE_OK || statement == 0 ||
      statement->bind_int(statement, 1, 9) != CPKT_SQLITE_OK ||
      statement->step(statement) != CPKT_SQLITE_ROW ||
      statement->column_int(statement, 0) != 1 || statement->step(statement)
      != CPKT_SQLITE_ROW || statement->column_int(statement, 0) != 2 ||
      auxdata_count != 2 || statement->finalize(statement) != CPKT_SQLITE_OK) return 78;
  aggregate_count = 0;
  statement = 0;
  if (cpkt_sqlite_create_function(db, "aggregate_counter", 1, CPKT_SQLITE_UTF8,
      &aggregate_count, 0, aggregate_counter_step, aggregate_counter_final, 0)
      != CPKT_SQLITE_OK || db->prepare(db,
      "select aggregate_counter(value) from (select 1 as value union all select 2)",
      -1, 0, &statement, 0) != CPKT_SQLITE_OK || statement == 0 ||
      statement->step(statement) != CPKT_SQLITE_ROW ||
      statement->column_int(statement, 0) != 2 || aggregate_count != 2 ||
      statement->finalize(statement) != CPKT_SQLITE_OK) return 79;
  deferred_close_database = cpkt_sqlite_new(":memory:");
  increment = 41;
  statement = 0;
  if (deferred_close_database == 0 || cpkt_sqlite_create_function(
      deferred_close_database, "plus_one", 1, CPKT_SQLITE_UTF8, &increment,
      plus_one, 0, 0, 0) != CPKT_SQLITE_OK ||
      deferred_close_database->prepare(deferred_close_database,
      "select plus_one(1)", -1, 0, &statement, 0) != CPKT_SQLITE_OK ||
      statement == 0 || statement->database(statement) != deferred_close_database) {
    return 64;
  }
  ...
  if (statement->database(statement) != deferred_close_database ||
      ...
      statement->finalize(statement) != CPKT_SQLITE_OK) return 65;
  deferred_close_database = cpkt_sqlite_new(":memory:");
  blob = 0;
  if (deferred_close_database == 0 || deferred_close_database->tx(
      deferred_close_database, "create table deferred_blob(value blob);"
      "insert into deferred_blob values(x'78797a');", 0, 0) != CPKT_SQLITE_OK ||
      cpkt_sqlite_open_blob(deferred_close_database, "main", "deferred_blob",
      "value", cpkt_sqlite_i64_make(0, 1), 0, &blob) != CPKT_SQLITE_OK ||
      blob == 0 || blob->database != deferred_close_database) return 66;
  deferred_close_database->close(deferred_close_database);
  ...
  deferred_backup = 0;
  if (backup_source == 0 || backup_destination == 0 || backup_source->tx(
      backup_source, "create table deferred_backup(value integer);"
      "insert into deferred_backup values(7);", 0, 0) != CPKT_SQLITE_OK ||
      cpkt_sqlite_backup_start(backup_destination, "main", backup_source, "main",
      &deferred_backup) != CPKT_SQLITE_OK || deferred_backup == 0 ||
      deferred_backup->destination != backup_destination ||
      deferred_backup->source != backup_source) return 68;
  backup_source->close(backup_source);
  backup_destination->close(backup_destination);
  if (deferred_backup->close(deferred_backup) != CPKT_SQLITE_OK) return 69;
  strict_close_database = cpkt_sqlite_new(":memory:");
  statement = 0;
  if (strict_close_database == 0 || strict_close_database->prepare(
      strict_close_database, "select 8", -1, 0, &statement, 0)
      != CPKT_SQLITE_OK || statement == 0 ||
      cpkt_sqlite_close_strict(strict_close_database) != CPKT_SQLITE_BUSY ||
      statement->step(statement) != CPKT_SQLITE_ROW ||
      statement->column_int(statement, 0) != 8 ||
      statement->finalize(statement) != CPKT_SQLITE_OK ||
      cpkt_sqlite_close_strict(strict_close_database) != CPKT_SQLITE_OK) return 74;
  statement = 0;
  other_statement = 0;
  if (db->prepare(db, "select 6", -1, 0, &statement, 0) != CPKT_SQLITE_OK ||
      statement == 0 || cpkt_sqlite_next_statement(db, 0, &other_statement)
      != CPKT_SQLITE_OK || other_statement == 0 || other_statement == statement ||
      ...
      statement->finalize(statement) != CPKT_SQLITE_OK) return 70;
  configuration_result = 0;
  if (cpkt_sqlite_global_config_int_out(
      CPKT_SQLITE_GLOBAL_CONFIG_PCACHE_HEADER_SIZE, &configuration_result)
      != CPKT_SQLITE_OK || configuration_result <= 0) return 35;
  configuration_result = 0;
  if (cpkt_sqlite_database_config_int(db,
      CPKT_SQLITE_DATABASE_CONFIG_ENABLE_FOREIGN_KEYS, 1, &configuration_result)
      != CPKT_SQLITE_OK || configuration_result != 1) return 34;
  if (cpkt_sqlite_set_lock_timeout(db, 10, 0) != CPKT_SQLITE_OK) return 42;
  collation_state.count = 0;
  collation_state.status = CPKT_SQLITE_OK;
  if (cpkt_sqlite_set_collation_needed16(db, 0, 0) != CPKT_SQLITE_OK ||
      cpkt_sqlite_set_collation_needed(db, collation_needed, &collation_state)
      != CPKT_SQLITE_OK || db->tx(db,
      "select 'b' as value union all select 'a' order by value collate cpkt_needed",
      0, 0) != CPKT_SQLITE_OK || collation_state.count != 1 ||
      collation_state.status != CPKT_SQLITE_OK) return 46;
  ...
  if (cpkt_sqlite_set_autovacuum_callback(db, autovacuum_callback, &autovacuum,
      ...
      autovacuum.destroy_count != 1) return 47;
  autovacuum.destroy_count = 0;
  if (cpkt_sqlite_set_client_data(db, "cpkt.client", &autovacuum,
      ...
      autovacuum.destroy_count != 1) return 54;
  trace.statement_count = 0;
  trace.row_count = 0;
  trace.profile_count = 0;
  if (cpkt_sqlite_set_trace(db, CPKT_SQLITE_TRACE_STATEMENT |
      CPKT_SQLITE_TRACE_ROW | CPKT_SQLITE_TRACE_PROFILE, trace_callback, &trace)
      != CPKT_SQLITE_OK || db->tx(db, "select 'trace'", 0, 0) != CPKT_SQLITE_OK ||
      trace.statement_count == 0 || trace.row_count == 0 || trace.profile_count == 0 ||
      cpkt_sqlite_set_trace(db, 0, 0, 0) != CPKT_SQLITE_OK) return 48;
  trace.statement_count = 0;
  trace.profile_count = 0;
  if (cpkt_sqlite_set_legacy_trace(db, legacy_trace_callback, &trace) != 0 ||
      db->tx(db, "select 'legacy trace'", 0, 0) != CPKT_SQLITE_OK ||
      trace.statement_count == 0 ||
      cpkt_sqlite_set_legacy_profile(db, legacy_profile_callback, &trace) != &trace ||
      db->tx(db, "select 'legacy profile'", 0, 0) != CPKT_SQLITE_OK ||
      trace.profile_count == 0 || cpkt_sqlite_set_legacy_profile(db, 0, 0) != &trace) {
    return 84;
  }
  ...
  if (cpkt_sqlite_create_module(db, "cpkt_test_vtab", &virtual_methods)
      ...
      "create virtual table cpkt_virtual using cpkt_test_vtab;", 0, 0)
      != CPKT_SQLITE_OK || db->prepare(db, "select value from cpkt_virtual", -1,
      0, &statement, 0) != CPKT_SQLITE_OK || statement == 0 ||
      statement->step(statement) != CPKT_SQLITE_ROW ||
      statement->column_int(statement, 0) != 42 ||
      statement->step(statement) != CPKT_SQLITE_DONE ||
      statement->finalize(statement) != CPKT_SQLITE_OK ||
      db->tx(db, "insert into cpkt_virtual values (9);", 0, 0) != CPKT_SQLITE_OK ||
      ...
      virtual_table_shadow_name_count == 0) return 61;
  rtree.callback_count = 0;
  rtree.query_count = 0;
  if (cpkt_sqlite_register_rtree_geometry(db, "cpkt_contains_point",
      rtree_contains_point, &rtree) != CPKT_SQLITE_OK || db->tx(db,
      ...
      0, 0) != CPKT_SQLITE_OK || rtree.callback_count == 0) return 49;
  if (cpkt_sqlite_register_rtree_query(db, "cpkt_score_point", rtree_score_point,
      &rtree, 0) != CPKT_SQLITE_OK || db->tx(db,
      "select id from boxes where min_x match cpkt_score_point(5, 5);", 0, 0)
      != CPKT_SQLITE_OK || rtree.query_count == 0) return 50;
  filename_parameters[0] = "cache";
  filename_parameters[1] = "shared";
  filename = cpkt_sqlite_filename_new("main.db", "main.db-journal", "main.db-wal", 1,
      filename_parameters);
  if (filename == 0 || filename->database(filename) == 0 ||
      ...
      strcmp(filename->uri_parameter(filename, "cache"), "shared") != 0) return 2;
  ...
  fts_api = 0;
  if (fts_database == 0 || cpkt_sqlite_fts5_api_open(fts_database, &fts_api)
      != CPKT_SQLITE_OK || fts_api == 0 || fts_api->version(fts_api) < 3 ||
      fts_api->create_tokenizer(fts_api, "cpkt_test", &fts_state,
      fts5_tokenizer_create, fts5_tokenizer_destroy, fts5_tokenizer_tokenize,
      ...
      fts_database->tx(fts_database,
      "create virtual table fts using fts5(content, tokenize='cpkt_test');"
      "insert into fts values('alpha');", 0, 0) != CPKT_SQLITE_OK ||
      fts_database->prepare(fts_database,
      "select count(*) from fts where fts match 'alpha'", -1, 0, &statement, 0)
      != CPKT_SQLITE_OK || statement == 0 ||
      statement->step(statement) != CPKT_SQLITE_ROW ||
      ...
      statement->step(statement) != CPKT_SQLITE_ROW ||
      statement->column_int(statement, 0) != 1 ||
      statement->finalize(statement) != CPKT_SQLITE_OK || fts_state.auxiliary_count != 1 ||
      fts_state.create_count == 0 || fts_state.tokenize_count == 0) return 32;
  fts_api->close(fts_api);
  fts_database->close(fts_database);
  if (fts_state.destroy_count == 0 || fts_state.binding_destroy_count != 1) return 33;
  utf16_filename[0] = ':';
  utf16_filename[1] = 'm';
  utf16_filename[2] = 'e';
  utf16_filename[3] = 'm';
  ...
  if (utf16_database == 0 || utf16_database->tx(utf16_database,
      "create table utf16_open_test (value integer);", 0, 0) != CPKT_SQLITE_OK) {
    if (utf16_database != 0) utf16_database->close(utf16_database);
    return 57;
  }
  utf16_sql[0] = 's';
  utf16_sql[1] = 'e';
  ...
  utf16_sql[8] = 0;
  statement = 0;
  if (utf16_database->prepare16(utf16_database, utf16_sql, -1, 0,
      &statement, 0) != CPKT_SQLITE_OK || statement == 0 ||
      statement->step(statement) != CPKT_SQLITE_ROW ||
      statement->column_int(statement, 0) != 9 ||
      statement->finalize(statement) != CPKT_SQLITE_OK ||
      cpkt_sqlite_drop_modules(utf16_database, 0, 0) != CPKT_SQLITE_OK) return 58;
  utf16_database->close(utf16_database);
  (void) remove("cpkt-sqlite-unlock-notify-test.db");
  if (cpkt_sqlite_enable_shared_cache(1) != CPKT_SQLITE_OK) return 55;
  unlock_database = cpkt_sqlite_open("cpkt-sqlite-unlock-notify-test.db",
      CPKT_SQLITE_OPEN_READWRITE | CPKT_SQLITE_OPEN_CREATE |
      CPKT_SQLITE_OPEN_SHAREDCACHE, 0);
  unlock_blocked = cpkt_sqlite_open("cpkt-sqlite-unlock-notify-test.db",
  ...
  if (unlock_database == 0 || unlock_blocked == 0 ||
      unlock_database->tx(unlock_database,
      "begin immediate; create table unlock_test (value integer);", 0, 0)
      != CPKT_SQLITE_OK ||
      ...
      unlock_notify.context_matches == 0) return 56;
  unlock_blocked->close(unlock_blocked);
  unlock_database->close(unlock_database);
  (void) cpkt_sqlite_enable_shared_cache(0);
  (void) remove("cpkt-sqlite-unlock-notify-test.db");
  if (db->tx(db, "create table item (name text, value integer, payload blob)", 0, 0)
      != CPKT_SQLITE_OK) return 3;
  if (db->overload_function(db, "cpkt_overload_probe", 1) != CPKT_SQLITE_OK) return 59;
  if (cpkt_sqlite_table_column_metadata(db, "main", "item", "name",
      ...
      column_metadata.auto_increment != 0) return 43;
  table = 0;
  if (cpkt_sqlite_get_table(db, "select 'alpha' as name, 1 as value", &table, 0)
      != CPKT_SQLITE_OK || table == 0 || table->row_count(table) != 1 ||
      ...
      strcmp(table->value(table, 0, 1), "1") != 0) return 44;
  table->close(table);
  statement = 0;
  if (db->prepare(db, "insert into item values (?1, ?2, ?3)", -1,
      CPKT_SQLITE_PREPARE_PERSISTENT, &statement, 0) != CPKT_SQLITE_OK ||
      statement == 0) return 4;
  ...
  if (statement->bind_text(statement, 1, "alpha", -1) != CPKT_SQLITE_OK ||
      ...
      statement->step(statement) != CPKT_SQLITE_DONE ||
      statement->finalize(statement) != CPKT_SQLITE_OK) return 5;
  statement = 0;
  if (db->prepare(db, "select ?1", -1, 0, &statement, 0) != CPKT_SQLITE_OK ||
      statement == 0) return 51;
  other_statement = 0;
  if (db->prepare(db, "select ?1", -1, 0, &other_statement, 0)
      != CPKT_SQLITE_OK || other_statement == 0) return 52;
  if (statement->bind_text(statement, 1, "transfer", -1) != CPKT_SQLITE_OK ||
      ...
      != CPKT_SQLITE_OK || other_statement->step(other_statement) != CPKT_SQLITE_ROW ||
      strcmp((const char *) other_statement->column_text(other_statement, 0),
      "transfer") != 0 || statement->finalize(statement) != CPKT_SQLITE_OK ||
      other_statement->finalize(other_statement) != CPKT_SQLITE_OK) return 53;
  if (db->changes(db) != 1) return 6;
  statement = 0;
  if (db->prepare(db, "select value, payload from item", -1, 0, &statement, 0)
      != CPKT_SQLITE_OK || statement == 0 || statement->step(statement) != CPKT_SQLITE_ROW)
    return 7;
  ...
  if (actual.high != expected.high || actual.low != expected.low ||
      ...
      statement->expired(statement) != 0 || statement->is_explain(statement) != 0 ||
      statement->status(statement, CPKT_SQLITE_STATEMENT_STATUS_VM_STEP, 0) <= 0 ||
      statement->scan_status_i64(statement, 0,
      CPKT_SQLITE_SCAN_STATUS_LOOP_COUNT, 0, &actual) != CPKT_SQLITE_OK ||
      cpkt_sqlite_i64_compare(actual, cpkt_sqlite_i64_make(0, 1)) < 0 ||
      statement->finalize(statement) != CPKT_SQLITE_OK) return 8;
  count = 0;
  if (db->tx(db, "select name, value from item", callback_count, &count) != CPKT_SQLITE_OK ||
      count != 1) return 9;
  carray_values[0] = 7;
  carray_values[1] = 8;
  statement = 0;
  if (db->prepare(db, "select value from carray(?1)", -1, 0, &statement, 0)
      ...
      statement->column_int(statement, 0) != 8 ||
      statement->step(statement) != CPKT_SQLITE_DONE ||
      statement->finalize(statement) != CPKT_SQLITE_OK) return 10;
  utf16_text[0] = (unsigned short) 'o';
  utf16_text[1] = (unsigned short) 'k';
  utf16_text[2] = 0;
  statement = 0;
  if (db->prepare(db, "select ?1", -1, 0, &statement, 0) != CPKT_SQLITE_OK ||
      statement == 0 || statement->bind_text16(statement, 1, utf16_text, 4)
      != CPKT_SQLITE_OK || statement->step(statement) != CPKT_SQLITE_ROW ||
      statement->column_bytes16(statement, 0) != 4 ||
      statement->column_text16(statement, 0) == 0 ||
      statement->finalize(statement) != CPKT_SQLITE_OK) return 11;
  blob = 0;
  if (cpkt_sqlite_open_blob(db, "main", "item", "payload", cpkt_sqlite_i64_make(0, 1),
      0, &blob) != CPKT_SQLITE_OK || blob == 0 || blob->bytes(blob) != 3 ||
      blob->read(blob, blob_buffer, 3, 0) != CPKT_SQLITE_OK ||
      memcmp(blob_buffer, "xyz", 3) != 0 || blob->close(blob) != CPKT_SQLITE_OK) return 12;
  increment = 1;
  if (cpkt_sqlite_create_function(db, "plus_one", 1, CPKT_SQLITE_UTF8, &increment,
      plus_one, 0, 0, 0) != CPKT_SQLITE_OK) return 13;
  statement = 0;
  if (db->prepare(db, "select plus_one(4)", -1, 0, &statement, 0) != CPKT_SQLITE_OK ||
      statement == 0 || statement->step(statement) != CPKT_SQLITE_ROW ||
      statement->column_int(statement, 0) != 5 ||
      statement->finalize(statement) != CPKT_SQLITE_OK) return 14;
  if (db->tx(db, "create table session_item (id integer primary key, value text)", 0, 0)
      != CPKT_SQLITE_OK) return 15;
  session = 0;
  if (cpkt_sqlite_set_preupdate_hook(db, preupdate_noop, 0) != CPKT_SQLITE_OK ||
      cpkt_sqlite_session_new(db, "main", &session) != CPKT_SQLITE_MISUSE || session != 0 ||
      cpkt_sqlite_set_preupdate_hook(db, 0, 0) != CPKT_SQLITE_OK) return 16;
  if (cpkt_sqlite_session_new(db, "main", &session) != CPKT_SQLITE_OK || session == 0 ||
      (size_config = 1, session->object_config(session, 1, &size_config)) != CPKT_SQLITE_OK ||
      session->attach(session, "session_item") != CPKT_SQLITE_OK ||
      db->tx(db, "insert into session_item values (1, 'session')", 0, 0) != CPKT_SQLITE_OK)
    return 17;
  changeset = 0;
  if (session->changeset(session, &changeset) != CPKT_SQLITE_OK || changeset == 0 ||
      changeset->byte_count <= 0) return 18;
  iterator = 0;
  if (changeset->iterator(changeset, &iterator) != CPKT_SQLITE_OK || iterator == 0 ||
      iterator->next(iterator) != CPKT_SQLITE_ROW) return 19;
  table_name = 0;
  column_count = 0;
  operation = 0;
  indirect = 0;
  if (iterator->operation(iterator, &table_name, &column_count, &operation, &indirect)
      != CPKT_SQLITE_OK || table_name == 0 || strcmp(table_name, "session_item") != 0 ||
      column_count != 2 || operation != 18 || indirect != 0 ||
      (primary_key = 0, primary_key_count = 0,
       iterator->primary_key(iterator, &primary_key, &primary_key_count)) != CPKT_SQLITE_OK ||
      primary_key == 0 || primary_key_count != 2 || primary_key[0] != 1 ||
      iterator->close(iterator) != CPKT_SQLITE_OK) return 20;
  if (session->changeset_size(session).high != 0 || session->changeset_size(session).low == 0 ||
      session->memory_used(session).high != 0 || session->memory_used(session).low == 0) return 21;
  stream.input = 0;
  stream.input_size = 0;
  stream.input_offset = 0;
  stream.output_size = 0;
  if (session->changeset_stream(session, stream_output, &stream) != CPKT_SQLITE_OK ||
      stream.output_size != changeset->byte_count) return 22;
  replica = cpkt_sqlite_new(":memory:");
  if (replica == 0 || replica->tx(replica,
      "create table session_item (id integer primary key, value text)", 0, 0)
      != CPKT_SQLITE_OK) return 23;
  rebase = 0;
  if (cpkt_sqlite_changeset_apply_ex(replica, changeset, 0, 0, 0, 0, &rebase)
      != CPKT_SQLITE_OK || rebase != 0 ||
      replica->prepare(replica, "select count(*) from session_item", -1, 0,
      &statement, 0) != CPKT_SQLITE_OK || statement == 0 ||
      statement->step(statement) != CPKT_SQLITE_ROW ||
      statement->column_int(statement, 0) != 1 ||
      statement->finalize(statement) != CPKT_SQLITE_OK) return 24;
  replica->close(replica);
  replica = cpkt_sqlite_new(":memory:");
  if (replica == 0 || replica->tx(replica,
      "create table session_item (id integer primary key, value text)", 0, 0)
      != CPKT_SQLITE_OK || cpkt_sqlite_changeset_apply_v3(replica, changeset,
      accept_changeset_iterator, 0, 0, 0, &rebase) != CPKT_SQLITE_OK ||
      rebase != 0) return 25;
  replica->close(replica);
  ...
  stream.output_size = 0;
  iterator = 0;
  if (cpkt_sqlite_changeset_start_stream(stream_input, &stream, 0, &iterator) != CPKT_SQLITE_OK ||
      iterator == 0 || iterator->next(iterator) != CPKT_SQLITE_ROW ||
      iterator->close(iterator) != CPKT_SQLITE_OK || stream.input_offset != stream.input_size) return 26;
  inverted = 0;
  combined = 0;
  if (cpkt_sqlite_changeset_invert(changeset, &inverted) != CPKT_SQLITE_OK || inverted == 0) {
    return 27;
  }
  status = cpkt_sqlite_changeset_concat(changeset, inverted, &combined);
  if (status != CPKT_SQLITE_OK) return 40 + status;
  if (combined == 0) return 28;
  changegroup = 0;
  grouped = 0;
  if (cpkt_sqlite_changegroup_new(&changegroup) != CPKT_SQLITE_OK || changegroup == 0 ||
      changegroup->add(changegroup, changeset) != CPKT_SQLITE_OK ||
      changegroup->output(changegroup, &grouped) != CPKT_SQLITE_OK || grouped == 0 ||
      grouped->byte_count <= 0) return 29;
  grouped->free(grouped);
  changegroup->close(changegroup);
  combined->free(combined);
  inverted->free(inverted);
  changeset->free(changeset);
  if (cpkt_sqlite_set_preupdate_hook(db, preupdate_noop, 0) != CPKT_SQLITE_MISUSE) return 30;
  session->close(session);
  preupdate_blob.call_count = 0;
  preupdate_blob.column = -1;
  blob = 0;
  if (cpkt_sqlite_set_preupdate_hook(db, preupdate_blob_write, &preupdate_blob)
      != CPKT_SQLITE_OK || cpkt_sqlite_open_blob(db, "main", "item", "payload",
      cpkt_sqlite_i64_make(0, 1), 1, &blob) != CPKT_SQLITE_OK || blob == 0 ||
      blob->write(blob, "Q", 1, 0) != CPKT_SQLITE_OK ||
      blob->close(blob) != CPKT_SQLITE_OK || preupdate_blob.call_count != 1 ||
      preupdate_blob.column != 2 || cpkt_sqlite_set_preupdate_hook(db, 0, 0)
      != CPKT_SQLITE_OK) return 73;
  count = 0;
  cpkt_sqlite_set_update_hook(db, update_count, &count);
  if (db->tx(db, "insert into item values ('beta', 2, x'00')", 0, 0)
      != CPKT_SQLITE_OK || count != 1) return 31;
  cpkt_sqlite_set_update_hook(db, 0, 0);
  db->close(db);
  if (virtual_table.module_destroy_count != 1) return 62;
  if (cpkt_sqlite_global_recover() != CPKT_SQLITE_OK ||
      cpkt_sqlite_sleep(0) < 0) return 60;
