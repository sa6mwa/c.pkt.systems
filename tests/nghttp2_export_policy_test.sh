...
if [[ ! -f "$library" || ! -f "$allowlist" ]]; then
  printf 'nghttp2 export-policy inputs are missing\n' >&2
  exit 2
fi
...
if [[ "$actual" != "$expected" ]]; then
  printf 'libcpkt_nghttp2 dynamic export allowlist mismatch\nexpected:\n%s\nactual:\n%s\n' \
    "$expected" "$actual" >&2
  exit 1
fi

repo_root=$(cd "$(dirname "$0")/.." && pwd)
work_root=$(mktemp -d "$repo_root/build/nghttp2-export-policy.XXXXXX")
...
printf '%s\n' \
  ...
  'int main(void) { return (int)cpkt_nghttp2_native_u64(); }' \
...
if sentinel_output=$("${compiler_args[@]}" "$work_root/private-sentinel.c" \
    -L "$(dirname "$library")" -lcpkt_nghttp2 \
    -o "$work_root/private-sentinel" 2>&1); then
  printf 'private nghttp2 facade sentinel linked unexpectedly\n' >&2
  exit 1
fi
if [[ "$sentinel_output" != *cpkt_nghttp2_native_u64* ]]; then
