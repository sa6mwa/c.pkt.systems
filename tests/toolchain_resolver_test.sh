...
llvm_runtime="$llvm_root/lib/clang/22/lib/x86_64-unknown-linux-gnu"
mkdir -p "$llvm_root/bin" "$llvm_runtime"
for tool in clang clang++ ld.lld llvm-ar llvm-ranlib llvm-strip llvm-nm llvm-objcopy llvm-objdump llvm-addr2line llvm-readelf; do make_executable "$llvm_root/bin/$tool" '#!/bin/sh\nexit 0'; done
: > "$llvm_runtime/libclang_rt.asan.a"; : > "$llvm_runtime/libclang_rt.fuzzer.a"; : > "$llvm_runtime/libclang_rt.msan.a"
