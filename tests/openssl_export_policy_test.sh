...
if [[ ! -f "$library" || ! -f "$allowlist" ]]; then
  printf 'OpenSSL export-policy inputs are missing\n' >&2
  exit 2
fi
...
if [[ "$actual" != "$expected" ]]; then
  printf 'libcpkt_openssl dynamic export allowlist mismatch\nexpected:\n%s\nactual:\n%s\n' \
    "$expected" "$actual" >&2
  exit 1
fi
...
work_root=$(mktemp -d "$repo_root/build/openssl-export-policy.XXXXXX")
...
printf '%s\n' \
  ...
  'int main(void) { return (int) cpkt_openssl_native_u64(); }' \
...
if sentinel_output=$("${compiler_args[@]}" "$work_root/private-sentinel.c" \
    -L "$(dirname "$library")" -lcpkt_openssl \
    -o "$work_root/private-sentinel" 2>&1); then
  printf 'private OpenSSL facade sentinel linked unexpectedly\n' >&2
  exit 1
fi
if [[ "$sentinel_output" != *cpkt_openssl_native_u64* ]]; then
