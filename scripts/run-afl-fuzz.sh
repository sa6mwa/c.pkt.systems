...
if crash_file=$(find "$output_dir" -type f -path '*/crashes/id:*' -print -quit); then
  if [ -n "$crash_file" ]; then
    printf 'AFL++ recorded a crashing input: %s\n' "$crash_file" >&2
    exit 1
  fi
else
  printf 'failed to inspect AFL++ crash findings: %s\n' "$output_dir" >&2
  exit 1
fi
