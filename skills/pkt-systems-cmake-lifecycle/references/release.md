...
- Tag-dependent version and manifest contract tests are ordinary tests, not late package-verification tests. Run them before `release-matrix` so a broken lightweight-tag contract fails before expensive artifact builds and before the final release tag is created. These tests should create a temporary lightweight semver tag such as `v99.99.99` on the current `HEAD`, verify Make/CMake/script surfaces from that tagged state, and delete the tag through a trap. Treat the impossible version tag as reserved test-only state: delete any stale copy before untagged assertions, recreate it only inside the tagged assertion block, and delete it again before continuing. If `HEAD` already has an exact release tag, assert that the exact tag wins and skip the temporary-tag block. Do not use extra git checkouts, git worktrees, copied repositories, or generated source archives to simulate an exact tag on `HEAD`.
...
- Add focused pre-matrix tests for lightweight-tag version behavior and tag-derived artifact names. Use a temporary lightweight tag on the current `HEAD` and clean it with a trap. Remove any stale reserved test tag before untagged assertions, then recreate and delete it only around the tagged assertions. Do not create another checkout or worktree for this; extra checkout topology can hide the real release-branch `HEAD` contract and makes failures arrive too late in the release.
...
1. `make clean release` when the repository defines `release`. This is a clean release rehearsal on the candidate branch, not a publishable tagged release. In an untagged git worktree it may resolve the version as `0.0.0` or use an explicit project-prefixed release-candidate override when supplied, but it must exercise the same build, test, package, checksum, and verification graph as the final release.
   - The ordinary-test portion of this gate must include the temporary-lightweight-tag version checks before the release matrix starts.
