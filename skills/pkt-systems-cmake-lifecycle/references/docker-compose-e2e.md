...
For lightweight single-service developer dependencies, `compose/<service>/` is also an acceptable location for static service config. The same generated-state and release-exclusion rules apply.
...
Optional external tool e2e:

- Containerized inspectors, protocol conformance tools, browser-like tools, or upstream CLIs may be used as opt-in e2e gates.
- They must be guarded by a project-prefixed opt-in environment variable.
- They must use temporary work directories, ephemeral project-owned ports when possible, and cleanup traps.
