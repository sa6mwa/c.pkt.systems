...
- Darwin toolchain files should also set `CMAKE_LINKER` to the target `${host}-ld` and inject an absolute `-fuse-ld=${CMAKE_LINKER}` into executable, shared-library, and module linker flags. Keep the `PATH` fix and the `-fuse-ld` fix together: the former protects compiler-driver and upstream build-system discovery, while the latter makes generated CMake link lines auditable.
