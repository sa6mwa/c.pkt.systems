...
- Lightweight-tag version checks must be early ordinary tests in `test`, `test-all`, `prerelease`, and therefore the pre-matrix portion of `release`. They must not live only in `package-verify`, because that makes failures arrive after artifact builds. A correct test creates a temporary lightweight semver tag on the current `HEAD`, for example `v99.99.99`, validates script, Make, and CMake version surfaces that should observe an exact tag, and removes the tag with a trap. Treat that impossible version tag as reserved test-only state: remove any stale copy before untagged assertions, recreate it only inside the tagged assertion block, and delete it again before later assertions continue. When `HEAD` already has an exact release tag, test that the exact tag takes precedence and skip the temporary-tag block. Do not use extra git checkouts, git worktrees, copied repositories, or source-archive staging as a substitute for testing exact-tag behavior on the active release candidate.
