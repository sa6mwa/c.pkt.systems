...
2. Use a CMake custom command or CMake-script generator to convert that file
   into a build-tree `.inc` file containing the bytes of one C array initializer.
   Make the source asset, generator, and generated `.inc` explicit build
...
3. Include that generated file directly in a single project-owned C89 array,
   ...
   requires a C string, the generated bytes must include exactly one trailing
   NUL; otherwise expose the array together with its explicit byte length.
...
The generator must preserve every source byte in order and produce deterministic
output. Test the generated asset against the canonical file for exact length and
content, including the trailing-NUL contract when applicable. Test code may use
