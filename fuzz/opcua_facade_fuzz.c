...
static const char *cpkt_fuzz_c_string(struct cpkt_fuzz_cursor *cursor, char *buffer, size_t buffer_size) {
  size_t length;
  size_t i;
...
static void cpkt_fuzz_guid(struct cpkt_fuzz_cursor *cursor, unsigned char guid[16]) {
  size_t i;
...
static cpkt_opcua_node_id cpkt_fuzz_node_id(
  ...
  switch (cpkt_fuzz_u8(cursor) % 5U) {
    ...
    default:
      ...
      return cpkt_opcua_node_id_byte_string(namespace_index, byte_buffer, byte_count);
  }
}
...
static void cpkt_fuzz_node_id_roundtrip(struct cpkt_fuzz_cursor *cursor) {
  ...
  node_id = cpkt_fuzz_node_id(cursor, id_storage, sizeof(id_storage), bytes, sizeof(bytes));
  (void)cpkt_opcua_node_id_equal(node_id, node_id);
  if (cpkt_opcua_node_id_print(node_id, id_text, sizeof(id_text), &required) == CPKT_OPCUA_OK) {
    (void)cpkt_opcua_node_id_parse(
        id_text,
        &parsed,
        parsed_storage,
        cpkt_fuzz_size(cursor, sizeof(parsed_storage)),
        &required);
  }
  (void)cpkt_opcua_node_id_parse(
      cpkt_fuzz_c_string(cursor, id_text, sizeof(id_text)),
      &parsed,
      parsed_storage,
      cpkt_fuzz_size(cursor, sizeof(parsed_storage)),
      &required);
...
static void cpkt_fuzz_expanded_node_id_roundtrip(struct cpkt_fuzz_cursor *cursor) {
  ...
  node_id = cpkt_fuzz_node_id(cursor, node_storage, sizeof(node_storage), bytes, sizeof(bytes));
  switch (cpkt_fuzz_u8(cursor) % 4U) {
    ...
    case 1:
      ...
      expanded = cpkt_opcua_expanded_node_id_uri(namespace_uri, strlen(namespace_uri), node_id);
      break;
    case 2:
      expanded = cpkt_opcua_expanded_node_id_server(cpkt_fuzz_ulong(cursor), node_id);
      break;
    default:
      cpkt_fuzz_c_string(cursor, namespace_uri, sizeof(namespace_uri));
  ...
  if (cpkt_opcua_expanded_node_id_print(expanded, id_text, sizeof(id_text), &required) ==
      CPKT_OPCUA_OK) {
...
static void cpkt_fuzz_text_helpers(struct cpkt_fuzz_cursor *cursor) {
  ...
  cpkt_fuzz_guid(cursor, guid);
  if (cpkt_opcua_guid_print(guid, text, sizeof(text), &required) == CPKT_OPCUA_OK) {
    (void)cpkt_opcua_guid_parse(text, parsed_guid);
  }
  (void)cpkt_opcua_guid_parse(cpkt_fuzz_c_string(cursor, text, sizeof(text)), parsed_guid);
  ...
  (void)cpkt_opcua_qualified_name_print(
      ...
      strlen(name),
      text,
      cpkt_fuzz_size(cursor, sizeof(text)),
      &required);
  (void)cpkt_opcua_qualified_name_parse(
      text,
      &namespace_index,
      parsed,
...
static void cpkt_fuzz_values(struct cpkt_fuzz_cursor *cursor) {
  ...
  switch (cpkt_fuzz_u8(cursor) % 23U) {
    ...
    case 8:
      ...
      break;
    case 9:
      cpkt_opcua_value_uint64(&value, cpkt_fuzz_ulong(cursor), cpkt_fuzz_ulong(cursor));
      break;
    case 10:
      cpkt_opcua_value_datetime(&value, cpkt_fuzz_long(cursor), cpkt_fuzz_ulong(cursor));
      break;
    case 11:
    ...
    case 20:
      ...
      for (i = 0; i < count; ++i) {
        cpkt_fuzz_c_string(cursor, strings[i], sizeof(strings[i]));
        qualified_names[i].namespace_index = (unsigned short)cpkt_fuzz_ulong(cursor);
...
int cpkt_fuzz_one_input(const unsigned char *data, size_t size) {
  ...
  (void)cpkt_opcua_result_string((cpkt_opcua_result)(cpkt_fuzz_u8(&cursor) % 12U));
  return 0;
}
